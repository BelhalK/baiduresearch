#python3 misso.py --batchsize=128 --nbepochs=3 --nbruns=1 --ntrains=30000
...
with tf.Session() as sess:
    ...
    for _ in range(nb_runs):
      ...
      loss, kl, accuracy = run_experiment(algo='rmsprop', 
  	                         fake_data=fake_data, 
  	                         ...
  	                         verbose= True)
      misso.append(loss)
      misso_acc.append(accuracy)
    with open('losses/misso', 'wb') as fp: 
      pickle.dump(misso, fp)
    with open('losses/missoacc', 'wb') as fp: 
      pickle.dump(misso_acc, fp)

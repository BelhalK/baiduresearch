...
# python3 run.py --num_epochs 2 --viz_steps 200 --num_monte_carlo 5 --optimizer sgd --learning_rate 0.01 --lr_choice cyclical
# python3 run.py --num_epochs 2 --viz_steps 200 --num_monte_carlo 5 --optimizer hwa --start_avg 10 --avg_period 10 --lr_choice cyclical
...
from tfp.models import bayesian_resnet
from tfp.models import bayesian_vgg
...
class CIFARSequence(tf.keras.utils.Sequence):
  """Produces a sequence of CIFAR with labels."""

  def __init__(self, data=None, batch_size=128, fake_data_size=None):
    images, labels = data
    self.images, self.labels = CIFARSequence.__preprocessing(
        images, labels)
...
def main(argv):
  ...
  train_set, heldout_set = tf.keras.datasets.cifar10.load_data()
  
  train_seq = CIFARSequence(data=train_set, batch_size=FLAGS.batch_size)
  heldout_seq = CIFARSequence(data=heldout_set, batch_size=FLAGS.batch_size)
  ...
  # resnet model
  input_shape = [32, 32, 3]
  # model = bayesian_resnet.bayesian_resnet(input_shape,num_classes=10,kernel_posterior_scale_mean=-9.0,kernel_posterior_scale_stddev=0.1,kernel_posterior_scale_constraint=0.2)
  model = bayesian_vgg.bayesian_vgg(input_shape,num_classes=10,kernel_posterior_scale_mean=-9.0,kernel_posterior_scale_stddev=0.1,kernel_posterior_scale_constraint=0.2)
  # optimizer

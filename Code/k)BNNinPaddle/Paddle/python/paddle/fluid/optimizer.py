...
__all__ = [
    ...
    'OSGD', 'OSGDOptimizer', 'DAMSGrad', 'DAMSGradOptimizer'
]
...
class Optimizer(object):
    ...
    def _create_optimization_pass(self, parameters_and_grads, moment2):
        ...
        optimize_ops = []
        if framework.in_dygraph_mode():
            for param_and_grad, moment2_per_variable in zip(parameters_and_grads, moment2):
    ...
    def apply_optimize(self, loss, startup_program, params_grads, moment2):
        """
        Second part of `minimize`, appending optimization operators for
        given `params_grads` pairs.
        ...
        if framework.in_dygraph_mode():
            with program_guard(framework.default_main_program(),
                ...
                params_grads = append_regularization_ops(params_grads,
                                                         self.regularization)
                optimize_ops = self._create_optimization_pass(params_grads,moment2)
        else:
            program = loss.block.program
            with program_guard(program, startup_program):
    ...
    def minimize(self,
        ...
        optimize_ops = self.apply_optimize(
            loss, startup_program=startup_program, params_grads=params_grads, moment2=moment2)
...
class OSGDOptimizer(Optimizer):
    ...
    def __init__(self, learning_rate, regularization=None, name=None):
        ...
        super(OSGDOptimizer, self).__init__(
            learning_rate=learning_rate,
            regularization=regularization,
            name=name)
        self.type = "osgd"
        self._aux_param_str = "aux_param"
        self._hist_w_str = "hist_w"
        self._aux_theta_str = "aux_theta"
        self._aux_v_str = "aux_v"
        self._aux_v_hat_str = "aux_v_hat"
        self._aux_indicator_str = "aux_indicator"

    def _create_accumulators(self, block, parameters):
        assert isinstance(block, framework.Block)
        for p in parameters:
            self._add_accumulator(self._aux_param_str, p)
            self._add_accumulator(
                name=self._hist_w_str,
                param=p,
                dtype='float32',
                fill_value=0,
                shape=(7,) + p.shape)
            self._add_accumulator(self._aux_theta_str, p)
            self._add_accumulator(self._aux_v_str, p)
            self._add_accumulator(self._aux_v_hat_str, p)
            self._add_accumulator(
                name=self._aux_indicator_str,
                param=p,
                dtype='float32',
                fill_value=0,
                shape=[1])
    ...
    def _append_optimize_op(self, block, param_and_grad):
        assert isinstance(block, framework.Block)
        aux_param = self._get_accumulator(self._aux_param_str,
                                           param_and_grad[0])
        hist_w = self._get_accumulator(self._hist_w_str,
                                           param_and_grad[0])
        aux_theta = self._get_accumulator(self._aux_theta_str,
                                           param_and_grad[0])
        aux_v = self._get_accumulator(self._aux_v_str,
                                           param_and_grad[0])
        aux_v_hat = self._get_accumulator(self._aux_v_hat_str,
                                           param_and_grad[0])
        aux_indicator = self._get_accumulator(self._aux_indicator_str,
                                           param_and_grad[0])
        ...
        sgd_op = block.append_op(
            ...
            inputs={
                ...
                "AuxV" : aux_v,
                "AuxVHat" : aux_v_hat,
                "AuxIndicator" : aux_indicator
            },
            outputs={"ParamOut": param_and_grad[0],
                     "AuxParamOut": aux_param,
                     "HistWOut" : hist_w,
                     "AuxThetaOut" : aux_theta,
                     "AuxVOut" : aux_v,
                     "AuxVHatOut" : aux_v_hat,
                     "AuxIndicatorOut" : aux_indicator
            },
            attrs={
                "beta1": 0.9,
                "beta2": 0.99
...
class DAMSGradOptimizer(Optimizer):
    """
    ...
    def __init__(self,
        ...
        assert beta1 is not None
        assert epsilon is not None
        super(DAMSGradOptimizer, self).__init__(
            learning_rate=learning_rate,
            regularization=regularization,
            name=name)
        self.type = "DAMSGrad"
...
#            self._add_accumulator(self._moment2_acc_str, p)
    ...
    def _append_optimize_op(self, block, param_and_grad, moment2_per_variable):
        assert isinstance(block, framework.Block)
        
        moment2 = moment2_per_variable
        moment1 = self._get_accumulator(self._moment1_acc_str,
                                        param_and_grad[0])
#        moment2 = self._get_accumulator(self._moment2_acc_str,
#                                        param_and_grad[0])

...
parser.add_argument("--batch_size", default=32, type=int, help="per-agent batch size")
parser.add_argument(
    "--lr",
    default=0.1,
    type=float,
...
parser.add_argument(
    "--num_epochs", default=90, type=int, help="number of epochs to train"
)
parser.add_argument(
    "--num_iterations_per_training_epoch",
    default=None,
    type=int,
    help="number of iterations to run in the "
...
parser.add_argument("--momentum", default=0.9, type=float, help="optimization momentum")
parser.add_argument(
...
parser.add_argument(
    "--tag", default="", type=str, help="tag used to prepend checkpoint file names"
)
parser.add_argument(
    "--print_freq",
    default=10,
...
parser.add_argument("--checkpoint_dir", type=str, help="directory for saving log-files")
parser.add_argument(
...
parser.add_argument(
    "--no_cuda_streams",
    action="store_true",
    help="do not use multiple cuda streams which are used to " "speed up gossiping",
)
# --------------------------------------------------------------------------- #
...
def maindams():
    ...
    if args.all_reduce:
        # if all reduce then boils down to distributed computing
        # all nodes communicate to the others
        model = torch.nn.parallel.DistributedDataParallel(model)
    else:
        # here neighobrs nodes communicate only
        # make it take optimizer as one more argument
    ...
    if args.resume:
        if os.path.isfile(cmanager.checkpoint_fpath):
            log.info("=> loading checkpoint '{}'".format(cmanager.checkpoint_fpath))
            ...
            log.info(
                "=> loaded checkpoint '{}' (epoch {}; itr {})".format(
                    cmanager.checkpoint_fpath, checkpoint["epoch"], checkpoint["itr"]
                )
            )
        else:
...
def train(
    ...
    for i, (batch, target) in enumerate(_train_loader, start=itr):
        ...
        if i % 100 == 0:
            update_learning_rate(optimizer, epoch, itr=i, itr_per_epoch=len(loader))
        ...
        if i % args.print_freq == 0:
            with open(args.out_fname, "+a") as f:
                print(
                    ...
                    "{top5.val:.3f},{top5.avg:.3f},-1".format(
                        ep=epoch,
                        itr=i,
                        bt=batch_meter,
                        dt=data_meter,
        ...
        if (
            args.num_iterations_per_training_epoch != -1
...
def update_learning_rate(optimizer, epoch, itr=None, itr_per_epoch=None, scale=1):
    """
...
def parse_args():
    ...
    args.cpu_comm = (
        True
        if (args.backend == "gloo" and not args.push_sum and not args.all_reduce)
        else False
    )
    print("stop2")

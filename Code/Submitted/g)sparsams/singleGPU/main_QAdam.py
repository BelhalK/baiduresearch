...
from models.LocalSGD import LocalSGD
...
if __name__ == '__main__':
    ...
    for i, ke in enumerate(glob_params.keys()):
        ...
        m[i] = torch.ones(d[i]) * 1e-8
        v[i] = torch.ones(d[i]) * 1e-8
        error[i] = torch.zeros(d[i])
        
    local_m = [m]*args.num_users
    local_v = [v]*args.num_users
    if args.error == 1:
    ...
    glob = LocalSGD(net_glob.parameters(), lr=args.lr, momentum=0, 
...
#                print('Starting for worker nb {:3d}'.format(idx))
                ...
                if args.method == 'QAdam-full':
                    ...
                    for i, ke in enumerate(glob_params.keys()):
                        local_m[idx][i] = args.momentum*local_m[idx][i]+(1-args.momentum)*gg[i]
                        local_v[idx][i] = args.beta2*local_v[idx][i]+(1-args.beta2)*gg[i] **2
                        compress_grad[i] = local_m[idx][i]/torch.sqrt(local_v[idx][i])
                ...
                if args.method == 'QAdam':
                    ...
                    for i, ke in enumerate(glob_params.keys()):
                        local_m[idx][i] = args.momentum*local_m[idx][i]+(1-args.momentum)*gg[i]
                        local_v[idx][i] = args.beta2*local_v[idx][i]+(1-args.beta2)*gg[i] **2
                        aa = local_m[idx][i]/torch.sqrt(local_v[idx][i])
                        if args.error == 0:
                            compress_grad[i] = sparse.compute_topk(aa,max(args.sparsity,1/aa.numel())).float()
                        else:
                            compress_grad[i] = sparse.compute_topk(aa+local_error[idx][i],max(args.sparsity,1/aa.numel())).float()
                            local_error[idx][i] = local_error[idx][i] + aa - compress_grad[i]
                
                if args.method == 'QAdam-sign':
                    compress_grad = collections.OrderedDict()
                    for i, ke in enumerate(glob_params.keys()):
                        local_m[idx][i] = args.momentum*local_m[idx][i]+(1-args.momentum)*gg[i]
                        local_v[idx][i] = args.beta2*local_v[idx][i]+(1-args.beta2)*gg[i] **2
                        aa = local_m[idx][i]/torch.sqrt(local_v[idx][i])
                        if args.error == 0:
                            compress_grad[i] = sparse.sign_grad(aa).float()
                        else:
                            compress_grad[i] = sparse.sign_grad(aa+local_error[idx][i]).float()
                            local_error[idx][i] = local_error[idx][i] + aa - compress_grad[i]
...
#        loss_train_avg.append(loss_avg)
        ...
        if args.dataset=='cifar' and epoch == 99:
            args.lr = args.lr/10

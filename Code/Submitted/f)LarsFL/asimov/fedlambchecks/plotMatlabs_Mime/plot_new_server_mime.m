...
for n=1:length(num_users_list)
    ...
    for r=1:length(local_ep_list)
        ...
        mime_file=dlmread(['./fedlambchecks/checkpoints_' dataset '/resnet18_optmime_LAMBFalse_lambda0.0_workers2_lr1e-05_epoch100.txt'],'',1,0);              
        mime=mime_file(:,dim);

        mimelamb_file=dlmread(['./fedlambchecks/checkpoints_' dataset '/resnet18_optmime_LAMBTrue_lambda0.0_workers2_lr3e-05_epoch100.txt'],'',1,0);              
...
%         lamb(30:end)=lamb(30:end).*(1:(0.02/(length(lamb(30:end))-1)):1.02)';
        ...
        text(10,lamb(3),['TinyImageNet' ],'fontsize',20,'fontweight','bold','color','k');
        text(10,lamb(2),['+ RESNET18' ],'fontsize',20,'fontweight','bold','color','k');

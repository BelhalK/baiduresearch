...
#                                            lambda0=self.args.lambda0)
        ...
        for batch_idx, (images, labels) in enumerate(self.ldr_train):
            ...
            if self.args.model=='lstm':
                # net.hidden = (torch.zeros(1, self.args.local_bs, net.hidden_dim),
                #         torch.zeros(1, self.args.local_bs, net.hidden_dim))
                data_len = torch.sum(images>0,1)
                log_probs = net(images, data_len)
            else:
                # print(images.shape)
                log_probs = net(images)
            loss = self.loss_func(log_probs, labels)

...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        X_train = np.expand_dims(np.reshape(X_train,(-1,28,28)),1)
        X_test = np.expand_dims(np.reshape(X_test,(-1,28,28)),1)
        ...
        if args.customarg==1:
            ...
            print('non-iid data!')
            dict_users = fmnist_noniid(label_train, args.num_users)
    else:
    ...
    if args.LAMB:
        logger = Logger('{}/log{}_opt{}_LAMB{}_lambda{}_lr{}_ep{}_clients{}_frac{}_iid{}_seed{}.txt'.format(checkpoint_dir, logname,  args.optim, args.LAMB, args.lambda0, args.lr, args.local_ep,args.num_users, args.frac,args.customarg,args.seed), title=title)
    else:
        logger = Logger('{}/log{}_opt{}_LAMB{}_lr{}_ep{}_clients{}_frac{}_iid{}_seed{}.txt'.format(checkpoint_dir, logname,  args.optim, args.LAMB, args.lr, args.local_ep,args.num_users, args.frac,args.customarg,args.seed), title=title)        
...
#    glob_params = net_glob.state_dict()
    ...
    glob_v = collections.OrderedDict()
    ...
    for iter in range(args.epochs):
        ...
        glob = LocalUpdateAMS(args=args, dataset=dataset_train, idxs=list(range(len(dataset_train))),num_round=iter)
        glob_grad = glob.compute_gradient(net=copy.deepcopy(net_glob).to(args.device))
        for i, ke in enumerate(glob_params):
            glob_v[i] = args.beta2 * glob_v[i]**2 + (1-args.beta2) * glob_grad[i]**2
            
        glob_v_hat = FedMax(glob_v_hat, glob_v)
        ...
        for idx in idxs_users:
            ...
            if args.optim == 'mime':
                local_v[idx] = v  # local_v is never used in this model! put it here because I do not want to change the optimizer
                local_m[idx] = m

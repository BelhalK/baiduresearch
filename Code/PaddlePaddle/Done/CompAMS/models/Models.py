...
class LeNet(nn.Layer):
    def __init__(self, args):
      ...
      self.dropout1 = nn.functional.dropout(0.5)
      self.fc2 = nn.Linear(500, args.num_classes) # output nodes are 10 because our dataset have 10 different categories
      
    def forward(self, x):
      x = nn.functional.relu(self.conv1(x)) #Apply relu to each output of conv layer.
      x = nn.functional.max_pool2d(x, 2, 2) # Max pooling layer with kernal of 2 and stride of 2
      x = nn.functional.relu(self.conv2(x))
      x = nn.functional.max_pool2d(x, 2, 2)
      x = nn.functional.relu(self.conv3(x))
      x = nn.functional.max_pool2d(x, 2, 2)
      x = x.reshape((-1, 4*4*64)) # flatten our images to 1D to input it to the fully connected layers
      x = nn.functional.relu(self.fc1(x))
...
class LSTMNet(nn.Layer):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        self.word_embeddings = nn.functional.Embedding(vocab_size, embedding_dim, padding_idx=0)
    ...
    def forward(self, sentence):
        ...
        lstm_out, (ht,_) = self.lstm(embeds)
        dense = self.fc1(ht[-1])

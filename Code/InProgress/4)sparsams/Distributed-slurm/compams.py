...
import time
import os
import sparsification_dist as sparse
import collections
import sys
import compressor
                ...
                if self.all_reduce:
                    ...
                    if self.nodes > 1:
                        if self.compression_buffer:
                            ...
                            # d_p_new = torch.sign(d_p_new)
                            if args.method == 'topk':
                                d_p_new_comp = sparse.compute_topk(d_p_new + local_error,max(args.sparsity,1/d_p_new.numel())).float()
                                local_error.to(p.device).add_(d_p_new.to(p.device)-d_p_new_comp.to(p.device))
                                d_p_new = d_p_new_comp.to(p.device)
                            
                            if args.method == 'sign':
                                d_p_new_comp = sparse.sign_grad(d_p_new + local_error).float()
                                local_error.to(p.device).add_(d_p_new.to(p.device)-d_p_new_comp.to(p.device))
                                d_p_new = d_p_new_comp.to(p.device)

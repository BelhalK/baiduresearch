...
if mcmcmethod == "anilangevin":
    EXP_DIR = f'./out_toy/rings_convergent_{mcmcmethod}_th{anith}_eps{eps}/'
else: 
    EXP_DIR = f'./out_toy/rings_convergent_{mcmcmethod}_eps{eps}/'


title = 'rings-{}'.format(mcmcmethod)
if mcmcmethod=='anilangevin':
    logger = Logger('./out_toy/log_{}_th{}_ep{}.txt'.format(mcmcmethod,anith,eps), title=title)
else:
    logger = Logger('./out_toy/log_{}_ep{}.txt'.format(mcmcmethod,eps), title=title)
...
if config['optimizer_type'] == 'sgd' and eps > 0:
    # scale learning rate according to langevin noise for invariant tuning
    config['lr_init'] *= (eps ** 2) / 2
    config['lr_min'] *= (eps ** 2) / 2
...
def sample_s_t(batch_size, L=config['num_mcmc_steps'], init_type=config['init_type'], update_s_t_0=True):
    ...
    for ell in range(L):
        if mcmcmethod == "langevin":
            ...
            thtensor = t.Tensor(np.repeat(th, len(normofgrad))).reshape(normofgrad.shape) #threshold Tensor
...
for i in range(config['num_train_iters']):
    ...
    if (i + 1) % config['log_viz_freq'] == 0:
        ...
        q.plot_toy_density(True, f, eps, x_kde, EXP_DIR+'landscape/'+'toy_viz_{:>06d}.pdf'.format(i+1))

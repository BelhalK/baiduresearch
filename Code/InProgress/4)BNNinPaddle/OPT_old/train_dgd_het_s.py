...
#                    moment2.append(avg_dic_moment2_max[param_and_grad[0].name[6:]])
                    ones_variable = to_variable(np.ones(alg_u_param.numpy().shape).astype(np.float32))
                    ones_variable.stop_gradient = True
                    moment2.append(ones_variable)
#                print(len(moment2))
...
########
        ...
        if args.local == 1:
            scipy.io.savemat('local_gd_het/sgd_s'+str(args.lr)+'.mat', {'avg_loss_sgd'+str(args.lr).replace('.', ''):np.array(avg_loss_sav),
                                      'test_loss_sgd'+str(args.lr).replace('.', ''): np.array(test_loss_sav),
                                      'test_acc_sgd'+str(args.lr).replace('.', ''): np.array(test_acc_sav)})
        else:
            scipy.io.savemat('dgd_het/sgd_s'+str(args.lr)+'.mat', {'avg_loss_sgd'+str(args.lr).replace('.', ''):np.array(avg_loss_sav),
                                      'test_loss_sgd'+str(args.lr).replace('.', ''): np.array(test_loss_sav),
                                      'test_acc_sgd'+str(args.lr).replace('.', ''): np.array(test_acc_sav)})

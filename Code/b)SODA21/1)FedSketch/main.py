...
if __name__ == '__main__':
    ...
    if args.optim=='fedsketch':
        logger = Logger('{}/log{}_opt{}_sketch{}_lr{}_ep{}_clients{}_frac{}.topk{}_t{}_k{}_iid{}.txt'.format(checkpoint_dir, logname,  args.optim, args.sketching, args.glob_lr, args.local_ep,args.num_users, args.frac, args.topk, args.arraysize, args.binsize,args.customarg), title=title)
    else:
    ...
    ####create hash and signs functions for each user per layer
    # hashes = []
    # signs = []
    # for _ in range(args.num_users):
    #     indiv_hashes = collections.OrderedDict()
    #     indiv_signs = collections.OrderedDict()
    #     for i, ke in enumerate(glob_params.keys()):
    #         d[ke] = glob_params[ke].shape
    #         dim = glob_params[ke].numel() #total dimension per layer
    #         indiv_hashes[ke] = np.random.randint(k,size=(t,dim))
    #         indiv_signs[ke] = np.sign(np.random.rand(t,dim)-0.5)
    #     hashes.append(indiv_hashes)
    #     signs.append(indiv_signs)
    ...
    for iter in range(args.epochs):
        ...
        if args.optim == 'fedsketch':
            ...
            for i, weights in enumerate(w_locals): #sketching per local weight
                ...
                if args.optim == 'fedsketch':
                    local_true_grad = collections.OrderedDict()
                    local_error = errors[idxs_users[i]]
                ...
                for ke in weights.keys(): #sketching per layer (for scaling)
                    ...
                    if args.optim == 'fedsketch' and args.sketching == 'nips19':
                        vec =  init_weights - curr_weights + local_error[ke]
                        local_true_grad[ke] = vec
                    else:
                ...
                if args.optim == 'fedsketch':
                    true_grads.append(local_true_grad)
            ...
            if args.sketching == 'privix':
            ...
            elif args.sketching == 'nips19':
                glob_sketch = copy.deepcopy(sketch_avg)
                frac_topk = args.topk
                true_com_vec = FedAvg(true_grads)
                for ke in glob_sketch.keys():
                    dim = glob_params[ke].numel() 
                    glob_sketch[ke]=sko.HEAVYMIX(sketch_avg[ke], frac_topk, dim,hashfct[ke], sign[ke],1,true_com_vec[ke])
                for i, weights in enumerate(w_locals):
                    new_error = collections.OrderedDict()
                    gg = true_grads[i]
                    for ke in weights.keys():
                        new_error[ke] = gg[ke] - glob_sketch[ke]
                    errors[idxs_users[i]] = new_error
                ...
                for ke in tmp_glob_sketch.keys():
                    ...
                    tmp_glob_sketch[ke]=sko.HEAVYMIX(sketch_avg[ke], frac_topk, dim,hashfct[ke], sign[ke],1,true_com_vec[ke])
                ...
                heavymix_avg = sketched_heav_locals

...
\begin{center}
 \includegraphics[width=1.0\linewidth]{fig/node_aggre_sum.png}
 %\includegraphics[width=0.43\linewidth]{fig/node_aggre_cat.png}
\end{center}
   \caption{Aggregation with average.}
...
\begin{algorithmic}[1]
   ...
   \STATE $\mathbf{h}^{(i)} = \frac{1}{|ch(i)|} \sum_{j \in ch(i) } \mathbf{f}_{(j,i)}(\mathbf{h}^{(j)})$; \label{line:forward} \textcolor{blue}{// forward message passing}
    \ENDFOR
    \STATE $\widehat{\mathbf{h}}^{(i)} = \mathbf{h}^{(i)} \ \  \text{if} \ i \in$ layer L or $i \in \mathcal{R}_{\mathcal{G}}$;
   \FOR{$i \in \mathcal{V}$}
   ...
   \ENDFOR
    ...
    \STATE Updating flow-graph $\mathcal{G}$ with gradient ascending: $\theta^{(s+1)}_{\mathbf{f}} = \theta^{(s)}_{\mathbf{f}} + \nabla_{\theta_{\mathbf{f}}}\frac{1}{b} \sum_{i=1}^b  \mathcal{L}(\mathbf{x}_b; \theta^{(s)}_{\mathbf{f}})   \, .$
   %\ENDFOR 
   \ENDFOR
\end{algorithmic}
\end{algorithm}

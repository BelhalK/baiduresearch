...
%\IF{$\textrm{mod}(k,c) = 0$}
%	\STATE $n_{\textrm{m}} \leftarrow k/c$ (Number of models to average)
%		\STATE $\mu_{\ell}^{HWA} \leftarrow \frac{n_{\textrm{m}}\mu_{\ell}^{HWA} + \mu_{\ell}^{k+1}}{n_{\textrm{m}}+1}$
%		\STATE $\sigma^{HWA} \leftarrow \frac{n_{\textrm{m}}\mu_{\ell}^{HWA} + \mu_{\ell}^{k+1}}{n_{\textrm{m}}+1}$
%
%\ENDIF
%\ENDFOR
%\end{algorithmic}
...
\textcolor{red}{MNIST and CIFAR-10 experiments}.

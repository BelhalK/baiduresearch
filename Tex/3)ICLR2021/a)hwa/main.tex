...
We train a Bayesian variant of LeNet-5 convolutional neural network on the MNIST dataset. 
The training set is composed of $n=55\,000$ handwritten digits, $28 \times 28$ images. Each image is labelled with its corresponding number (from zero to nine).
...
We train here the Bayesian variant of the VGG neural network introduced in~\citep{simonyan2014very} on the CIFAR-10 dataset.
...
\subsection{Training Bayesian LeNet-5 for MNIST~\citep{lecun1998gradient} classification}

\begin{figure}[H]
\mbox{
\includegraphics[width=\textwidth]{figs/trainhwamnist_blenet.png}
}
\mbox{
\includegraphics[width=\textwidth]{figs/testhwamnist_blenet.png}
}
	\caption{Comparison for Bayesian LeNet CNN architecture on MNIST dataset.}
    \label{fig:mnist}
\end{figure}
...
\subsection{Training Bayesian VGG for CIFAR10~\citep{krizhevsky2009learning} classification}


\begin{figure}[H]
\mbox{
\includegraphics[width=\textwidth]{figs/trainhwacifar_bvgg.png}
}
\mbox{
\includegraphics[width=\textwidth]{figs/testhwacifar_bvgg.png}
}
	\caption{Comparison for Bayesian VGG CNN architecture on CIFAR-10 dataset.}
    \label{fig:cifar}
\end{figure}

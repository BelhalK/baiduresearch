...
\title{STANLey: \textbf{St}ochastic gradient \textbf{AN}isotropic \textbf{L}angevin dynamics for training Energy-Based Models}
...
With the growing efficacy  and potential of Energy-Based Modeling, also known as non-normalized probabilistic modeling, for modeling a generative process for different nature of high dimensional observations.
While the unknown normalizing constant of EBMs makes the training procedure intractable, resorting to Markov Chain Monte Carlo (MCMC) is in general a good option.
Realizing what MCMC entails for the end-to-end EBM training, we propose in this paper, a novel high dimensional sampling method, based on a anisotropic stepsize plugged into a vanilla discretized Langevin diffusion.
We motivate the necessity for a anisotropic update of the negative samples in the Markov Chain by the non linearity of the backbone, here a ConvNet, of the EBM.
Our resulting method, namely \algo, is an optimization algorithm for training Energy-Based models via our newly introduced MCMC.
We provide theoretical understanding of our sampling scheme by proving that our sampler leads to a geometrically uniformly ergodic Markov chain.
...
\textcolor{red}{ADD IN THE PROOF TABLE FOR ALL NOTATIONS USED}
...
\textcolor{red}{TO COMPLETE with TEXT on experiments and FID TABLES}

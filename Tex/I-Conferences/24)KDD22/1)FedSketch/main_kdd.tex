...
\newcommand\DrawBoxxhom[3][]{%
  \begin{tikzpicture}[remember picture,overlay]
    \draw[overlay,fill=gray!30,#1] 
    ([xshift=-4.8em,yshift=-1.1ex]{pic cs:#2}) 
    rectangle 
    ([xshift=200pt,yshift=-1.5ex]pic cs:#3);
  \end{tikzpicture}%
}
...
For instance, to reduce the communication cost,~\citep{ivkin2019communication} develops a distributed SGD algorithm using sketching, along with convergence analysis in the homogeneous setting, and establishes a communication complexity of order $\mathcal{O}(\log(d))$ per round, where $d$ is the dimension of the gradient vector compared to $\mathcal{O}(d)$ complexity per round of baseline mini-batch SGD. 
Nonetheless, the proposed sketching scheme in~\cite{ivkin2019communication}, built from a communication-efficiency perspective, is based on a deterministic procedure requiring access to the exact gradients information, hence not meeting the  privacy-preserving criteria.
...
Throughout the paper, we employ the commonly used \texttt{Count Sketch} \citep{DBLP:journals/tcs/CharikarCF04}, see Algorithm~\ref{alg:csketch}, as the building component of our algorithms. 
...
\noindent 1) \textit{Unbiased estimation}: As in~\cite{li2019privacy}, we have $ \mathbb{E}_{\mathbf{S}}\left[\texttt{PR}\left[\mathbf{S}\left(\boldsymbol{x}\right)\right]\right]=\boldsymbol{x}\, $.

\noindent 2) \textit{Bounded variance:} For the given $m<d$, $t=\mathcal{O}\left(\ln \left(\frac{d}{\delta}\right)\right)$ with probability $1-\delta$ we have:
    \begin{align}\notag
...
\STATE {Approximate $\mathbf{S}(\boldsymbol{x})$ using \texttt{HX} }
\STATE {Approximate} $\mathbf{S}\left(\boldsymbol{x} - \texttt{HX}[\mathbf{S}(\boldsymbol{x})]\right)$ with \texttt{PR} 
\STATE \textbf{Output:}
\vspace{- 0.1cm}
...
{\small $${\texttt{HP}}^{(r)}\triangleq  {\texttt{PR}}\left[{\mathbf{S}}^{(r-1)}\right]$$}
\tikzmark{i} 
\STATE \quad \textbf{if HEAPRIX variant:} \label{line:heaprix1}
{\small $${\texttt{HP}}^{(r)}\triangleq \texttt{HX}\left[{\mathbf{S}}^{(r-1)}\right]+\texttt{PR}\left[{\mathbf{S}}^{(r-1)}- \tilde{\mathbf{S}}^{(r-1)}\right]$$}
\STATE \quad Set $\boldsymbol{x}^{(r)}=\boldsymbol{x}^{(r-1)}-\gamma{\texttt{HP}}^{(r)}$ and $\boldsymbol{x}_j^{(0,r)}=\boldsymbol{x}^{(r)}$ 
\tikzmark{j}  
...
Furthermore, while in~\cite{haddadpour2020federated}, server stores a global model and aggregates the partial gradients from devices enabling it to extract some information regarding the device's local data, in contrast, in our algorithms server does not store the global model and only broadcasts the average sketches. 
% Thus, sketching-based communication FL algorithms such as ours do not reveal the exact values of the inputs.

\begin{remark}
As discussed in~\cite{horvath2020better}, induced compressors transform a biased compressor into an unbiased one at the cost of doubling the communication cost since the devices need to send $\mathcal{C}_1(\boldsymbol{x})$ and $\mathcal{C}_2\left(\boldsymbol{x}-\mathcal{C}_1\left(\boldsymbol{x}\right)\right)$ separately.
Here, we emphasize that with \texttt{HEAPRIX}, due to sketching techniques, the extra communication round cost is compensated with lower number of bits per round thanks to the lower dimension of the sketches.  
\end{remark}
...
{\small$$\mathbf{c}_j^{(r)}=\mathbf{c}_j^{(r-1)}-\frac{1}{\tau}\left[{\texttt{PR}}\left(\mathbf{S}^{(r-1)}\right)-{\texttt{PR}}\left(\mathbf{S}^{(r-1)}_{j}\right)\right]$$} {\small where ${\texttt{HP}}^{(r)}\triangleq \texttt{PR}(\mathbf{S}^{(r-1)})$}
\vspace{0.1cm}
\tikzmark{a}  
\STATE  \quad \textbf{if HEAPRIX variant:} 
{\small $$\mathbf{c}_j^{(r)}=\mathbf{c}_j^{(r-1)}-\frac{1}{\tau}\left(\texttt{HP}^{(r)}-\texttt{HP}^{(r)}_j\right)$$}
\tikzmark{b}
\STATE  \quad Set $\boldsymbol{x}^{(r)}=\boldsymbol{x}^{(r-1)}-\gamma\texttt{HP}^{(r)}$ and $\boldsymbol{x}_j^{(0,r)}=\boldsymbol{x}^{(r)}$ 
\STATE \quad \textbf{for} $\ell=0,\ldots,\tau-1$ \textbf{do}
...
\tikzmark{c}  
\STATE  \quad \textbf{if HEAPRIX variant:} 
\STATE  \qquad Device $j$ computes $\texttt{HP}^{(r)}_j\triangleq \texttt{HP}[\mathbf{S}_j^{(r)}]$.
\STATE  \qquad Second round of communication to obtain $\mathbf{\delta}_j^{(r)} :=  \mathbf{S}_j\left(\texttt{HX}[\mathbf{S}^{(r)}]\right)$.  \label{line:tildeS}
...
These results can be extended to the scenario where a subset of devices is sampled.
...
The main idea, that our algorithm is based upon, is the use of an approximation of the global gradient, $\mathbf{c}_j^{(r)}$ allows to correct the local gradient direction, see gradient tracking technique applied in~\cite{liang2019variance,haddadpour2020federated}. 
...
Most of the existing communication-efficient algorithms with compression only consider gradient-compression \emph{from devices to server}. 
...
Therefore, in Cross-Device FL setting, where there could be millions of devices, \texttt{HP} variant may not be practical, and we note that it could be more suitable for Cross-Silo FL settings. 
...
\textbf{Note:}
Such an improved communication cost over prior works can be explained by -- the joint exploitation of \emph{sketching} -- the reduction of the dimension of communicated messages -- the use of \emph{local updates} -- the reduction of the number of communication rounds reaching a desired convergence error.


\subsection{Convergence of  \texttt{FedSKETCHGATE}} 


We start by assuming that the local variance is bounded:
...
Suppose Assumptions~\ref{Assu:1} and~\ref{Assu:2} hold. 
Given $0<m\leq d$, and considering \texttt{FedSKETCHGATE}, Algorithm~\ref{Alg:PFLHet}, with sketch size $B=O\left(m\log\left(\frac{d R}{\delta}\right)\right)$ and $\gamma\geq p$ with probability $1-\delta$ we have  
...
%For the convergence result of convex objectives please see Section~\ref{thm:homog_case_append} in appendix.
...
MNIST~\cite{lecun1998gradient} is a popular benchmark dataset in federated learning. It contains handwritten digits from ten classes $0$ to $9$, with 60000 samples for training and 10000 samples for testing, where each data point is a $28\times 28$ image.
...
\begin{figure}[H]
	...
	\caption{Homogeneous case on MNIST: Comparison of compressed FL methods on LeNet CNN.}
...
\begin{figure}[H]
	...
	\caption{Heterogeneous case on MNIST: Comparison of compressed FL algorithms on LeNet CNN.}
...
%Section~\ref{app:proofs} contains the proofs of theorems and Section~\ref{app:numericals} presents additional~numerical~runs.
...
%        \\ \cmidrule(r){2-4}
        ...
        \\
        \midrule
        \makecell{\textbf{Basu et al.~\citep{basu2019qsparse} (with $\gamma=m/d$)}}  & \makecell[l]{$R=O\left(\frac{d}{m\epsilon^{1.5}}\right)$ \quad $\tau=O\left(\frac{m}{pd\sqrt{\epsilon}}\right)$\\ $B=O\left(d\right)$ \quad

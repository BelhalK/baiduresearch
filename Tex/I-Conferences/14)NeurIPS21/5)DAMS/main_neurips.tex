...
\begin{wrapfigure}[17]{r}{.5\linewidth}\vspace{-0.35in}
\begin{minipage}{\linewidth}
\begin{algorithm}[H]
...
Given that $[\hat v_{0,i}]_j \geq 0$ , we have $\mathbb E \left[ \sum_{t=1}^{T}   \|    (- \hat V_{t-2} + \hat V_{t-1} ) \|_{abs} \right] \leq  \sum_{i=1}^N \sum_{j=1}^d  \mathbb E[G_{\infty}^2]=  Nd G_{\infty}^2$.
%\begin{align} \notag
...
More details and experiments can be found in the supplementary material of our paper.
\vspace{-0.05in}
\begin{figure}[t]
\mbox{\hspace{-0.1in}
...
This difference is caused by the inconsistency among the parameters on different nodes when the stepsize is large. 

...
\begin{document}
\title{\vspace{-0.05in}
\texttt{FedSKETCH}: Communication-Efficient Federated Learning via Sketching\vspace{-0.05in}}
...
We also provide a convergence analysis for the \texttt{PRIVIX}/\texttt{DiffSketch}\footnote{{\color{blue}We use \texttt{PRIVIX} and \texttt{DiffSketch} \cite{li2019privacy} interchangeably throughout the paper.}} algorithm proposed in~\cite{li2019privacy}.
...
The final update reads {\color{blue}$\mathbf{S}[j][h_j(i)]=\mathbf{S}[j][h_{j}(i)]+\text{sign}_j(i).{x}_i$} for any $1 \leq j \leq t$ .
...
In this algorithm, to query the quantity $x_i$, the $i-th$ element of the vector $\boldsymbol{x}$, we compute the median of $t$ approximated values specified by the indices of $h_j(i)$ for $1\leq j\leq t$, see~\citep{li2019privacy}, {\color{blue}which is introduced under the name of \texttt{DiffSketch}}, or Algorithm~\ref{Alg:privix} in the Appendix (for more details).
%\begin{algorithm}[t]
...
\end{property}
{\color{blue} We note that bounded variance assumption does not necessary implies any compression as $d$ could be relatively large. Second, a version of this property is used in Section~B of Appendix~\cite{li2019privacy}}. Thus, with probability $1-\delta$ we obtain  $\texttt{PRIVIX}\in \mathbb{U}(1+c\frac{d}{m})$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{wrapfigure}[13]{r}{.51\linewidth}\vspace{-0.4cm}
...
%\end{lemma}
We note that Algorithm~\ref{alg:heavymix} is a variation of the sketching algorithm developed in~\cite{ivkin2019communication} with distinction that \texttt{HEAVYMIX} does not require a second round of communication to obtain the exact values of top$_m$.{\color{blue}This is mainly because in \texttt{SKETCGED-SGD}~\cite{ivkin2019communication} server has to obtain the exact values of \emph{the average of sketches}; however \texttt{HEAVYMIX} obtains exact value locally, thus does not require second round of communication.} 
...
{\color{blue}In the following we introduce \texttt{HEAPRIX} which is built upon \texttt{HEAVYMIX} and \texttt{PRIVIX} methods.} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{wrapfigure}[10]{r}{.57\linewidth}\vspace{-0.2in}
\begin{minipage}{\linewidth}
\begin{algorithm}[H]
...
\end{corollary}
\textit{Benefits of \texttt{HEAPRIX}:} Corollary~\ref{cor:small} states that, unlike \texttt{PRIVIX}, \texttt{HEAPRIX} compression noise can be made as small as possible using larger hash size. {\color{blue}In the distributed setting, 
contrary to \texttt{SKETCHED-SGD}~\cite{ivkin2019communication} where decompressing is happening at the server, \texttt{HEAPRIX} does not require having access to exact $\text{top}_m$ values of the input as it is based on \texttt{HEAVYMIX}, thus helps preserving privacy.} 
...
\begin{wrapfigure}[30]{r}{.55\linewidth}\vspace{-0.4in}
\begin{minipage}{\linewidth}
\begin{algorithm}[H]
...
\begin{wrapfigure}[32]{r}{.55\linewidth}\vspace{-1.8cm}
\begin{minipage}{\linewidth}
\begin{algorithm}[H]
...
Therefore, to mitigate the effect of data heterogeneity, we introduce a new algorithm called \texttt{FedSKETCHGATE} described in Algorithm~\ref{Alg:PFLHet}. This algorithm leverages the idea of gradient tracking applied in~\cite{haddadpour2020federated} (with compression) and a special case of $\gamma=1$ without compression~\citep{liang2019variance}. 
...
%Extensive comparison with related methods, involving sketches or not, such as \citep{rothchild2020fetchsgd} or \cite{karimireddy2019scaffold}, can be found Section~\ref{app:comparison} of the Appendix.
...
{\color{blue}We would like to also mention that there prior studies such as~ \cite{tang2019doublesqueeze} and \cite{zheng2019communication} that analyze the two-way compression, but since \cite{philippenko2020artemis} is the state-of-the-art on this topic we only compared our results with these papers.} 

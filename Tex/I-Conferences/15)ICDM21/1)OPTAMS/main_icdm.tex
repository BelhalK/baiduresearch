...
Suppose the learner incurs a sequence of convex loss functions $\{ \ell_{t}(\cdot) \}_{t >0}$.
...
\textbf{Comparison to \textsc{Optimistic-Adam}~\citep{DISZ18}.}This is an optimistic variant of ADAM, namely~\textsc{Optimistic-Adam}. 
...
The \textit{IMDB} movie review dataset, popular for text classification, is a binary dataset with $25\,000$ training and testing samples respectively. 
...
\subsection{Choice of parameter \lowercase{$r$}}\label{sec:choicer}
...
We provide in this section, proof sketches for our two main results regarding convex and nonconvex losses.
\subsection{Sketch Proof of Theorem~\ref{THM:MAINCONVEX}}\label{app:thmmainconvex}
...
\subsection{Sketch Proof of Theorem~\ref{THM:BOUNDOPT}}\label{app:BOUNDOPT}
\begin{Theorem*}
Assume H\ref{ASS:BOUNDEDPARAM}-H\ref{ass:bounded}, $\beta_1 < \beta_2 \in [0,1)$ and a sequence of decreasing stepsizes $\{\eta_t\}_{t>0}$, then the following result holds:
\beq\notag
\begin{split}
...
%
%  
%{\Large\bfseries Appendix: An Optimistic Acceleration of AMSGrad for Nonconvex Optimization \par}
%
%

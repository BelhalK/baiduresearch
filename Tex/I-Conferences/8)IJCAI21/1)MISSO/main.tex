...
%\author{
%Belhal Karimi$^1$\footnote{Contact Author}\and
%Hoi-To Wai$^2$\and
%Eric Moulines$^{3}$\And
%Ping Li$^1$\\
%\affiliations
...
Another application is variational inference~\cite{ghahramani2015probabilistic}, in which  the goal is to approximate the posterior distribution of parameters given the observations;  see~\cite{neal2012bayesian,blundell2015weight,rezende2014stochastic}.
...
\eeq
\end{assumption}
%We remark that H\ref{ass:sur} is a common condition used for surrogate functions, see~\cite[Section 2.3]{mairal2015miso}.
...
%\beq\label{pairvi}
%\begin{split}
%&\rsur{i}{\param}{\op}{z} \eqdef \\
%& \pscal{ \grd_{\param} d(\op) - \jacob{\param}{t}{ z,\op}\nabla_w \log p(y_i | x_i, w)\big|_{w = t( z, \op )} } { \param - \op}\\
%&  +\frac{L}{2}\| \param - \op\|^2 \eqsp.
%\end{split}
%\eeq
...
\[\Scale[0.9]{\Delta_{( K_{\sf max} )} \eqdef 2 n L \EE[  \sumSur{0}{\hp{0}} - \sumSur{K_{\sf max}}{\hp{K_{\sf max}}} ] +  4 L C_{\sf r} \overline{M}_{(K_{\sf max})} \eqsp.}\]
...
%    \subfloat[LeNet-5 on MNIST]{{\includegraphics[width=1.35in]{pic_paper/small_lenet.png} }}%
    \subfloat[LeNet-5: MNIST]{{\includegraphics[width=1.6in]{pic_paper/lenet_vs_wallclock.png} }}%    
    \subfloat[ResNet-18: CIFAR-10]{{\includegraphics[width=1.6in]{pic_paper/resnet_vs_wallclock.png} }}%
...
\textit{MISSO: Minimization by Incremental Stochastic Surrogate Optimization for Large Scale Nonconvex and Nonsmooth Problems}}%
...
\subsection{Plots against epochs elapsed}
...
\begin{figure}[H]
    \centering
    \subfloat[ResNet-18 on CIFAR10]{{\includegraphics[width=3.31in]{pic_paper/resnet_vs_wallclock.png} }}%
%    \subfloat[ResNet-18 on CIFAR-10]{{\includegraphics[width=2.71in]{pic_paper/small_resnet.png} }}%
...
\begin{figure}[H]
    \centering
%    \subfloat[LeNet-5 on MNIST]{{\includegraphics[width=1.35in]{pic_paper/small_lenet.png} }}%
    \subfloat[LeNet-5: MNIST]{{\includegraphics[width=3.5in]{pic_paper/small_lenet.png} }}%    
    \subfloat[ResNet-18: CIFAR-10]{{\includegraphics[width=3.5in]{pic_paper/small_resnet.png} }}%

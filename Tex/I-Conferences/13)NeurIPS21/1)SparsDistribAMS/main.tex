...
For $x\in\mathbb R^d$, denote $\mathcal S$ as the size-$k$ set of $i\in[d]$ with largest $k$ magnitude $|x_i|$. The \textbf{Top-$k$} compressor is defined as $\mathcal C(x)=x_i$, if $i\in\mathcal S$; $\mathcal C(x)=0$ otherwise.
\end{definition}
...
\begin{equation}
\begin{split}
 \frac{1}{\maxiter}\sum_{t=0}^{\maxiter -1} \EE[\norm{\nabla f(\theta_t)}^2] \leq \frac{\EE[ f(\theta_{0}) - f(\theta_{\maxiter})]}{ \Delta_1 \eta_t \maxiter} + 
...
\end{align*}
where (a) uses the fact that $\tilde g_t+e_{t+1}=g_t+e_t$, $e_1=0$ at initialization. By smoothness assumption Assumption~\ref{ass:smooth} we have
\begin{align*}
...
%\end{align*}
%where (a) uses Assumption~\ref{ass:quant}.

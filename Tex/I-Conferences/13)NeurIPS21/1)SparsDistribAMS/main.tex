...
\begin{document}
\title{Distributed Adaptive Optimization with Gradient Compression}
...
We develop a simple optimization leveraging the adaptivity of AMSGrad, and the computational virtue of \textbf{Top-$k$} sparsification, for tackling a large finite-sum of nonconvex objective functions.
...
\item Under mild assumption, such as nonconvexity and smoothness, we provide a non-asymptotic convergence rate of \algo\ in the general case, \ie when the number of workers is equal to $n$ and with unspecified values for the hyperparameters. 
...
Then, we develop in Section~\ref{sec:main}, our method, namely \algo, based on the \textbf{Top-$k$} compression method using AMSGrad as a prototype optimization algorithm for our scheme.
Theoretical understanding of our method's behaviour with respect to convergence towards a stationary point is developed in Section~\ref{sec:theory} under both the decentralized setting, \ie multiple workers which communicate with a central server, and the single machine setting.
We present numerical illustrations showing the advantages of our method in Section~\ref{sec:experiment}.
...
Thus, it intrinsically assigns different learning rates to different coordinates throughout the training -- elements with smaller previous gradient magnitude tend to move at larger rate via a larger steps. 
AdaGrad has been shown to perform well especially under some sparsity structure \textcolor{red}{\textbf{BK}: sparsity in the model or the data or both?}. 

Other adaptive methods include AdaDelta~\cite{Proc:adadelta} and Adam~\cite{kingma2014adam} which introduce momentum and moving average of second moment estimation into AdaGrad hence leading to better performances. 
...
\begin{algorithmic}[1]
\small
\STATE \textbf{Input}: parameter $\beta_1$, $\beta_2$, and $\eta_t$. 
...
\end{equation}
where $n$ denotes the number of workers, $f_i$ represents the average loss (over the local data samples) for worker $i \in \inter$ and $\theta$ the global model parameter taking value in $\Theta$, a subset of $\mathbb{R}^d$.
...
\begin{Remark}
It is well-known~\cite{stich2018sparsified,Proc:Zheng_NIPS19} that for \textbf{Top-$k$}, $q^2=1-\frac{k}{d}$; for \textbf{Block-Sign}, by Cauchy-Schwartz inequality we have $q^2=1-\min_{i\in [M]} \frac{1}{d_i}$.  \textcolor{red}{\textbf{BK:} define $[M]$ and $d_i$}
\end{Remark}

The intuition of \textbf{Top-$k$} is that, it has been observed in many deep neural networks that during training, most gradients are typically very small and can be regarded as redundant---gradients with large magnitude contain most information. The \textbf{Block-Sign} compressor is a simple extension of the 1-bit \textbf{SIGN} compressor, adapted to different gradient magnitude in different blocks, which, for neural nets, are usually set as the distinct network layers. The scaling factor in Definition~\ref{def:sign} is to preserve the (possibly very different) gradient magnitude in each layer. In principle, \textbf{Top-$k$} would perform the best when the gradient is sparse, or only has a few very large absolute values, while \textbf{Block-Sign} compressor would work well when most gradients have similar magnitude within each layer.




\subsection{\algo\ with Error Feedback for Distributed Optimization}


We present in Algorithm~\ref{alg:sparsams} our method based on a AMSGrad type of update in the central server and a compression coupled with an error computation on each worker.

The key difference of our \textbf{Top-$k$} based AMSGrad distributed optimization method compared with \citep{chen2020quantized}, developing a quantized variant of Adam~\cite{kingma2014adam} is that, in our method, only compressed gradients are transmitted from the workers to the central server. 
...
Thus, that method is very much like the sparsified distributed SGD, with the exception that the ratio $\frac{m}{v}$ plays the role of the gradient vector $g$ communication-wise. 
In our optimization method in Algorithm~\ref{alg:sparsams}, the moment estimates $m$ and $v$ are computed only at the central server, with the compressed version of the workers gradients instead of the full gradient. 
This constitutes be the key of our algorithm in convergence analysis and in the practical benefits in the numerical runs.
...
\STATE\quad  Compute $\tilde g_{t,i}=\textbf{Top-$k$}(g_{t,i}+e_{t,i},k)$ \label{line:topk} 
...
\section{Non-Asymptotic Convergence Analysis of \algo\ }\label{sec:theory}
...
\subsection{General case convergence rate}
...
In this paper, we are particularly interested in the case when the number of decentralized machines is large but we also provide theoretical and experimental insights on the single-machine case ($n=1$).

We begin by considering the general case for Algorithm~\ref{alg:sparsams} when the number of worker can be large and the hyperparameters are unspecified.
Under the mild assumption stated above, we derive the following convergence bound in the decentralized setting:


\begin{Theorem}  \label{theo:single rate}
Denote $C_0=\sqrt{\frac{4(1+q^2)^3}{(1-q^2)^2}G^2+\epsilon}$, $C_1=\frac{\beta_1}{1-\beta_1}+\frac{2q}{1-q^2}$. Under Assumption~\ref{ass:quant} to Assumption~\ref{ass:var}, with $\eta_t=\eta\leq \frac{\epsilon}{4LC_0}$, then for $T >0$, \algo\ satisfies
\begin{align*}
...
We remark from this bound in Theorem~\ref{theo:single rate}, that the more quantization we apply to our gradient vectors ($q \uparrow$), the larger the upper bound of the stationary condition is, \ie the slower the algorithm is. 
...
We first provide the formulation of our method in the single machine setting in Algorithm~\ref{alg:sparsamssingle}.
...
Under Assumption~\ref{ass:smooth} to Assumption~\ref{ass:var},setting the stepsize as $\eta_t = L\sqrt{\frac{n}{\maxiter}}$, the sequence of iterates $\{\theta_t\}_{t>0}$ output from Algorithm~\ref{alg:sparsamssingle} satisfies:

\textcolor{red}{complete with single machine corollary}
\end{Corollary}
...
Our proposed \textbf{Top-$k$}-EF with AMSGrad matches that of full AMSGrad, in distributed learning. Number of local workers is 20. Error feedback fixes the convergence issue of using solely the \textbf{Top-$k$} gradient. 
...
\section{Proof of Theorem~\ref{theo:single rate}}
...
\begin{align*}
    \sum_{t=1}^T \|D_t\|_1 &=\sum_{t=1}^T \sum_{i=1}^d (\frac{1}{\sqrt{\hat v_{t-1}+\epsilon}}-\frac{1}{\sqrt{\hat v_t+\epsilon}})\\
    &=\sum_{i=1}^d (\frac{1}{\sqrt{\hat v_0+\epsilon}}-\frac{1}{\sqrt{\hat v_T+\epsilon}})\\
...
Thus,
\begin{align*}
    \sum_{t=1}^T \|D_t\|^2&=\sum_{t=1}^T \sum_{i=1}^d (\frac{1}{\sqrt{\hat v_{t-1}+\epsilon}}-\frac{1}{\sqrt{\hat v_t+\epsilon}})^2\\
    &\leq \sum_{t=1}^T \sum_{i=1}^d (\frac{1}{\hat v_{t-1}+\epsilon}-\frac{1}{\hat v_t+\epsilon})\\
    &\leq \frac{d}{\epsilon}.
\end{align*}
...
\begin{align}
    ...
    &\overset{(b)}{\leq} \frac{\eta \rho}{2\epsilon}\mathbb E[\|\nabla f(\theta_t)\|^2]+\frac{\eta^3 L}{2\rho}\mathbb E[\|\frac{\frac{\beta_1}{1-\beta_1}m_{t-1}'+\mathcal E_t}{\sqrt{\hat v_{t-1}+\epsilon}}\|^2]+\frac{\eta^2 C_1LG^2}{\sqrt\epsilon} \mathbb E[\|D_t\|_1]  \nonumber\\
...
Now integrating \eqref{eq:I}, \eqref{eq:II}, \eqref{eq:III} and \eqref{eq:IV} into \eqref{eq0}, we obtain
\begin{align*}
    &\mathbb E[f(x_{t+1})]-f(x_t)\\
...
\end{align*}
Denote $C_0\eqdef \sqrt{\frac{4(1+q^2)^3}{(1-q^2)^2}G^2+\epsilon}$. Setting $\eta\leq \frac{\epsilon}{4LC_0}$ and choosing $\rho=\frac{\epsilon}{2C_0}$, we obtain
\begin{align*}
    &\mathbb E[f(x_{t+1})]-f(x_t)\\

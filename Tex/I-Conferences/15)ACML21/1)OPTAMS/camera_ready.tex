...
\title[OPT-AMS for Nonconvex Optimization]{An Optimistic Acceleration of AMSGrad for Nonconvex Optimization}
 ...
 % in braces, i.e., e.g., \Name{John {Smith Jones}} similarly
 % if the name has a "von" part, e.g \Name{Jane {de Winter}}.
 % If the first letter in the forenames is a diacritic
 % enclose the diacritic in braces, i.e., e.g., \Name{{\'E}louise Smith}
...
\begin{abstract}
We propose a new variant of AMSGrad~\citep{reddi2019convergence}, which is a popular adaptive gradient based optimization algorithm widely used for training deep neural networks. 
...
Deep learning models have been successful in several applications, including robotics~\citep{levine2016end}, computer vision~\citep{he2016deep,goodfellow2014generative}, reinforcement learning~\citep{mnih2013playing}, natural language processing~\citep{GMH13}, etc. 
With the sheer size of modern datasets and the dimension of neural networks, speeding up training is of utmost importance.
To do so, several algorithms have been proposed in recent years, such as  AMSGrad~\citep{reddi2019convergence}, Adam~\citep{kingma2014adam}, RMSprop~\citep{tieleman2012rmsprop}, AdADELTA~\citep{zeiler2012adadelta}.
All the prevalent algorithms for training deep networks mentioned above combine two ideas: the idea of adaptivity from AdaGrad~\citep{duchi2011adaptive} and the idea of momentum from Nesterov's Method~\citep{nesterov2003introductory} or Heavy ball method~\citep{polyak1964some}.
Their decentralized and federated variants have also proven their efficacy, see e.g.,~\citet{chen2020toward,chen2021convergent,karimi2021fed}. 
...
On the other hand, Nesterov's Method or Heavy ball Method is an accelerated optimization algorithm whose updates not only depend on the current iterate and gradient but also on the past gradients (i.e., momentum). 
Popular algorithms such as AMSGrad~\citep{reddi2019convergence} and Adam~\citep{kingma2014adam} leverage these ideas to accelerate the training of nonconvex objective functions, for instance deep neural~networks~losses.

In this paper, we propose an algorithm that goes beyond the hybrid of the adaptivity and momentum approach. 
Our algorithm is inspired by Optimistic Online learning~\citep{CJ12,RS13b,SALS15,mertikopoulos2018optimistic}, which assumes that, in each round of online learning, a \emph{predictable process} of the gradient of the loss function is available. 
...
\cite{daskalakis2018training} develop Optimistic-Adam leveraging optimistic online mirror descent~\citep{RS13b}.
...
\cite{daskalakis2018training} build on these related works and show that Optimistic-Mirror-Descent can avoid the cycle behavior in a bilinear zero-sum game accelerating the convergence. 
...
\item We conduct numerical experiments and show that the proposed algorithm not only accelerates the training procedure, but also leads to better empirical generalization performance.
\end{itemize}
...
The idea of Optimistic Online learning~\citep{CJ12,RS13b,SALS15} is as follows.
...
The optimistic variant, namely Optimistic-FTRL and developed in~\citet{SALS15} reads
\begin{equation} \label{optFTRL}
\textstyle w_t  = \arg \min_{w \in \Theta} \{ \langle w , L_{t-1} + m_t \rangle + \frac{1}{\eta} \textsf{R}(w) \}\eqsp,
\end{equation}
where $\{m_{t}\}_{t>0}$ is a predictable process incorporating (arbitrary) knowledge about the sequence of gradients $\{ g_{t}:=\nabla \ell_t(w_t)\}_{t>0}$.
Under the convexity assumption, it has been shown in~\citet{SALS15} that the regret of Optimistic-FTRL is at most $\mathcal{O}(\sqrt{\sum_{t=1}^T \| g_t - m_t \|_*^2 } )$.
...
We have a similar argument when comparing \textsc{OPT-AMSGrad} and AMSGrad regret bounds in Section~\ref{sec:convex}.
...
\vspace{0.1in}
\noindent\textbf{Adaptive optimization methods.}\ 
...
Specifically, the learning rate of AdaGrad at time $T$ for dimension $j$ is proportional to the inverse of $\sqrt{ \Sigma_{t=1}^T g_t[j]^2 }$, where $g_t[j]$ is the $j$-th element of the gradient vector 

\begin{wrapfigure}[10]{r}{.5\linewidth}\vspace{-0.15in}
...
As we mainly focus on the convergence rate of our method, we would like to highlight that complementary efforts can also be made towards improving the generalization abilities of those algorithms, see~\citet{zhou2020towards} for instance.
...
\begin{wrapfigure}[21]{r}{.5\linewidth}\vspace{-0.4in}
\begin{minipage}{\linewidth}
 \algsetup{indent=1em}
\begin{algorithm}[H]
\begin{algorithmic}[1] 
...
The proposed \textsc{OPT-AMSGrad}  inherits three properties: \textsf{(i)} Adaptive learning rate of each dimension as AdaGrad~\citep{duchi2011adaptive} (lines 6-8-9). \textsf{(ii)} Exponential moving average of the past gradients as Nesterov's Method~\citep{nesterov2003introductory} and the Heavy-Ball~\citep{polyak1964some} (line 5). \textsf{(iii)} Optimistic update that exploits \emph{prior knowledge} of the next gradient vector as in optimistic online learning algorithms~\citep{CJ12,RS13b,SALS15} (line 9).
...
In this subsection only, we assume convexity of $\{\ell_t\}_{t>0}$ and that $\Theta$ has a bounded diameter $D_{\infty}$, which is a standard assumption for adaptive methods~\citep{kingma2014adam,reddi2019convergence}.
...
We can compare the bound of Corollary~\ref{COR:COROLLARY} with that of AMSGrad with $\eta_t = \eta/\sqrt{t}$:
...
\end{assumption}
We assume that the optimistic guess $m_t$ at iteration $t$ and the true gradient $g_t$ are correlated and make a classical assumption in nonconvex optimization on the magnitude of the gradient:
...
\end{Theorem}
Firstly, the bound for our OPT-AMSGrad method matches the complexity bound of $\mathcal{O}( \sqrt{d/T_{\sf M}} + 1/T_{\sf M} )$ of~\citet{ghadimi2013stochastic} for SGD considering the dependence of T only, and of~\citet{zhou2018convergence} for AMSGrad method.
...
In this setting, the stochastic objective \eqref{eq:minproblem} reads $f(w, \xi) = \mathcal{L}(\textsf{MLN}( w, \xi), y) +\frac{\lambda}{2}\norm{w}^2\eqsp,$
...
\vspace{0.1in}
\noindent\textbf{Proof by induction:} As in~\citet{defossez2020convergence}, we will prove the containment of the weights by induction.
...
\citet{zaheer2018adaptive,chen2018convergence,ward2019adagrad,zhou2018convergence,zou2018convergence,li2019convergence} provide some theoretical analysis of Adam-type algorithms when applying them to smooth nonconvex optimization problems. 
...
To get some adaptive data dependent bound written in terms of the gradient norms observed along the trajectory when applying  \textsc{OPT-AMSGrad} to nonconvex optimization, one can follow the approach of~\citet{agarwal2019efficient} or~\citet{chen2019universal}.
...
\vspace{0.1in}
\noindent \textbf{Comparison to Optimistic-Adam~\citep{daskalakis2018training}.}\ This is an optimistic variant of ADAM,
...
\vspace{-0.1in}
\end{minipage}\end{wrapfigure}
 namely~Optimistic-Adam. A slightly modified version is summarized in Algorithm~\ref{OPT-DISZ}. Here, their method Optimistic-Adam$+\hat{v}_t$ corresponds to Optimistic-Adam with the additional max operation $\hat{v}_t = \max ( \hat{v}_{t-1}, v_t)$ to guarantee that the weighted second moment is monotone increasing.
...
\citet{daskalakis2018training} focuses on training GANs as a two-player zero-sum game, and their work is inspired by these works and shows that Optimistic-Mirror-Descent can avoid the cycle behavior in a bilinear zero-sum game thus accelerating convergence. 
...
Based on the analysis in the previous section, we understand that the choice of the prediction $m_t$ plays an important role in the convergence of \textsc{OPT-AMSGrad}.
Some classical works in gradient prediction methods include Anderson acceleration~\citep{walker2011anderson} or Minimal Polynomial Extrapolation~\citep{cabay1976polynomial}. These methods aim at finding a fixed point $g^{*}$ and assume that $\{g_t \in \mathbb R^d\}_{t>0} $ has the following linear relation:
...
\begin{wrapfigure}[9]{r}{.54\linewidth}\vspace{-0.3in}
\begin{minipage}{\linewidth}
 \algsetup{indent=1em}
\begin{algorithm}[H]
\begin{algorithmic}[1] 
...
\STATE \textbf{Output:} $\Sigma_{i=0}^{r-1} c_i g_i$, approximation of fixed point $g^*$.
\end{algorithmic}
\end{algorithm}
\end{minipage}\end{wrapfigure}
For our numerical experiments, we run \textsc{OPT-AMSGrad} using Algorithm~\ref{alg:algex} to construct the sequence $\{m_t\}_{t>0}$ which is based on estimating the limit of a sequence using the last iterates~\citep{brezinski2013extrapolation}. Specifically, at iteration $t$, $m_t$ is obtained by \textsf{(a)} calling Algorithm~\ref{alg:algex} with a sequence of $r$ past gradients, $\{ g_{t-1},g_{t-2}, \dots, g_{t-r} \}$ as input yielding the vector $c = [c_0, \dots, c_{r-1}] $ and \textsf{(b)} setting $m_t:= \Sigma_{i=0}^{r-1} c_i g_{t-r+i}$.
...
\begin{center}
    \mbox{\hspace{-0.2in}
    \subfigure[]{\includegraphics[width=0.27\textwidth]{simulation/fig/fig1_left.eps}}\hspace{-0.1in}
    \subfigure[]{\includegraphics[width=0.27\textwidth]{simulation/fig/fig1_right.eps}}\hspace{-0.1in}
    \subfigure[]{\includegraphics[width=0.27\textwidth]{simulation/fig/fig2_left.eps}}\hspace{-0.1in}
    \subfigure[]{\includegraphics[width=0.27\textwidth]{simulation/fig/fig2_right.eps}}
    }
...
the update towards the optimal point.
     }
     \label{simu}
...
Now let us consider another problem: an online learning problem proposed in~\citet{reddi2019convergence}, who used  this example to show that Adam~\citep{kingma2014adam} fails to converge.
...
Sub-figure (c) shows that Opt-extra converges faster than  other methods while Opt-1 is not  better than GD.
...
Sub-figure (d) shows that $-m_t$, obtained by extrapolation, always points to $w^*=-1$, while the~one obtained by using the previous negative direction points to the opposite direction~in~two~thirds~of rounds.  The extrapolation method is less affected by the gradient oscillation~and always makes the prediction in the right direction, which suggests that the method captures the aggregate~effect.
...
As the number of past gradients $r$ is important in gradient prediction (Algorithm \ref{alg:algex}), we compare  the performance for different $r\in\{3,5,10\}$ on two datasets (which will be described shortly). 

\begin{wrapfigure}{r}{3.2in}\vspace{-0.25in}
\begin{center}
\mbox{\hspace{-0.05in}
\includegraphics[width=1.7in]{new_figure/new_mnist_img_figure/mnist_img_train_loss_r3510_2.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/cifar10_train_loss_r3510.eps}
}
\end{center}\vspace{-0.25in}
\caption{Training loss w.r.t. $r$.}\label{fig:compare}\vspace{-0.15in}
\end{wrapfigure}
From the results in Figure~\ref{fig:compare} we see that, taking into consideration both quality of gradient prediction and computational cost, $r=5$ is a good choice for most applications. 
...
Yet, in some sense, this characteristic, specific to deep neural networks, is itself a fundamental problem of gradient prediction methods. 
...
The other benchmark method is the Optimistic-Adam$+\hat{v}_t$~\citep{daskalakis2018training}, which is described Algorithm~\ref{OPT-DISZ}. 
...
\vspace{0.08in}
\noindent\textbf{Datasets.}\hspace{0.1in}Following~\cite{reddi2019convergence} and~\cite{kingma2014adam}, we compare different algorithms on \textit{MNIST}, \textit{CIFAR10},
...
For \textit{MNIST}, we use two noisy variants namely \textit{MNIST-back-rand} and \textit{MNIST-back-image}~\citep{Proc:Larochelle_ICML07, Proc:ABC_UAI10,arXiv:Li_2018several}. 
...
\vspace{0.08in}
\noindent\textbf{Network architectures.} We adopt a multi-layer fully connected neural network with hidden layers of $200$ connected to another layer with $100$ neurons (using \textrm{ReLU} activations and \textrm{Softmax} output). 
...
The latter network includes a word embedding layer with $5\,000$ input entries representing most frequent words embedded into $32$ dimension.
The output of the embedding layer is equal to $100$ LSTM units connected to $100$ fully connected \textrm{ReLU} layers. 
...
\centering
\mbox{%\hspace{-0.2in}
\includegraphics[width=1.95in]{simulation/fig2/M_rand_train_loss_no1.eps}%\hspace{-0.1in}
\includegraphics[width=1.95in]{simulation/fig2/M_image_train_loss_no1.eps}%\hspace{-0.1in}
\includegraphics[width=1.95in]{simulation/fig2/cifar_cnn_train_loss_no1.eps}
}

\vspace{-0.02in}

\mbox{%\hspace{-0.2in}
\includegraphics[width=1.95in]{simulation/fig2/imdb_lstm_train_loss_no1.eps}%\hspace{-0.1in}
\includegraphics[width=1.95in]{simulation/fig2/cifar10_resnet_train_loss.eps}%\hspace{-0.1in}
\includegraphics[width=1.95in]{simulation/fig2/cifar100_resnet_train_loss.eps}
}\vspace{-0.16in}
\caption{Training loss vs. Number of iterations for fully connected NN, CNN, LSTM and ResNet.}
\label{fig:train_loss}\vspace{-0.2in}
\end{figure}
...
We observe: \textsf{(a)} in the long term, \textsc{OPT-AMSGrad} algorithm may converge to a better point with smaller loss value, and \textsf{(b)} in these applications, our proposed \textsc{OPT-AMSGrad} also outperforms the competing methods in terms of test accuracy. 
\begin{figure}[t]
%\centering


\mbox{%\hspace{-0.1in}
\includegraphics[width=1.6in]{new_figure/new_mnist_img_figure/mnist_img_train_loss_disz_2.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/new_mnist_img_figure/mnist_img_train_acc_disz_2.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/mnist_img_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/mnist_img_test_acc_disz.eps}
}


\mbox{%\hspace{-0.1in}
\includegraphics[width=1.6in]{new_figure/cifar10_train_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar10_train_acc_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar10_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar10_test_acc_disz.eps}
}

\mbox{%\hspace{-0.1in}
\includegraphics[width=1.6in]{new_figure/cifar100_train_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar100_train_acc_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar100_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.6in]{new_figure/cifar100_test_acc_disz.eps}
}


\vspace{-0.15in}
\caption{\textit{MNIST-back-image} + CNN, \textit{CIFAR10} + Res-18 and \textit{CIFAR100} + Res-50 . We compare three methods in terms of training (cross-entropy) and testing loss and accuracy.} \label{fig:testandtrain}\vspace{-0.2in}
\end{figure}
...
We propose \textsc{OPT-AMSGrad}, which combines optimistic online learning and AMSGrad to improve sample efficiency and accelerate the training process, in particular for fitting deep neural networks on a finite batch of observations. 
...
Numerical experiments  using various deep learning models demonstrate the effectiveness of \textsc{OPT-AMSGrad}.
...
\noindent{\Large\bfseries Supplementary Material: An Optimistic Acceleration of AMSGrad for Nonconvex Optimization \par}
...
& \leq \frac{\eta_{t}^{2} (1- \beta_1)}{1 - \beta_2}  \mathbb{E}\left[\sum_{i=1}^{d}\sum_{r=1}^t \gamma^{t-r}\right]\\  
&= \frac{\eta_{t}^{2} d (1- \beta_1)}{1 - \beta_2}  \mathbb{E}\left[\sum_{r=1}^t  \gamma^{t-r}\right] \eqsp,
\end{split}
\eeq

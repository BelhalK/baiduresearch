...
Deep learning models have been successful in several applications, from robotics~\citep{levine2016end}, computer vision~\citep{he2016deep,goodfellow2014generative}, reinforcement learning~\citep{mnih2013playing}, natural language processing~\citep{GMH13}, etc. 
...
On the other hand, Nesterov's Method or Heavy ball Method is an accelerated optimization algorithm which updates not only depends on the current iterate and gradient but also depends on the past gradients (i.e., momentum). 
...
\begin{wrapfigure}[11]{r}{.5\linewidth}\vspace{-0.1in}
...
\begin{wrapfigure}[22]{r}{.5\linewidth}\vspace{-0.3in}
\begin{minipage}{\linewidth}
 \algsetup{indent=1em}
\begin{algorithm}[H]
\begin{algorithmic}[1] 
...
% \captionsetup{justification=centering}
    \vspace{-0.1in}
    \includegraphics[width=2.8in]{plots/plot.pdf}
        \vspace{-0.1in}
...
\noindent \textbf{Comparison to Optimistic-Adam~\citep{daskalakis2018training}.}This is an optimistic variant of ADAM, namely~Optimistic-Adam. 
...
\citep{daskalakis2018training} focuses on training GANs as a two-player zero-sum game. 
\citep{daskalakis2018training} is inspired by these works and shows that Optimistic-Mirror-Descent can avoid the cycle behavior in a bilinear zero-sum game thus accelerating convergence. 
...
Some classical works in gradient prediction methods include Anderson acceleration~\citep{walker2011anderson} or Minimal Polynomial Extrapolation~\citep{cabay1976polynomial}.
% and Reduced Rank Extrapolation~\citep{eddy1979extrapolating}.
...
\begin{center}
    \mbox{%\hspace{-0.2in}
    \subfigure[]{\includegraphics[width=0.25\textwidth]{simulation/fig/fig1_left.eps}}
    \subfigure[]{\includegraphics[width=0.25\textwidth]{simulation/fig/fig1_right.eps}}
    \subfigure[]{\includegraphics[width=0.25\textwidth]{simulation/fig/fig2_left.eps}}
    \subfigure[]{\includegraphics[width=0.25\textwidth]{simulation/fig/fig2_right.eps}}
    }
...
For \textit{MNIST}, we use two noisy variants namely \textit{MNIST-back-rand} and \textit{MNIST-back-image}. 
...
\vspace{-0.05in}
\caption{\textit{MNIST-back-image} + CNN, \textit{CIFAR10} + Res-18 and \textit{CIFAR100} + Res-50 . We compare three methods in terms of training (cross-entropy) and testing loss and accuracy.} \label{fig:testandtrain}\vspace{-0.15in}
\end{figure}

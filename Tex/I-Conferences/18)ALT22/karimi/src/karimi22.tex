...
\title[ Minimization by Incremental Stochastic Surrogate]{Minimization by Incremental Stochastic Surrogate Optimization for Large Scale Nonconvex Problems}
...
To tackle the optimization problem Eq.~\eqref{eq:opt}, a popular approach is to apply the Majorization-Minimization (MM) method which iteratively minimizes a majorizing surrogate function. A large number of existing procedures fall into this general framework, for instance gradient-based or proximal methods or  the Expectation-Maximization (EM) algorithm~\citep{mcLachlan2008em} and some variational Bayes inference techniques~\citep{jordan1999var}; see for example~\citet{razaviyayn2013unified} and~\citet{lange2016mm} and the references therein.
When the number of terms $n$ in Eq.~\eqref{eq:opt} is large, the vanilla MM method may  be intractable because it requires to construct a surrogate function for all the $n$ terms ${\cal L}_i$ at each iteration. Here, a remedy is to apply the Minimization by Incremental Surrogate Optimization (MISO) method proposed by~\citet{mairal2015miso}, where the surrogate functions are updated incrementally. The MISO method can be interpreted as a combination of MM and ideas which have emerged for variance reduction in stochastic gradient methods~\citep{schmidt2017minimizing}.
An extended analysis of MISO has been proposed in~\citet{qian2019miso}.

The success of the MISO method rests upon the efficient minimization of surrogates such as convex functions, see~\citet[Section 2.3]{mairal2015miso}. 
A notable application of MISO-like algorithms is described in~\citet{mensch2017stochastic} where the authors build upon the stochastic Majorization-Minimization framework of~\citet{mairal2015miso} to introduce a method for sparse matrix factorization.
...
The objective function in Eq.~\eqref{eq:opt} is composed of a finite sum of possibly nonsmooth and nonconvex functions.
A popular approach here is to apply the MM method, which tackles Eq.~\eqref{eq:opt} through alternating between two steps --- {\sf (i)} minimizing a  \emph{surrogate} function which upper bounds the original objective function; and {\sf (ii)} updating the surrogate function to tighten the upper bound.
...
\eeq
\end{assumption}
%We remark that H\ref{ass:sur} is a common condition used for surrogate functions, see~\citet[Section 2.3]{mairal2015miso}.




We remark that H\ref{ass:sur} is a common assumption used for surrogate functions, see~\citet[Section 2.3]{mairal2015miso}.
...
H\ref{ass:diff} can be implied by applying~\citet[Proposition 1]{razaviyayn2013unified}.

\begin{wrapfigure}[16]{r}{0.51\textwidth}
\begin{minipage}{0.5\textwidth}\vspace{-0.1in}
\begin{algorithm}[H]
...
The inequality Eq.~\eqref{eq:lowerbd} implies $\sur{i}{\param}{\op} \geq {\cal L}_i ( \param ) > - \infty$ for any $\param \in \Param$.
...
Under H\ref{ass:sur}, H\ref{ass:diff}, it was shown that the MISO method converges almost surely to a stationary point of Eq.~\eqref{eq:opt}~\citep[Prop. 3.1]{mairal2015miso}.
...
\end{equation}
Plugging Eq.~\eqref{eq:integralsurrogate} into the MISO method is not feasible since the update step in Step~\ref{miso:iter} involves a minimization of an expectation.
Several motivating examples of Eq.~\eqref{eq:opt} are given in Section~\ref{sec:framework}.

In this paper, we propose the \emph{Minimization by Incremental Stochastic Surrogate Optimization} (MISSO) method  which replaces the expectation in Eq.~\eqref{eq:integralsurrogate} by \emph{Monte Carlo} integration and then optimizes the objective function Eq.~\eqref{eq:opt} in an incremental manner.
...
We now discuss two applications (examples) of the MISSO method.
...
\end{equation}
Using Eq.~\eqref{eq:vi}, we decompose ${\cal L}(\param) = n^{-1} \sum_{i=1}^{n}{{\cal L}_i(\param)} + {\rm const}.$ where
...
\end{equation}
Directly optimizing the finite sum objective function in Eq.~\eqref{eq:VI} can be difficult.
First, with $n \gg 1$, evaluating the objective function ${\cal L}( \param )$ requires a full pass over the entire dataset.
Second, for some complex models, the expectations in Eq.~\eqref{eq:variationalobjective} can be intractable even if we assume a simple parametric model for $q(w; \param)$.
...
By~\citet[Proposition~1]{blundell2015weight}, the gradient of $-r_i(\cdot)$ in Eq.~\eqref{eq:variationalobjective} is:
...
\end{align}
Finally, using Eq.~\eqref{eq:quad_sur} and Eq.~\eqref{pairvi}, the surrogate function Eq.~\eqref{eq:ssur} is given by 
...
\noindent We are particularly interested in the \emph{constrained optimization} setting where $\Param$ is a bounded set.
We thus control the supremum norm of the MC approximation, in Eq.~\eqref{eq:ssur}, as:
...
To characterize such control, we will have recourse to the notion of metric entropy (or bracketing number) as developed in~\citet{van2000asymptotic,vershynin2018high,wainwright2019high}.
...
In~\citet[Theorem 8.2.3]{vershynin2018high}, the authors recall the uniform law of large numbers:\vspace{0.05in}
$$\EE \left[\underset{f \in \mathcal{F}}{ \sup } \left|\frac{1}{M} \sum_{i=1}^{M} f\left(z_{i,m}\right)-\EE[f(z_i)]\right| \right] \leq \frac{C L}{\sqrt{M}} $$
\vspace{0.05in}
for all $z_{i,m}, i \in \llbracket 1, M \rrbracket$, where $\mathcal{F}$ is a class of $L$-Lipschitz functions.
Moreover, in~\citet[Theorem 8.1.3 ]{vershynin2018high} and~\citet[Theorem 5.22]{wainwright2019high}, applying the Dudley inequality yields:
$$\EE [ \underset{f \in \mathcal{F}}{\sup} |X_{f}-X_{0} | ] \leq \frac{1}{\sqrt{M}} \int_{0}^{1} \sqrt{\log \mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right)} d \varepsilon\eqsp,$$
where $\mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right)$ is the bracketing number and $\epsilon$ denotes the level of approximation (the bracketing number goes to infinity when $\epsilon  \to 0$). Finally, in~\citet[p.271, Example]{van2000asymptotic}, $\mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right)$ is bounded from above for a class of parametric functions $ \mathcal{F}= {f_{\param}: \param \in \Param}$:
$$\mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right) \leq K\left(\frac{\operatorname{diam} \Theta}{\varepsilon}\right)^{d},  \quad \textrm{for all} \quad 0<\varepsilon<\operatorname{diam} \Theta\eqsp.
$$
...
\textbf{Stationarity measure.} As problem Eq.~\eqref{eq:opt} is a constrained optimization task, we consider the following stationarity measure:
...
Thus, the sequence $\{ \hp{k} \}_{k \geq 0}$ is said to satisfy an \emph{asymptotic stationary point condition}. This is equivalent to~\citet[Definition 2.4]{mairal2015miso}.
...
As expected, the MISSO method converges to a stationary point of Eq.~\eqref{eq:opt} asymptotically and at a sublinear rate $\EE[ g_-^{(K)} ] \leq {\cal O}( \sqrt{\Delta_{( K_{\sf max} )}/ K_{\sf max}} )$. In other terms, MISSO requires ${\cal O} (nL/\epsilon)$ iterations to reach an $\epsilon$-stationary point when the suboptimality condition, that characterizes stationarity, is $ \EE \big[\| g_-( \hp{K} ) \|^2 \big] $.
...
In fact, when the optimization problem Eq.~\eqref{eq:opt} is unconstrained, \ie $\Param = \rset^p$, then $ \EE \big[ g( \hp{K} ) \big] = \EE \big[ \grd {\cal L}(\hp{K}) \big]$.
...
showing that $\EE[W_{\infty}] = w_{\infty}$, proving (ii). Using Eq.~\eqref{chik} we have that $W_k \leq W_{k-1} - X_k$ which yields that $\sum_{j=1}^{\infty}{X_j} \leq W_{0} - W_{\infty} < \infty$ and $\sum_{j=1}^{\infty}{\EE[X_j]} \leq \EE[W_{0}] - w_{\infty} < \infty$.
\end{proof}
...
%\caption{Convergence of parameters ${\bm \delta}$ and ${\bm \beta}$ for the SAEM, the MCEM and the MISSO methods. The convergence is plotted against the wallclock time.}
%\vspace{-0.1in}
...
\begin{figure}[H]
    \begin{center}
    \mbox{
        \includegraphics[width=0.5\textwidth]{fig2/logisticdelta.eps}
        \includegraphics[width=0.5\textwidth]{fig2/logisticbeta.eps}
    }
    \end{center}\vspace{-0.2in}
\caption{Convergence of parameters $\delta_1$ and $\beta_1$, the first component of vectors ${\bm \delta}$ and ${\bm \beta}$, for the SAEM, the MCEM and the MISSO methods,  plotted against the wallclock time (in seconds).}
\label{fig:misso_trauma}
\end{figure}


We compare in Figure~\ref{fig:misso_trauma} the convergence behavior of the estimated parameters $\bm{\delta}$ and $\bm{\beta}$ using SAEM~\citep{delyon1999} (with stepsize $\gamma_k = 1/k^{\alpha}$ where $\alpha = 0.6$ after tuning), MCEM~\citep{wei1990mcem}  and the proposed MISSO method.
...
This application follows \textbf{Example 2} described in Section~\ref{sec:framework}. We use variational inference and the ELBO loss Eq.~\eqref{eq:variationalobjective} to fit Bayesian Neural Networks on different datasets.
At iteration $k$, minimizing the sum of stochastic surrogates defined as in Eq.~\eqref{eq:ssur} and Eq.~\eqref{pairvi} yields the following MISSO update --- {\sf step (i)} pick a function index $i_k$ uniformly on $\inter[n]$; {\sf step (ii)} sample a Monte Carlo batch $ \{ z_{m}^{(k)} \}_{m=1}^{\Bsize{k}}$ from ${\cal N}(0,\Id)$; and {\sf step (iii)}  update the parameters, with $\tilde{w} = t(\hp{k-1},z_m^{(k)})$, as
 \beq\notag
\begin{split}
...
Under the prior distribution $\prior$, see Eq.~\eqref{eq:vi}, the weights are assumed  independent and identically distributed according to ${\cal N}(0,1)$.
...
We train here the Bayesian variant of the ResNet-18 neural network introduced in~\citet{he2016deep} on CIFAR-10. 
...
For efficiency purpose and lower variance, the Flipout estimator~\citep{wen2018flipout} is used.
%
%
%\begin{figure}[H]
%    \begin{center}
...
\begin{figure}[t]
    \begin{center}
    \mbox{
        \includegraphics[width=0.5\textwidth]{fig2/mnist.eps}
        \includegraphics[width=0.5\textwidth]{fig2/cifar.eps}
    }
    \end{center}
    
    \vspace{-0.2in}
    
\caption{Negated ELBO versus time elapsed for fitting (Left) Bayesian LeNet-5 on MNIST and (Right) Bayesian ResNet-18 on CIFAR-10. The solid curve is obtained from averaging over 5 independent runs of the methods. The convergence is plotted against the wallclock time.}\label{fig:lenetopt}
\end{figure}
...
For all these methods, the loss function Eq.~\eqref{eq:variationalobjective} and its gradients were computed by Monte Carlo integration based on the re-parametrization described above.
...
\begin{center}
{\LARGE \textbf{Appendix for ''Minimization by Incremental Stochastic Surrogate Optimization for Large Scale Nonconvex Problems''}}
\end{center}
\vspace{0.5in}
...
To prove the first bound in Eq.~\eqref{eq:misso_rate}, using the optimality of $\hp{k+1}$, one has
...
\end{split}
\eeq
Taking the conditional expectations on both sides of Eq.~\eqref{eq:firsteq} and re-arranging terms give:
...
\eeq
Proceeding from Eq.~\eqref{eq:afterarrange}, we observe the following lower bound for the left hand side
\beq\notag
\begin{split}
...
\end{split}
\eeq
where (a) is due to $\eSur{k}{\hp{k}} = 0$ [cf.~H\ref{ass:sur}], (b) is due to Eq.~\eqref{eq:surbd} and we have defined the summation in the last equality as $- \delta^{(k)}( \hp{k} )$.
%We further obtain
%\beq
%\sumSur{k}{\hp{k}} - {\cal L} ( \hp{k} ) \geq - \delta^{(k)}( \hp{k} ) + \frac{1}{2L} \| \grd \eSur{k}{\hp{k}} \|^2
%\eeq
Substituting the above into Eq.~\eqref{eq:afterarrange} yields
\beq
...
Plugging this back into Eq.~\eqref{eq:prebdd} yields
\beq\notag
\begin{split}
& \EE \big[ \| \grd \eSur{K}{\hp{K}} \|^2 \big] = \frac{1}{K_{\sf max}} \sum_{k=0}^{K_{\sf max}-1} \EE [ \| \grd \eSur{k}{\hp{k}} \|^2 ] \\
...
\end{split}
\eeq
This concludes our proof for the first inequality in Eq.~\eqref{eq:misso_rate}.

To prove the second inequality of Eq.~\eqref{eq:misso_rate}, we define the shorthand notations $g^{(k)} \eqdef g( \hp{k} )$, $g_-^{(k)} \eqdef - \min\{0, g^{(k)} \}$, $g_+^{(k)} \eqdef \max\{0, g^{(k)} \}$.
We observe that
\beq\notag
\begin{split}
...
\end{split}
\eeq
where (a) is due to H\ref{controlapprox} and (b) is due to Eq.~\eqref{eq:mkcal}.
This implies
\beq\notag
...
\end{enumerate}
\end{Lemma}
We proceed from Eq.~\eqref{eq:firsteq} by re-arranging terms and observing that
\beq\notag
\begin{split}
...
Under H\ref{ass:sur}, the finite sum of surrogate functions $\Sur{k}{\param}$, defined in Eq.~\eqref{eq:sumsurrodet}, is lower bounded by a constant $c_k > - \infty$ for any $\param$. To this end, we observe that
\beq \label{eq:dvk}
...
\end{split}
\eeq
Note that from the definitions Eq.~\eqref{eq:dvk}, Eq.~\eqref{eq:dxk}, Eq.~\eqref{eq:dek}, we have
...
\eeq
Using Eq.~\eqref{eq:mkcal} and the assumption on the sequence $\{\Bsize{k}\}_{k \geq 0}$, we obtain that
\beq\notag
...
\eeq
and subsequently applying Eq.~\eqref{eq:surbd}, we have $\lim_{ k \rightarrow \infty } \| \eSur{k}{\hp{k}} \| = 0$ almost surely. Finally, it follows from Eq.~\eqref{eq:surbd} and Eq.~\eqref{eq:gmbd} that
\beq
...
Finally, we prove that ${\cal L}(\hp{k})$ converges almost surely. As a consequence of Lemma~\ref{lemmarsapp}, it is clear that $\{ V_k \}_{k \geq 0}$ converges almost surely and so is $\{ \Sur{k}{\hp{k}} \}_{k \geq 0}$, \ie we have $\lim_{k \rightarrow \infty} \Sur{k}{\hp{k}} = \underline{\cal L}$. Applying Eq.~\eqref{eq:esur} implies that
\beq\notag
...
\end{equation}
showing that $\EE[W_{\infty}] = w_{\infty}$ and concluding the proof of (ii). Moreover, using Eq.~\eqref{chikbis} we have that $W_k \leq W_{k-1} - X_k$ which yields:
\begin{equation}
\begin{split}
...
We recall the MISO deterministic surrogate defined in Eq.~\eqref{pairmcem}:
\beq\notag
...
\end{split}
\eeq
Minimizing the total surrogate Eq.~\eqref{eq:mixedsurrogate} boils down to performing a quasi-Newton step.
It is perhaps sensible to apply some diagonal loading which is perfectly compatible with the surrogate interpretation we just gave. The logistic parameters are estimated as follows:
\beq\notag
...
\eeq
where $\tilde{\mathcal{L}}^{(1)}_i(\delta,\theta^{(\tau_i^k)},\{z_{i,m}\}_{m=1}^{M_{(\tau_i^k)}}) $ is the MC approximation of the MISO surrogate defined in Eq.~\eqref{eq:surrogatelogit} and which leads to the following quasi-Newton step:
\beq\notag
...
Besides, $\tilde{\mathcal{L}}^{(1)}_i(\beta, \Omega,\op, \{z_m\}_{m=1}^M)$ and $\tilde{\mathcal{L}}^{(2)}_i(\beta, \Omega,\op, \{z_m\}_{m=1}^M)$ are defined as MC approximation of $\hat{\mathcal{L}}^{(1)}_i(\beta, \Omega,\op)$ and $\hat{\mathcal{L}}^{(2)}_i(\beta, \Omega,\op)$, for all $i \in \inter[n]$ as components of the surrogate function Eq.~\eqref{eq:surrogatedet}.
...
We plot Figure~\ref{fig:misso_trauma_epochs}, the updated parameters for the Logistic regression example against~the~epochs~elapsed.
...
\begin{figure}[H]
    \begin{center}
    \mbox{
        \includegraphics[width=0.5\textwidth]{fig2/logisticdeltaepoch.eps}
        \includegraphics[width=0.5\textwidth]{fig2/logisticbetaepoch.eps}
    }
    \end{center}\vspace{-0.25in}
\caption{Convergence of parameters $\delta_1$ and $\beta_1$, the first component of vectors ${\bm \delta}$ and ${\bm \beta}$, for the SAEM, the MCEM and the MISSO methods. The convergence is plotted against epochs elapsed.}\label{fig:misso_trauma_epochs}\vspace{-0.2in}
\end{figure}
...
We describe in Table~\ref{table:lenet} the architecture of LeNet-5  introduced in~\citet{lecun1998gradient} and trained on MNIST, and in Table~\ref{table:resnet} the architecture of Resnet-18 trained on CIFAR-10. 
\begin{table}[h]
\begin{center}
\resizebox{\textwidth}{!}{ 
\begin{tabular}{ l c c c c r}
...
\end{tabular}
    ...
    \caption{\vspace{-0.05in}LeNet-5 architecture}    \label{table:lenet}\vspace{-0.3in}
\end{center}
\end{table}
...
\end{tabular}
    ...
    \caption{\vspace{-0.1in}ResNet-18 architecture} \label{table:resnet}
\end{center}
\end{table}
...
For the considered VI problem, we recall the surrogate functions~Eq.~\eqref{eq:quad_sur} are quadratic  and indeed a simple projection step suffices to ensure boundedness of the iterates.
...
We provide for completeness, the plots for all methods against the number of epochs elapsed in Figure~\ref{fig:lenetoptepochs}.

\begin{figure}[H]
    \begin{center}
    \mbox{
        \includegraphics[width=0.5\textwidth]{fig2/mnistepoch.eps}
        \includegraphics[width=0.5\textwidth]{fig2/cifarepoch.eps}
    }
    \end{center}

\vspace{-0.2in}

 \caption{Negated ELBO versus epochs elapsed for fitting (Left) Bayesian LeNet-5 on MNIST and (Right) Bayesian ResNet-18 on CIFAR-10. The solid curve is obtained from averaging over 5 independent runs of the methods.}\label{fig:lenetoptepochs}
\end{figure}

 
 \subsection{Wallclock time}
We provide Table~\ref{tab:tablebnn}, the running time for each method, related to Figure~\ref{fig:lenetopt}, used to train a Bayesian variant of LeNet-5 on MNIST.
The incremental method as MISSO and MC-SAG displays a similar wall clock time, despite being a bit worse given (a) the initialization that requires to compute a vector of $n$ gradients kept in memory and updated through the iterations and (b) the average operation for each parameters update to compute the aggregated drift term, see Eq.~\eqref{eq:missoupdate}.
...
We provide Table~\ref{tab:tableresnet}, the running time for each method, related to Figure~\ref{fig:lenetopt}, used to train a Bayesian variant of ResNet-18 on CIFAR10.
...
\begin{tabular}{ l c c r}
  \hline
 & {MC-Adam} & {MC-Adagrad} & {MISSO} \\ 
  \hline
{ResNet-18 on CIFAR}  & {31879} & {31675} & {32092}  \\
  \hline
\end{tabular}
}
\caption{Resnet on CIFAR: running time in seconds for $200$ epochs.}  \label{tab:tableresnet}
\end{center}
\end{table}

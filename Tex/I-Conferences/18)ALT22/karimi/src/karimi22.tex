...
For instance, this is the case for inference in latent variable models via maximum likelihood~\citep{mcLachlan2008em,karimi2019convergence,karimi2019global}. Another application is variational inference~\citep{ghahramani2015probabilistic}, in which  the goal is to approximate the posterior distribution of parameters given the observations;  see for example~\citet{neal2012bayesian,blundell2015weight,polson2017deep,rezende2014stochastic, li2017dropout, khan2018fast,osawa2019practical,karimi2021hwa}.
...
By~\citet[Proposition~1]{blundell2015weight}, the gradient of $-r_i(\cdot)$ in \eqref{eq:variationalobjective} is:
...
On the other hand, the experiment conducted on CIFAR-10 (Figure~\ref{fig:lenetopt}(Right)) using a much larger network, \ie a Bayesian variant of ResNet-18 showcases the need of a well-tuned adaptive methods to reach lower training loss (and also faster). Our MISSO method is similar to the Monte Carlo variant of ADAM but slower than Adagrad optimizer. 
Recall that the purpose of this paper is to provide a common class of optimizers, such as VI, in order to study their convergence behaviors, and not to introduce a novel method outperforming the baselines methods.
The latter remark deals with the improvement of the generalization of those algorithms, see \citet{zhou2020towards} for instance.

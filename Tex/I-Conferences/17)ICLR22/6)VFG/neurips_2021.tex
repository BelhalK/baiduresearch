...
\end{align}
Hence minimizing $\textbf{\text{KL}}^{(i)}$ is equal to minimize $\{\|\mathbf{h}^{(i)}- \widehat{\mathbf{h}}^{(u)}\|_1 + \|\mathbf{h}^{(i)}- \widehat{\mathbf{h}}^{(v)}\|_1 \}$ which achieves the objective in~\eqref{eq:i_parent}. 

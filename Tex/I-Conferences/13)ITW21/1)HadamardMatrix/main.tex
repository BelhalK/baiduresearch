...
$$\mathbf{M}=\mathbf{a}\mathbf{a}^T+\mathbf{b}\mathbf{b}^T + 10^{-4}\mathbf{I}_4 \ ,$$ 
...
The matrix $\mathbf{a}\mathbf{a}^T+\mathbf{b}\mathbf{b}^T$  is of rank $2$. Obviously, $\mathbf{M}>0$. 
...
There exists an $n\times n$ matrix with non-negative entries noted $\mathbf{A}=(a_{ij})$.
For any $n\geq 4$, there exists $\alpha = 1.1$, $\delta>0$ such that for any $n\times n$ symmetric matrix $\mathbf{M}=(m_{ij})$ where for all $1\leq i, j \leq n$, $m_{ij}\geq 0$, the following holds:
...
(i)  The matrix $\mathbf{X}^T\mathbf{X}$ is positive definite for every matrix $X$ with entries $x_{ij}\in [a_{ij}, a_{ij}+\delta]$ and $1\leq i\leq n$ and $1\leq j \leq p.$

(ii) The Hadamard power $(\mathbf{X}^T\mathbf{X})^{(\alpha)}\ngeqslant 0$  for any $x_{ij}\in [a_{ij}, a_{ij}+\delta]$, $1\leq i\leq n$ and $1\leq j \leq p$.
\end{lemma}
\medskip
...
Our main results yield the existence of an interval of non integer values for the exponent of the Hadamard powers such that for any positive definite random matrix, up to a mild assumption on the support of its entries, has non non-negative Hadamard powers.

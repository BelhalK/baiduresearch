...
Several attempts have been proposed for the standalone task of posterior sampling through the use of Langevin diffusion, see the Unadjusted Langevin in \cite{brosse2017tamed}, the MALA algorithm in \cite{roberts,robertsmala,durmus2017fast} or leveraging Hamiltonian Dynamics as in \cite{girolami2011riemann}.
...
\beq\label{eq:anila}
z_{k}^{m} = z_{k-1}^m + \stepsize_t/2  \nabla f_{\theta_t}(z_{k-1}^m) + \sqrt{\stepsize_t} \mathsf{B}_k \eqsp,
\eeq
...
As in \cite{allassonniere2015convergent}, we state the assumptions required for our analysis.
...
%
%The complete proof is deferred in the appendix and is also developed in \cite{allassonniere2015convergent} in the context of Bayesian Mixed Effect  models trained with the EM algorithm.
%
...
\begin{center}
\mbox{\hspace{-0.1in}
        \includegraphics[width=1.8in]{figs/fid_flowersmore.eps} \hspace{-0.2in}
        \includegraphics[width=1.8in]{figs/fid_cifarmore.eps}
...
\quad \textbf{Datasets.}
We use CelebA dataset \cite{liu2015deep} to evaluate our learning algorithm. 
...
\textbf{Methods and Settings.}
Nonconvergent MCMC are also used in this experiments and the number of MCMC transitions is set to $K = 50$.
Global learning parameters of the gradient update is set to $0.01$ for all methods.
...
\begin{figure}[H]
  \centering
  \subfloat{\includegraphics[width=0.45\textwidth]{figs/celebStanley.jpg}} \\
  \subfloat{\includegraphics[width=0.45\textwidth]{figs/celebVanilla.jpg}}
	\caption{(Image inpainting). CelebA dataset. \textbf{Top row:} \algo\ \textbf{Bottom row:} Vanilla Langevin.}
		\label{fig:celebinpainting}
\end{figure}


\textbf{Results.} 
Figure~\ref{fig:fidceleb} displays the FID curves for all methods.
We note that along the iterations, our method \algo\ outperforms the other baseline and is similar to HMC, while only requiring first order information for the computation of the adaptive stepsize.
...
\end{center}
\caption{(CIFAR Dataset). 1: \algo\ 2: MH. 3: HMC 4: GD without noise after 100k iterations.}
	\label{fig:cifarapp}
\end{figure}

...
\usepackage{xcolor}
\usepackage[colorlinks=true,linkcolor=red,urlcolor=blue,citecolor=blue!90!black]{hyperref}
...
\aistatstitle{Layerwise and Dimensionwise Locally Adaptive Optimization}
...
In this paper, we focus specifically on the training of deep neural networks, under the FL framework.
...
We provide detailed convergence analysis for \algo\, and compare it with other existing methods.
Finally, we provide extensive experimental results with both iid and non-iid local data distribution to validate the theory and demonstrate that the proposed algorithm exhibits faster convergence speed, which could lead to reduced communication cost. In addition, in many cases, \algo\ also achieves improved generalization accuracy.
\end{abstract}
...
\clearpage
 ...
 Section~\ref{app:numericals} contains additional numerical runs on MNIST and FMNIST.
...
When applying the same CNN, as for CIFAR10, to the FMNIST data, we observe similar pattern: \algo\ gives much faster convergence. The \textbf{communication efficiency} can be observed as follows: for example, on MNIST+CNN with 1 local epoch, it takes around 7 rounds for \algo\ to get the accuracy of Fed-AMS at round 16, indicating a more than 50\% reduction in communication. On FMNIST, we can also save $\sim$50\% communication rounds with both 1 and 3 local epochs.


 \begin{figure}[H]
    \begin{center}
        \mbox{
        \hspace{-0.05in}\includegraphics[width=0.3\textwidth]{new_figure/mnist_testerror_mlp_ep1_client50_iid0.pdf}
        \hspace{-0.1in}\includegraphics[width=0.3\textwidth]{new_figure/mnist_testerror_mlp_ep5_client50_iid0.pdf}
        }
        \mbox{
        \hspace{-0.05in}\includegraphics[width=0.3\textwidth]{new_figure/mnist_testerror_cnn_ep1_client50_iid0.pdf}
        \hspace{-0.1in}\includegraphics[width=0.3\textwidth]{new_figure/mnist_testerror_cnn_ep5_client50_iid0.pdf}
        }
        \mbox{
...
%       \begin{center}
%           \mbox{
%           \includegraphics[width=0.45\textwidth]{figure/cifar_testerror_resnet_ep1_client10_iid1_SGD.eps}
%                   \includegraphics[width=0.45\textwidth]{figure/cifar_testerror_resnet_ep3_client10_iid1_SGD.eps}
%           }
%       \end{center}
%   	\caption{\textbf{From Left to Right}: Test accuracy on CIFAR+ResNet9, with iid data distribution. 10 clients and (Left) 1 local epoch, (Right) 3 local epochs.}

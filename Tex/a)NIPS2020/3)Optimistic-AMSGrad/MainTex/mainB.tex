...
Our algorithm is inspired by \textsf{Optimistic Online learning} \citep{CJ12,rakhlin2013online,SALS15,ALLW18,mertikopoulos2018optimistic}, which assumes that a good \emph{predictable process} of the gradient of the loss function in each round of online learning is available, and plays an action by exploiting these predictors. 
By exploiting this (possibly) arbitrary process, algorithms in \textsf{Optimistic Online learning} enjoy smaller regret than the ones without.
...
A single work along that direction stands out. \citep{DISZ18} develops \textsf{Optimistic-Adam} in their paper leveraging optimistic online mirror descent \citep{RS13b}.
Yet, \textsf{Optimistic-Adam} is specifically designed to optimize two-player games (e.g. GANs \citep{goodfellow2014generative}).
GANs is a two-player zero-sum game. There have been some related works in \textsf{Optimistic Online learning} like \citep{CJ12,RS13b,SALS15}) showing that if both players use some kinds of $\textsf{Optimistic}$-update, then accelerating the convergence to the equilibrium of the game is possible.
\citep{DISZ18} was inspired by these related works and showed that \textsf{Optimistic-Mirror-Descent} can avoid the cycle behavior in a bilinear zero-sum game, which accelerates the convergence. 
...
\begin{itemize}
\item We derive an optimistic variant of \textsf{AMSGrad} borrowing techniques from online learning procedures. Our method relies on \textsf{(I)} the addition of \emph{prior knowledge} in the sequence of the model parameter estimations alleviating a predictable process able to provide good guesses of gradients of the loss functions through the iterations and \textsf{(II)} the construction of a \emph{double update} algorithm done sequentially. We interpret this two-projection step as the learning of both an underlying scheme which makes the gradients sequentially predictable and the global parameter learning.
...
Besides the global analysis of \textsf{OPT-AMSGrad}, we conduct experiments and show that the proposed algorithm not only accelerates convergence of loss function, but also leads to better empirical generalization performance.

Section~\ref{sec:prelim} is devoted to introductory notions on online learning for regret minimization and adaptive learning methods for nonconvex stochastic optimization. We introduce in Section~\ref{sec:opt} our new algorithm called \textsf{OPT-AMSGrad} and provide a comprehensive global analysis in both \emph{convex}/\emph{online}  and \emph{nonconvex}/\emph{offline} settings in Section~\ref{sec:analysis}.
...
The idea of \textsf{Optimistic Online learning} (e.g. \citep{CJ12,rakhlin2013online,SALS15,ALLW18}) is as follows.
...
\textsf{Adam} does not have the max operation on line 7 (i.e. $\hat{v}_t = v_t$ for \textsf{Adam}) while \citep{RKK18} adds the operation to guarantee a non-increasing learning rate,$\frac{\eta_t }{ \sqrt{\hat{v}}_t }$, which helps for the convergence (i.e. average regret $\frac{\mathcal{R}_T}{T} \rightarrow 0$).
...
We formulate in this section the proposed optimistic acceleration of AMSGrad, noted \textsf{OPT-AMSGrad}, and detailed in Algorithm~\ref{alg:optamsgrad}.    \vspace{-0.15in}
...
\captionsetup{justification=centering}
    ...
    \caption{\textsf{OPT-AMSGRAD Underlying Structure}.}
     \label{fig:scheme}
\end{figure}
\end{minipage}
...
The learner also updates an auxiliary variable $\tilde{w}_{t+1} \in \Theta$ (line 8). 

Observe that the proposed algorithm does not reduce to \textsf{AMSGrad} when $m_{t}=0$.
Furthermore, combining line 8 and line 9 and get a single line as $w_{t+1}= \tilde{w}_{t}  - \eta_t \frac{\theta_t}{ \sqrt{\hat{v}}_t } - \eta_{t} \frac{h_{t+1}}{ \sqrt{\hat{v}}_t }$. 

Compared to \textsf{AMSGrad}, the updates is constructed by a double level update that interlink some auxiliary state and the model parameter state, as initially introduced in \citep{rakhlin2013online}.
It uses the auxiliary variable (hidden model) to update and commit $w_{t+1}$ (line 9), which exploits the guess $m_{t+1}$ of $g_{t+1}$, see Figure \ref{fig:scheme} for a schematic illustration.
In the following analysis, we show that the interleaving actually leads to some cancellation in the regret bound.

Such two-levels method where the guess $m_t$ is equal to the last known gradient $g_{t-1}$ has been exhibited recently in \citep{chiang2012online}.
The gradient prediction procedure plays naturally an important role and will be tackled Section~\ref{sec:numerical}.


The proposed \textsf{OPT-AMSGrad} inherits three properties:
\begin{itemize}
\item Adaptive learning rate of each dimension as \textsf{AdaGrad} \citep{DHS11}. (line 6, line 8 and line 9)
\item Exponential moving average of the past gradients as \textsf{Nesterov's method} \citep{N04} and the \textsf{Heavy-Ball} method \citep{P64}. (line 5)
\item Optimistic update that exploits a good guess of the next gradient vector
as optimistic online learning algorithms \citep{CJ12,rakhlin2013online,SALS15}. (line 9)
\end{itemize}
...
Consequently, $\psi_t(\cdot)$ is 1-strongly convex with respect to the norm $\| \cdot \|_{\psi_t}:=  \sqrt{ \langle \cdot, \text{diag}\{\hat{v}_t\}^{1/2} \cdot \rangle }$.
Namely, $\psi_t(\cdot)$ satisfies $\psi_t(u) \geq \psi_t(v) + \langle \psi_t(v), u - v \rangle + \frac{1}{2} \| u - v\|^2_{\psi_t}$ for any point $u,v$.
...
\begin{Corollary}
Suppose $\beta_1=0$ and $\{v_t\}_{t>0}$ is an increasing monotone sequence, then we obtain the following regret bound for any $w^{*} \in \Theta$ and sequence $\{ \eta_t \}_{t>0}$: 
\begin{equation} 
\begin{aligned}
\mathcal{R}_T \leq & \frac{ B_{\psi_1}(w^*, \tilde{w}_{1})}{\eta_1}
+ \sum_{t=1}^T\frac{\eta_t}{2} \| g_t - m_t  \|_{\psi_{t-1}^*}^2 +\frac{D_{\infty}^2}{\eta_{\min}} \sum_{i=1}^d \left[ (1-\beta_2) \sum_{s=1}^{T} \beta_2^{T-s} (g_s[i] - m_s[i])^2 \right]^{1/2} \hspace{-0.3cm},
\end{aligned}
\end{equation}
...
For convex regret minimization, the resutls above yields that the learner suffers regret of $\mathcal{O}(\sqrt{\sum_{t=1}^T \| g_t - m_t  \|^2_{\psi_{t-1}}})$ with an access to an arbitrary predictable process $\{m_t\}_{t>0}$ of the mini-batch gradient. The better the predictors, the lower the regret will be.
One can thus wonder how the learner can build those good gradients predictions $\{m_t\}_{t>0}$.
Is this process can be learnt through the iterations?

Those questions are interesting research questions and will not be dealt in this paper for the sake of page limit.
Though, for implementation purposes, we derive a simple, yet effective, gradient prediction algorithm, see Algorithm~\ref{alg:algex} in Section~\ref{sec:numerical} to embed to our new \textsf{OPT-AMSGrad} method.

\textbf{Nonconvex Analysis (Finite-Time Upper Bound):}
...
The choice can accelerate the convergence to equilibrium in some two-player zero-sum games \citep{rakhlin2013online,SALS15,DISZ18}, in which each player uses an optimistic online learning algorithm against its opponent.
...
We propose to use the extrapolation algorithm of \citep{SAB16}.
Extrapolation studies estimating the limit of sequence using the last few iterates \citep{BZ13}. 
Some classical works include Anderson acceleration \citep{WN11}, minimal polynomial extrapolation \citep{CJ76}, reduced rank extrapolation \citep{E79}.
...
The goal is to find the fixed point of $x^{*}$. \citep{SAB16} relaxes the assumption to certain degrees. It assumes that the sequence $\{x_t\} \in \mathbb R^d$ satisfies
...
The extrapolation algorithm we used is shown in Algorithm~\ref{alg:algex}.
Some theoretical guarantees regarding the distance between the output and $x^*$ are provided in \citep{SAB16}.
\begin{algorithm}[h]
\begin{algorithmic}[1] 
\small
...
\end{algorithmic}
\end{algorithm}
For our numerical experiments in the next section, we run \textsf{OPT-AMSGrad} using Algorithm~\ref{alg:algex} to get $m_t$. 
Specifically, $m_t$ is obtained by \textsf{(a)} calling Algorithm~\ref{alg:algex} with input being a sequence of some past $r+1$ gradients, $\{ g_t, g_{t-1},g_{t-2}, \dots, g_{t-r} \}$, %to obtain $m_t$,
where $r$ is a parameter and \textsf{(b)} setting $m_t:= \Sigma_{i=0}^{r-1} c_i g_{t-r+i}$ from the output of Algorithm~\ref{alg:algex}.
...
In this section, we provide experiments on classification tasks with various neural network architectures and datasets to demonstrate the effectiveness of \textsf{OPTIMISTIC-AMSGrad}.

\textbf{Methods.}\hspace{0.1in}We consider two baselines. The first one is the original \textsf{AMSGrad}. 
The hyper-parameters are set to be $\beta_1$ and $\beta_2$ to be $0.9$ and $0.999$ respectively, as recommended by \citep{RKK18}. We tune the learning rate $\eta$ over a fine grid and report the best result. 
The other competing method is the aforementioned \textsf{Optimistic-Adam$+\hat{v}_t$} method, see \citep{DISZ18}. The key difference is that it uses previous gradient as the gradient prediction of the next iteration. We also report the best result achieved by tuning the step size $\eta$ for \textsf{Optimistic-Adam$+\hat{v}_t$}.
For \textsf{Optimistic-AMSGrad}, we use the same $\beta_1$, $\beta_2$ and 
the best step size $\eta$ of \textsf{AMSGrad} for a fair evaluation of the improvement brought by the extra optimistic step. Yet,
\textsf{Optimistic-AMSGrad} has an additional parameter $r$ that controls the number of previous gradients used for gradient prediction. Fortunately, we observe similar performance of \textsf{Optimistic-AMSGrad} with different values of $r$. Hence, we report $r=5$ for now when comparing with other baselines. We will address on the choice of $r$ at the end of this section.
...
\textbf{Datasets.}\hspace{0.1in}Following \citep{RKK18} and \citep{KB15}, we compare different algorithms on \textit{MNIST}, \textit{CIFAR10},
\textit{CIFAR100}, and \textit{IMDB} datasets. 
For \textit{MNIST}, we use two noisy variants named as 1.65\textit{MNIST-back-rand} and 1.65\textit{MNIST-back-image} from \citep{MNIST07}. They both have 12000 training samples and 50000 test samples, where random background is inserted to the original \textit{MNIST} hand written 
...
\end{center}
\caption{Training loss with different $r$.}\label{fig:compare}
\end{wrapfigure}
\vspace{-0.1in}
Recall that our proposed algorithm has the parameter $r$ that governs the use of past information.
Figure~\ref{fig:compare} compares the performance under different values of $r=3,5,10$ on two datasets. From the result we see that the choice of $r$ does not have significant impact on learning performance. Taking into consideration both quality of gradient prediction and computational cost, it appears that $r=5$ is a good choice. We remark that empirically, the performance comparison among $r=3,5,10$ is not absolutely consistent (i.e. more means better) in all cases. One possible reason is that for deep neural nets (with highly non-convex loss), using gradient information from too long ago may not be helpful in accurate gradient prediction. Nevertheless, $r=5$ seems to be good for most applications.

\subsection{Some Remarks on the Experiments}
\textbf{Discussion on the iteration cost:}
We observe that the iteration cost (i.e., actual running time per iteration) of our implementation of \textsf{Optimistic-AMSGrad} with $r=5$ is roughly two times larger than the standard \textsf{AMSGrad}. When $r=3$, the cost is roughly 0.7 times longer. Nevertheless, \textsf{OPTIMISTIC-AMSGrad} may still be beneficial in terms of training efficiency, since fewer iterations are typically needed. For example, in Figure \ref{fig:testandtrain}, to reach the training loss of \textsf{AMSGrad} at 100 epochs, the proposed method only needs roughly 20 and 40 epochs, respectively. That said, \textsf{OPTIMISTIC-AMSGrad} needs 40\% and 80\% time to achieve same training loss as \textsf{AMSGrad}, in this two problems.
...
\noindent \textbf{Memory usage:} Our algorithm needs a storage of past $r$ gradients for each coordinate, in addition to the estimated second moments and the moving average. Though it seems demanding compared to the standard AMSGrad, it is relatively cheap compared to Natural gradient method (e.g.,~\citep{MG15}), as Natural gradient method needs to store some matrix inverse.
\section{Conclusion}
In this paper, we propose \textsf{Optimistic-AMSGrad}, which combines optimistic learning and \textsf{AMSGrad} to improve sampling efficiency and
accelerate the process of training, in particular for deep neural networks. With a good gradient prediction, the regret can be smaller than that of standard \textsf{AMSGrad}. Experiments on various deep learning problems demonstrate the effectiveness of the proposed method in improving the training efficiency. 
...
\begin{equation} \label{bound:optadabeta}
\begin{aligned}
\mathcal{R}_T \leq & \frac{1}{\eta_{\min}} D_{\infty}^2 \sum_{i=1}^d \hat{v}_{T}^{1/2}[i] + \frac{ B_{\psi_1}(w^*, \tilde{w}_{1})}{\eta_1}
...
of \textsf{Adam}-type algorithms when applying them to smooth nonconvex optimization problems.
...
\textsf{Optimistic-AMSGrad} to nonconvex optimization,
...
for convex loss functions (e.g. \textsf{AdaGrad}) to the ones that can find an approximate stationary point of non-convex loss functions. 
Their approaches are modular so that simply using \textsf{Optimistic-AMSGrad}
as the base algorithm in their methods will immediately lead to a variant of \textsf{Optimistic-AMSGrad} that enjoys some guarantee on nonconvex optimization.
The variant can outperform the ones instantiated by other \textsf{Adam}-type algorithms when
...
and $\| \cdot \|_{(t)}:= 
\sqrt{ \langle \cdot, \text{diag}\{\hat{v}_t\}^{1/2} \cdot \rangle }$, then the update might be viewed as an optimistic variant of $\textsf{AdaGrad}$. However, no experiments was provided in \citep{MY16}. 


\textbf{Comparison to \textsf{Optimistic-Adam}~\citep{DISZ18}.}\hspace{0.1in}We are aware that \citep{DISZ18} proposed one version of optimistic algorithm for ADAM, which
is called \textsf{Optimistic-Adam} in their paper. A slightly modified version is summarized in Algorithm~\ref{OPT-DISZ}. Here, \textsf{Optimistic-Adam$+\hat{v}_t$} is \textsf{Optimistic-Adam} in \citep{DISZ18} with the additional max operation $\hat{v}_t = \max ( \hat{v}_{t-1}, v_t)$ to guarantee that the weighted second moment is monotone increasing.

\begin{algorithm}[h]
\begin{algorithmic}[1]
\caption{\textsf{Optimistic-Adam~\citep{DISZ18}+$\hat{v}_t$}. \label{OPT-DISZ}}
\STATE Required: parameter $\beta_1$, $\beta_2$, and $\eta_t$.
...
We want to emphasize that the motivations are different. \textsf{Optimistic-Adam} in their paper is designed to optimize two-player games (e.g. GANs \citep{goodfellow2014generative}),
...
\citep{DISZ18} focuses on training GANs \citep{goodfellow2014generative}. GANs is a two-player zero-sum game. There have been some related works in \textsf{Optimistic Online learning} like \citep{CJ12,RS13b,SALS15})
showing that if both players use some kinds of $\textsf{Optimistic}$-update,
then accelerating the convergence to the equilibrium of the game is possible.
\citep{DISZ18} was inspired by these related works and showed that \textsf{Optimistic-Mirror-Descent}
can avoid the cycle behavior in a bilinear zero-sum game, which accelerates the convergence. Furthermore, \citep{DISZ18} did not provide theoretical analysis of \textsf{Optimistic-Adam}.



\section{Additional Remarks and Runs on the Gradient Prediction Process}
\textbf{Two illustrative examples.}\hspace{0.1in}We provide two toy examples to demonstrate how \textsf{OPTIMISTIC-AMSGrad} works with the chosen extrapolation method. First, consider minimizing a quadratic function $H(w) := \frac{b}{2} w^2 $  
...
\textsf{Optimistic-AMSGrad} with $\beta_1=0$ and the second moment term $\hat{v}_t$ being dropped: 
...
\footnote{\citep{RKK18} uses this example to show that \textsf{Adam} \citep{KB15} fails to converge.}.

...
Our algorithm is inspired by \textsf{Optimistic Online learning} \citep{CJ12,rakhlin2013online,SALS15,ALLW18,mertikopoulos2018optimistic}, which assumes that a good \emph{guess} of the loss function in each round of online learning is available, and plays an action by exploiting the guess. 
...
Compared to \textsc{AMSGrad}, the updates is constructed by a double level update that interlink some auxiliary state and the model parameter state, as initially introduced in \citep{rakhlin2013online}.
...
as optimistic online learning algorithms \cite{CJ12,rakhlin2013online,SALS15}. (line 9)
\end{itemize}
...
\subsection{Convex Analysis: Regret Analysis}
...
The choice can accelerate the convergence to equilibrium in some two-player zero-sum games \cite{rakhlin2013online,SALS15,DISZ18}, in which each player uses an optimistic online learning algorithm against its opponent.

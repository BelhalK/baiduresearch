...
In the stochastic optimization literature, the problem we are tackling formulates as follows:
...
\beq
\norm{w_t} \leq W \quad \textrm{almost surely}
\eeq
\end{assumption}
...
Assume assumption H~\ref{ass:bounded}, then the quantities defined in Algorithm~\ref{alg:opt-ams} satisfy for any $w \in \Theta$ and $t>0$:
...
\beq
\sum_{k=1}^{T_{\sf max}} \eta_{t}^{2} \EE \left[\left\|\hat{v}_{t}^{-1/2} \theta_{t}\right\|_{2}^{2}\right] \leq  \frac{\eta^{2} d T_{\sf max} (1- \beta_1)}{(1 - \beta_2)(1-\gamma)} 
\eeq
\end{Lemma}
...
In this section, we provide experiments on classification tasks with various neural network architectures and datasets to demonstrate the effectiveness of \textsf{OPT-AMSGrad} in practical applications.

\textbf{Methods.} We consider two baselines. The first one is the original \textsf{AMSGrad}. 
...
The other competing method is the aforementioned \textsf{Optimistic-Adam} method \cite{DISZ18}.
 The key difference is that it uses previous gradient as the gradient prediction of the next iteration. 
 We also report the best result achieved by tuning the step size $\eta$ for \textsf{Optimistic-Adam}.

For \textsc{OPT-AMSGrad}, we use the same $\beta_1$, $\beta_2$ and 
...
\textsc{OPT-AMSGrad} has an additional parameter $r$ that controls the number of previous gradients used for gradient prediction. Fortunately, we observe similar performance of \textsc{OPT-AMSGrad} with different values of $r$. Hence, we report $r=5$ for now when comparing with other baselines. We will address on the choice of $r$ at the end of this section.

...
State-of-the-art algorithms like \textsc{AMSGrad} \citep{RKK18} and \textsc{AdAM} \citep{KB15} leverage these ideas to accelerate the training process of highly nonconvex objective functions such as deep neural networks losses.
...
We emphasize in Section~\ref{sec:opt} the importance of leveraging a good guess $m_t$ for updating $w_t$ in order to get a fast convergence rate (or equivalently, small regret) and present Section~\ref{sec:numerical} a simple yet effective predictable process $\{m_{t}\}_{t>0}$ leading to an empirical evidence of acceleration.
...
%For the hyper-parameters of \textsc{AMSGrad}, it is suggested in~\citep{RKK18} that $\beta_1=0.9$ and $\beta_2=0.99$.
\vspace{-0.1in}
...
The learner also updates an auxiliary variable $\tilde{w}_{t+1} \in \Theta$ (line 8) and finally updates the current model parameter (line 9).
...
Furthermore, combining line 8 and line 9 and get a single update as $w_{t+1}= \tilde{w}_{t}  - \eta_t \frac{\theta_t}{ \sqrt{\hat{v}}_t } - \eta_{t} \frac{h_{t+1}}{ \sqrt{\hat{v}}_t }$. 
Compared to \textsc{AMSGrad}, the algorithm is characterized by a two-level update that interlink some auxiliary state $\tilde{w}_{t}$ and the model parameter state, $w_t$, similarly to the \textsc{Optimistic Mirror Descent} algorithm developed in \citep{rakhlin2013online}.
It leverages the auxiliary variable (hidden model) to update and commit $w_{t+1}$, which exploits the guess $m_{t+1}$ of $g_{t+1}$, see Figure \ref{fig:scheme} for a schematic illustration.
...
The last one, perhaps less known outside the \textsc{Online learning} community, can actually lead to acceleration when the prediction of the next gradient is good. This property will be elaborated in the following subsection in which we provide the theoretical analysis of \textsc{OPT-AMSGrad}.
...
Though, for implementation purposes, we derive a simple, yet effective, gradient prediction algorithm, see Algorithm~\ref{alg:algex} in Section~\ref{sec:numerical} to embed to our new \textsc{OPT-AMSGrad} method.
...
\beq
\begin{split}
\EE\left[\|\nabla f(w_T)\|^2\right] \leq \tilde{C}_1 \sqrt{\frac{d}{T_{\sf max}}} + \tilde{C}_2 \frac{1}{T_{\sf max}} \eqsp,
\end{split}
\eeq
...
From the analysis in the previous section, we know that whether \textsc{OPT-AMSGrad} converges faster than its counterpart depends on how $m_{t}$ is chosen.
...
\end{algorithmic}
\end{algorithm}
For our numerical experiments in the next section, we run \textsc{OPT-AMSGrad} using Algorithm~\ref{alg:algex} to get $m_t$. 
...
The other competing method is the aforementioned \textsc{Optimistic-Adam$+\hat{v}_t$} method, see \citep{DISZ18}. The key difference is that it uses previous gradient as the gradient prediction of the next iteration. We also report the best result achieved by tuning the step size $\eta$ for \textsc{Optimistic-Adam$+\hat{v}_t$}.

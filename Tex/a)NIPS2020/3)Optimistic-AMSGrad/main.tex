...
By capitalizing on this (possibly) arbitrary process, algorithms in \textsc{Optimistic Online learning} enjoy smaller regret than the ones without.
...
The gradient prediction process plays an important role as discussed Section~\ref{sec:numerical}.
...
Suppose $\beta_1=0$ and $\{v_t\}_{t>0}$ is an increasing monotone sequence, then we obtain the following regret bound for any $w^{*} \in \Theta$ and sequence of stepsizes $\{ \eta_t = \eta/\sqrt{t}\}_{t>0}$: 
\begin{equation}\notag
\begin{aligned}
\mathcal{R}_T \leq & \frac{ B_{\psi_1}(w^*, \tilde{w}_{1})}{\eta_1}
...
\end{assumption}
We now derive needed auxiliary Lemmas for our global analysis.
...
\eeq
\end{Lemma}
We now formulate the main result of our paper giving a finite-time upper bound of the suboptimality condition $\EE\left[\|\nabla f(w_T)\|^2\right]$ as the convergence criterion of interest, see \citep{ghadimi2013stochastic}.
...
\begin{wrapfigure}[12]{r}{1.7in}\vspace{-0.5in}
\begin{center}
\mbox{

...
Is this process can be learned through the iterations?
Those questions are interesting research questions and will not be dealt in this paper for the sake of page limit.
Though, for implementation purposes, we derive a simple, yet effective, gradient prediction algorithm, see Algorithm~\ref{alg:algex} in Section~\ref{sec:numerical} to embed to our new \textsc{OPT-AMSGrad}.
...
The random termination number \eqref{eq:random} is inspired by \citep{ghadimi2013stochastic} which enables to derive novel non-asymptotic convergence results to stationary point for non-convex optimization. 

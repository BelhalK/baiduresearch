...
\beq
\overline{w}_{k+1} - \overline{w}_k = \frac{\beta_1}{1 - \beta_1} \tilde{\theta}_{k-1} \left[ \eta_{k-1} \hat{v}_{k-1}^{-1/2} - \eta_{k} \hat{v}_{k}^{-1/2}\right] - \eta_{k} \hat{v}_{k}^{-1/2} \tilde{g}_k \eqsp,
\eeq
%where $\tilde{\theta}_k = \theta_k + \beta_1 \theta_{k-1}$ and $\tilde{g}_k = g_k + m_{k+1} - \beta_1(g_{k-1} + m_k)$. 
...
\end{split}
\eeq
%Denote $\tilde{\theta}_k = \theta_k + \beta_1 \theta_{k-1}$ and $\tilde{g}_k = g_k + m_{k+1} - \beta_1(g_{k-1} + m_k)$.
...
\beq
\begin{split}
\overline{w}_{k+1} - \overline{w}_k = \frac{\beta_1}{1- \beta_1} \tilde{\theta}_{k-1} \left[ \eta_{k-1} \hat{v}_{k-1}^{-1/2} - \eta_{k} \hat{v}_{k}^{-1/2} \right] - \eta_k \hat{v}_k^{-1/2} \tilde{g}_k
\end{split}
\eeq
\end{proof}
...
\beq\label{eq:termA1}
\begin{split}
\nabla f(w_k)^\top (\overline{w}_{k+1} - \overline{w}_k)  \leq  \frac{\beta_1 (1+\beta_1 + a)}{1 - \beta_1} \major^2 \left[ \norm{\eta_{k-1} v_{k-1}^{-1/2}} - \norm{\eta_{k} v_{k}^{-1/2} }\right] - \nabla f(w_k)^\top\eta_{k} v_{k}^{-1/2} \tilde{g}_k 
\end{split}
\eeq
...
Observe that at layer $\ell$:
\beq\label{eq:boundderivativeloss}
\begin{split}
\norm{\nabla_{w^{(\ell)}}  \mathcal{L}(\textsf{MLN}( w, \xi), y)} & =  \norm{\mathcal{L}'(\textsf{MLN}( w, \xi), y)\nabla_{w^{(\ell)}}\textsf{MLN}( w, \xi)}\\
&  = \norm{\mathcal{L}'(\textsf{MLN}( w, \xi), y)\sigma'(w^{(\ell)} h^{(\ell-1)}(w,\xi))h^{(\ell-1)}(w,\xi)}\\
& \leq \frac{T}{4}
\end{split}
\eeq
...
Suppose an iteration index $K$ and a coordinate $i$ of the last layer $L$ such that $w^{(L)}_{K, i} \geq \frac{T}{4} + B$.
...
This last equation yields $\theta^{(L)}_{K,i} \geq 0$ (given the algorithm and $\beta_1 = 0$).
\newpage

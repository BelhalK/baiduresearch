...
\begin{itemize}
\item In this paper, we investigate the possibility of using any adaptive gradient methods in the decentralized, and gossip-based (nodes have only a local view of the whole communication graph), training paradigm. We develop a general technique that converts an adaptive gradient method from a centralized method to its decentralized variant.
...
We show the importance of consensus on adaptive learning rates by proving a divergent problem instance for a recently proposed decentralized adaptive gradient method, namely, DADAM~\citep{nazari2019dadam}, a decentralized version of ADAM, which lacks of consensus mechanisms on the adaptive learning rates of each node.
...
The network contains only two nodes and the matrix $W$ satisfies $W_{ij} = \frac{1}{2}$ for all indices $i,j$. 
For simplicity, we consider running DADAM algorithm with $\beta_1 = \beta_2 = \beta_3 = 0$ and $\epsilon = 0.6$.   
Suppose we initialize DADAM at $x_{1,i} = 0$ for all $i \in [N]$ and use the following learning rate $\alpha = 0.001$.  
We observe that at $x_{1,i} = 0$, $\nabla f_1(x_{1,1}) = 0$, we have  $\nabla f_2(x_{1,2}) = 1$, leading to $\hat v_{1,1} = 0.6$ and $\hat v_{1,2} = 1$. 
Thus, from step 1, we will obtain $\hat v_{1,2} \geq 1$. 
In addition, it is can be easily proved that, with the above selected stepsize, we always have $\hat v_{1,1} < 1 $, in fact, it will never reach the value of $0.6$.
...
However, in our decentralized setting, every node can only communicate with its neighbors (gossip-based) and such central server does not exist.
...
For the terms composing $T_3$ in \eqref{eq: exp_telescope}, we can upper bound them by
\begin{align}\notag
...
\end{align}
where the second inequality is due to Lemma \ref{lem: mean_after_max}, introduced Section~\ref{app: proof_lemmas}, and the fact that $U_t = \max(\tilde U_t,\epsilon)$ element-wisely.
%\begin{lemma}\label{lem: mean_after_max}
%	Given  a set of numbers $a_1,\cdots,a_n$ and denote their mean to be $\bar a = \frac{1}{n}\sum_{i=1}^n a_i$. In addition, define $b_i(r) \triangleq = \max(a_i,r)$ and $\bar b (r) =  \frac{1}{n}\sum_{i=1}^n b_i(r)$. For any $r$ and $r'$ with $r' \geq r$ we have 
%	\begin{align}\notag
%	\sum_{i=1}^n |b_i(r) - \bar b(r)| \geq \sum_{i=1}^n |b_i(r') - \bar b(r')|

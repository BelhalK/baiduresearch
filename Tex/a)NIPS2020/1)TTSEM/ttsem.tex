...
While Theorem~\ref{thm:isaem} suffers only from the MC noise induced by the latent data sampling step, Theorem~\ref{thm:vrsaem} and Theorem~\ref{thm:fisaem} exhibit in their convergence bounds two different phases. The upper bounds display a bias term due to the initial conditions, \ie the term $ \Delta V = V(\hat{\bss}^{(0)} )  - V(\hat{\bss}^{(K_{\sf max})}) $, and a double dynamic burden exemplified by the term $\EE\big[\norm{ \hs{k} -  \tilde{S}^{(k)} }^2\big] $. Indeed, the following remarks are worth doing on this quantity :
\vspace{-.2cm}
\begin{itemize}

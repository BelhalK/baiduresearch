...
We motivate the choice of double dynamics by invoking the variance reduction virtue of each stage of the method on both sources of noise: the incremental update and the MC approximation.
...
\textbf{Contributions} This paper \textit{introduces} and \textit{analyzes} a new class of methods which purpose is to update two proxies for target expected quantities in a two-timescale manner. 
Those approximated quantities are then used to optimize \eqref{eq:em_motivate} for modern examples and settings using EM \textsf{Maximization} step.
...
% The Stochastic Approximation is performed during the remaining iterations where $\gamma_k = 1/k^\alpha$, where $\alpha \in (0,1)$, ensuring the almost sure convergence of the estimate.
 It is inappropriate to start with small values for step size $\gamma_k$ and large values for the number of simulations $M_k$. Rather, it is recommended that one decrease $\gamma_k$, as in $\gamma_k = 1/k^\alpha$, where $\alpha \in (0,1)$, and keep a constant and small number of MC samples $M_k$ which shows a great advantage over the \textsf{MC-step} \eqref{eq:mcstep}, which requires large $M_k$ to converge.
...
Efficient strategies to scale to large datasets include incremental and variance reduced methods.
We will explicit a general update that covers those latter variants and that represents the \textit{second level} of our algorithm, namely the incremental update of the approached statistics $\hat{S}^{(k)}$ inside the \textsf{SA-Step}.
\beq \label{eq:sestep}
...
The following table provides the definitions of the proxy $\StocEstep^{(k)}$ for three variants of \eqref{eq:sestep}.
...
%\hline
%\textbf{Algorithm} & \textbf{Statistics Proxy $\StocEstep^{(k+1)}$} & $\bm{ \rho_k}$ \\
%\hline
%\ISAEM\ \citep{ kuhn2019properties} & $ \StocEstep^{(k+1)} = \StocEstep^{(k)} + {\textstyle \frac{1}{n}}\big( \tilde{S}_{i_k}^{(k)}  - \tilde{S}_{i_k}^{(\tau_{i_k}^k)} \big) \quad \tagarray\label{1}$ & $1$ \\ [0.2cm]
%\SAEMVR\ & $\StocEstep^{(k+1)} = \tilde{S}^{(\ell(k))} +  \big( \tilde{S}_{i_k}^{(k)}  -\tilde{S}_{i_k}^{(\ell(k))}   \big) \label{eq:vrsaem}\quad \tagarray\label{eq:isaem}$ & $\rho$ \\ [0.2cm]
%\FISAEM\ & $\StocEstep^{(k+1)} = \overline{\StocEstep}^{(k)} + \big( \tilde{S}_{i_k}^{(k)}  - \tilde{S}_{i_k}^{(t_{i_k}^k)} \big) \label{eq:fisaem} , \quad  \overline{\StocEstep}^{(k+1)} = \overline{\StocEstep}^{(k)} + n^{-1} \big( \tilde{S}_{j_k}^{(k)}  - \tilde{S}_{j_k}^{(t_{j_k}^k)} \big)$ &$ \rho$ \\ [0.2cm]
%\hline
%\end{tabularx}
%\end{center}
\vspace{-0.3cm}
...
\beq\notag
\tilde{S}_{i_k}^{(k)}=  \frac{1}{M_k} \sum_{m=1}^{M_k} S(z_{i_k,m}^{(k)}, y_{i_k}) \quad \textrm{with} \quad z_{i_k,m}^{(k)}, \sim p(z_{i_k}|y_{i_k};\theta^{(k)})\eqs.
\eeq
The stepsize is set to $\rho_{k+1} = 1$ for the \ISAEM\ method and we initialize with $\StocEstep^{(0)} = \tilde{S}^{(0)}$; $\rho_{k+1} = \rho$ is  constant for the \SAEMVR\ and \FISAEM\ methods. Note that we initialize as follows $\overline{\StocEstep}^{(0)} = \overline{\bss}^{(0)}$ for the \FISAEM\ which can be seen as a slightly modified version of SAGA inspired by \citep{reddi2016fast}.
...
\end{algorithm}
\vspace{-.4cm}
The update in \eqref{eq:twolevels} is said to have two-timescale as the step sizes satisfy $\lim \limits_{k \to \infty} \gamma_k/\rho_k < 1$ such that $ \tilde{S}^{(k+1)} $  is updated at a faster time-scale, determined by $\rho_k$, than $\hat{\bss}^{(k+1)}$, determined by $\gamma_k$.
...
%The first intermediate result, see proof in Appendix~\ref{app:prooflemmainc}, is the computation of the quantity $\hat{S}^{(k+1)} - \hat{\bss}^{(k)}$, which corresponds to the drift term of \eqref{eq:rmstep} and reads as follows:
...
%\end{Lemma}
The following Theorem for the \ISAEM\ algorithm, with proof in Appendix~\ref{app:theoremisaem}, is derived under a control of the Monte Carlo fluctuations as described by Assumption H\ref{ass:mcerror} and is built upon an intermediary Lemma, detailed in Appendix~\ref{app:theoremisaem}, characterizing the quantity of interest $\hat{S}^{(k+1)} - \hat{\bss}^{(k)}$.
...
\beq\notag
\begin{split}
 \EE[ \| \grd V( \hs{K} ) \|^2 ] \leq & \frac{2 n^{2/3} \overline{L}}{\mu {\sf P}_{\sf max} \upsilon_{\min}^2\upsilon_{\max}^2}\EE[ V( \hs{0} ) - V( \hs{K_{\sf max}}) ] \\
 &+ \frac{2 n^{2/3} \overline{L}}{\mu{\sf P}_{\sf max}  \upsilon_{\min}^2\upsilon_{\max}^2} \sum_{k=0}^{K_{\sf max}-1} \left[  \tilde{\eta}^{(k+1)} + \chi^{(k+1)} \EE\left[\norm{ \hs{k} - \tilde{S}^{(k)}}^2\right]\right]\eqs.
 \end{split}
\eeq
\end{Theorem} 
...
\beq\notag
\begin{split}
 \EE[ \| \grd V( \hs{K} ) \|^2 ] \leq &\frac{4\alpha  \overline{L} n^{2/3}}{{\sf P}_{\sf max}\upsilon_{\min}^2\upsilon_{\max}^2}  \big[ V(\hat{\bss}^{(0)} )  - V(\hat{\bss}^{(K_{\sf max})}) \big]\\
 &   + \frac{4\alpha  \overline{L} n^{2/3}}{{\sf P}_{\sf max}\upsilon_{\min}^2\upsilon_{\max}^2} \sum_{k=0}^{K_{\sf max}-1} \left[ \Xi^{(k+1)}  +\Gamma_{k+1} \EE\left[\norm{ \hs{k} - \tilde{S}^{(k)}}^2\right]\right]\eqs.
\end{split}
\eeq
%\beq
%\begin{split}
...
This section presents several numerical applications for our proposed algorithm.


\subsection{Gaussian Mixture Models}

\begin{wrapfigure}[12]{r}{2.5in}\vspace{-0.5in}
\begin{center}
...
The constraint set on $\Param$ is given by $\Param = \{ \omega_m,~m=1,...,M-1 : \omega_m \geq 0,~\sum_{m=1}^{M-1} \omega_m \leq 1\} \times \{ \mu_m \in \rset ,~m=1,...,M \}.$
%\beq \textstyle\notag
...
\textbf{Numerical Experiment:} We apply model \eqref{eq:deformablemodel} and our algorithms to a collection of handwritten digits, called the US postal database \citep{hull1994database}, featuring $n = 1\, 000$ $(16 \times 16)$-pixel images for each class of digits from $0$ to $9$.
...
Intuitively, if those variances are large, the kernels centered arounds the equidistributed landmark spread out on too many of its neighbors. Bad choices of such hyperparameters can lead to thicker shapes.

%
%\begin{figure}[H]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{pic_paper/deformabledigit5.eps}\vspace{-.2cm}
%\caption{Estimation of the template: from top to bottom: batch, online, \ISAEM\ ,\SAEMVR\ and \FISAEM\  . Columns represent 1 to 7 epochs.}\vspace{-.2cm}
%\label{fig:results}
%\end{center}
%\end{figure}
%
%\begin{figure}[H]
%\begin{center}
%\includegraphics[width=\textwidth]{pic_paper/moreepochs}\vspace{-.2cm}
%\caption{Estimation of the template: from top to bottom: batch, online, \ISAEM\ ,\SAEMVR\ and \FISAEM\ through 7 epochs. Note that Batch method templates are replicated in-between epochs for a fair comparison with incremental variants. }\vspace{-.2cm}
%\label{fig:results}
%\end{center}
%\end{figure}
...
\end{center}
\end{figure}
Figure~\ref{fig:results} displays the virtue of the \SAEMVR\ and \FISAEM\ methods that obtain a more \textit{contrasted} and \textit{accurate} template estimate. The incremental and online version are looking much better on the very first epochs compared to the batch method, which is intuitive given the high computational cost of the batch method. After a few epochs, the batch SAEM seem to estimate similar template as the incremental an online methods due to the high variance of the latter procedures. Our variance reduced and fast incremental come into play in the long run and sharpen the final template estimates contrasting between the background and the regions of interest in the image.
...
&   + \frac{4\alpha  \overline{L} n^{2/3}}{\upsilon_{\min}^2} \sum_{k=0}^{K_{\sf max}-1} \Xi^{(k+1)}  +  \sum_{k=0}^{K_{\sf max}-1}\Gamma_{k+1} \EE\left[\norm{ \hs{k} - \tilde{S}^{(k)}}^2\right]
\end{split}
\eeq
...
\begin{split}
 ...
 &   + \frac{4\alpha  \overline{L} n^{2/3}}{\upsilon_{\min}^2\upsilon_{\max}^2} \sum_{k=0}^{K_{\sf max}-1} \Xi^{(k+1)}  +  \sum_{k=0}^{K_{\sf max}-1}\Gamma_{k+1} \EE\left[\norm{ \hs{k} - \tilde{S}^{(k)}}^2\right]
\end{split}
\eeq


%\textbf{ Bounding $ \EE\big[\norm{ \hs{k} -  \tilde{S}^{(k)} }^2\big] $} Remark that this term is the price we pay for the two-timescale dynamics and corresponds to the gap between the two asynchronous updates (one is on  $\hs{k}$ and the other on $ \tilde{S}^{(k)}$).
%
%

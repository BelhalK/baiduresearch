...
\begin{document}
\title{\vspace{-0.1in}Fast Two-Timescale Stochastic EM Algorithms\vspace{-0.15in}}
\author{
  Belhal Karimi \\
...
\caption{Two-Timescale Stochastic EM methods.}\label{alg:ttsem}
  \begin{algorithmic}[1]
  \STATE \textbf{Input:} initializations $\hat{\param}^{(0)} \leftarrow 0$, $\hat{\bss}^{(0)} \leftarrow \tilde{S}^{(0)}$, ${\sf K}_{\sf m }$ $\leftarrow$ maximum iteration number. \STATE Set the terminating iteration number, $K \in \{0,\dots,{\sf K}_{\sf m }-1\}$, as a discrete r.v.~with:\vspace{-.1cm}
  \beq \label{eq:random}
...
Recalling that $K$ is an independent discrete r.v.~drawn from $\{1,\dots,{\sf K}_{\sf m }\}$ with distribution $\{ \gamma_k/ {\sf P}_{\sf m},0 \leq k \leq {\sf K}_{\sf m } - 1 \}$, as in \eqref{eq:random}, then the convergence criterion used in our study reads
\beq\notag
...
\eeq
where we recall $\operatorname{B}(\bss) = \jacob{ \phi }{ \param }{ \mstep{\bss} } \Big( \hess{{L}}{\param}( {\bss}; \mstep{\bss} )  \Big)^{-1} \jacob{ \phi }{ \param }{\mstep{\bss} }^\top$. The proof of \eqref{eq:semigrad2} follows directly from the assumption~H\ref{ass:eigen}.
\end{proof}

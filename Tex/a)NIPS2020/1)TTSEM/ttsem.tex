...
\textbf{Contributions:} This paper \textit{introduces} and \textit{analyzes}\footnote{Proofs of this paper are moved to the Supplementary Material} a new class of methods which purpose is to update two proxies for the target expected quantities in a two-timescale manner. 
...
\textbf{Role of the stepsize $\gamma_k$:} It is inefficient to start with small values for step size $\gamma_k$ and large values for the number of simulations $M_k$. 
...
We present in this section a finite-time and global (independent of the initilization) analysis of the incremental variant of the Stochastic Approximation of the EM algorithm. 
%We want to draw the attention of the readers that the word "global" here does not mean for a global optimum of the nonconvex function, but of the independence of our analysis on the initialization and the iteration $k$ (finite time).
...
The upper bounds display \textsf{(1)} a bias term due to the initial conditions, \ie the term $ \Delta V = V(\hat{\bss}^{(0)} )  - V(\hat{\bss}^{(K_{\sf max})}) $, and \textsf{(2)} a double dynamic burden exemplified by the term $\EE\big[\norm{ \hs{k} -  \tilde{S}^{(k)} }^2\big] $. Indeed, the following remarks are worth doing on this quantity :
\vspace{-.2cm}
\begin{itemize}
\item This term is the price we pay for the two-timescale dynamic and corresponds to the gap between the two \emph{asynchronous} updates (one on  $\hs{k}$ and the other on $ \tilde{S}^{(k)}$).
...
Two numerical examples illustrate the benefits of our scheme on both toy dataset and real complex tasks.

...
\caption{OPTIMISTIC-AMSGRAD}\label{alg:sem}
  ...
  \STATE Get mini-batch stochastic gradient $g_{k}$ at $w_{k}$
   \STATE $theta_{k}=\beta_{1} \theta_{k-1}+\left(1-\beta_{1}\right) g_{k}$
   \STATE $\hat{v}_{k}=\max \left(\hat{v}_{k-1}, v_{k}\right)$
   \STATE $v_{k}=\beta_{2} v_{k-1}+\left(1-\beta_{2}\right)\left(g_{k}-m_{k}\right)^{2}$
   \STATE $ \hat{v}_{k}=\max \left(\hat{v}_{k-1}, v_{k}\right)$
   \STATE $ w_{t+\frac{1}{2}}=\Pi_{\mathcal{K}}\left[w_{t-\frac{1}{2}}-\eta_{k} \frac{\theta_{k}}{\sqrt{\hat{v}_{k}}}\right]$
   \STATE $ w_{k+1}=\Pi_{\mathcal{K}}\left[w_{t+\frac{1}{2}}-\eta_{k+1} \frac{\frac{h_{k+1}}{\sqrt{v}_{k}}}{\beta_{1}}\right]$

...
An extended analysis of MISO in both the convex and nonconvex case has been proposed in \citep{qian2019miso}.
...
Our work aims to build the theory to understand the behavior for a \textit{generic class} of incremental stochastic surrogate methods for nonconvex optimization. 
...
Nevertheless, the dependence on the dimension highly depends on the class of surrogate functions $\mathcal{F}$ used in our scheme, as smaller bounds on these controlling terms can be derived for simpler class, such as quadratic functions.
...
\eSur{k}{\param} \eqdef \Sur{k}{\param}- {\cal L} ( \param ),~~~ \overline{M}_{(k)}= \sum_{k=0}^{K_{\sf max}-1} \Bsize{k}^{-1/2}\eqsp.
\eeq

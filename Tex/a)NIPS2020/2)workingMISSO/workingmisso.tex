...
\beq
\EE \left[\underset{f \in \mathcal{F}}{ \sup } \left|\frac{1}{M} \sum_{i=1}^{M} f\left(z_{i,m}\right)-\EE[f(z_i)]\right| \right] \leq \frac{C L}{\sqrt{M}} \quad \textrm{for all} \quad z_{i,m}, i \in \llbracket 1, M \rrbracket
\eeq
...
\beq
\EE \left[ \underset{f \in \mathcal{F}}{\sup}\left|X_{f}-X_{0}\right|\right] \leq \frac{1}{\sqrt{M}} \int_{0}^{1} \sqrt{\log \mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right)} d \varepsilon
\eeq
...
\eSur{k}{\param} \eqdef \Sur{k}{\param}- {\cal L} ( \param ),~~~ \overline{M}_{(k)}= \sum_{k=0}^{K_{\sf max}-1} \Bsize{k}^{-1/2}
\eeq

...
In particular, we provide convergence guarantees of stochastic EM and Variational Inference-type methods, under mild conditions.
...
For $i \in \inter[n]$, the complete data log-likelihood reads:
\beq \notag
...
\begin{split}
\mu_\ell^{(k)} = \frac{1}{n}\sum_{i=1}^{n}{\mu_\ell^{(\tau_{i}^{k})}} -  \frac{\gamma}{n}\sum_{i=1}^{n}{\hat{{\bm{\delta}}}^{(k)}_{\mu_\ell,i} }\eqsp,
\end{split}
\eeq

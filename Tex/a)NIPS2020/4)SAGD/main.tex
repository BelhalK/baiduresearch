...
denotes the $n$ available training samples. $\hat \g_t$ denotes the sample gradient evaluated on $S$ such that $\hat \g_t = \nabla \hat f(\w) = \frac{1}{n}\sum_{j=1}^n \nabla \ell(\w_t, \z_j)$. For a vector $\v$, $\v^2$ represents that $\v$ is element-wise squared.  We use $\v^i$ or $[\v]_i$ to denote the $i$-th coordinate of $\v$ and $\|\v\|_2$ is the $\ell_2$-norm of $\v$. We denote $[d]=\{1,\dots,d\}$.
...
%\citet{dwfe2015a, dwfe2015b, dwfe2015c} designed a practical method named \textsc{Thresholdout}, which can be used to test a large number of hypotheses. 
...
\FOR{$t = 0,...,T-1$}
	...
	\STATE \label{line:dpg} Set $\tilde \g_t = \hat \g_t + \b_t$, where $\b_t^i$ is drawn i.i.d from Lap$(\sigma), \forall\, i \in [d]$.
\STATE  \label{line:adap1}
$\m_t = \tilde \g_t$ and $\v_t = \left(1-\beta_{2}\right) \sum_{i=1}^{t} \beta_{2}^{t-i} \tilde \g_{i}^{2}$.
\STATE  \label{line:adap2} $\w_{t+1}=\w_{t}-\eta_t \m_t /(\sqrt{\v_t}+\nu)$.
\ENDFOR 
...
%Note that although the probability $d\beta + d\exp(-\mu)$ has a dependence on dimension $d$, we can choose appropriate $\beta$ and $\mu$ to make the probability arbitrarily small.  
...
\textsc{SAGD} with \textsc{DPG-Sparse}  (Algorithm~\ref{algo: sparse}) is  
...
%risk bound.   To simplify the results, we use $\rho_{n,d} =\mathcal{O} \left(\ln n + \ln d\right)$.
...
%, beyond trading off between statistical rate and optimization rate. 
\vspace{-0.05in}
...
\includegraphics[width = 0.52 \textwidth]{figure/ptb_3lstm.png}
 ...
 %\vspace{-0.15in}
 \caption[]{Top row: Test loss and accuracy of ResNet-18 andVGG-19 on CIFAR10. 
% For ResNet-18, \textsc{SAGD} achieves the lowest test loss. For VGG-19, \textsc{SAGD} achieves the best test accuracy among all the methods.
 Bottom row:  Train and test perplexity of 2-layer LSTM and 3-layer LSTM. 
...
%Regarding the test accuracy, we can see that when the training sample size is small, \ie $n \in [50, 20000]$, \textsc{SAGD} achieves the best test accuracy among all the algorithms. When the sample size is large, i.e, $n \geq 20000$,  \textsc{SAGD}, SGD, and Adam perform better than the other algorithms.
%We use ResNet-18~\citep{hezh2016} and VGG-19~\citep{sizi2014} for the CIFAR-10 image classification task. We run 100 epochs and decay the learning rate by 0.1 every 30 epochs. 
%Results are reported in Figure~\ref{fig:cifar10}. Figure~\ref{fig:cifar10} (a) shows that \textsc{SAGD} and SGD perform better than the other algorithms in terms of the test loss. Regarding the test accuracy, the \textsc{SAGD}, Adam Padam, and SGD outperform the others. Figure~\ref{fig:cifar10} (b) reports the results on VGG-19. In terms of the test loss, \textsc{SAGD} perform similarly to SGD, and the lowest value is attained by \textsc{SAGD}, SGD and AdaBound when $n \in [20000, 50000]$. Regarding the test accuracy, we can see that when the training sample size is small, \ie $n \in [50, 20000]$, \textsc{SAGD} achieves the best among all the algorithms. When the sample size is large, i.e, $n \geq 20000$,  \textsc{SAGD}, SGD, and Adam perform better than the others.

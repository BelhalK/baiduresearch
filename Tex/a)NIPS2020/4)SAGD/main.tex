...
%whereas it is unrealistic to have infinite samples.
%Conceptually, this SO oracle would be much more well behaved if we had access to infinite samples. 
...
using the ideas from \emph{adaptive data analysis}~\citep{dwfe15} and \emph{differential privacy}~\citep{dwro2014}. The main idea %in SAGD 
...
%It is proven 
%We prove that the training set nearly possesses the statistical nature of fresh samples albeit reused during the iterative process of SAGD.
For large scale learning problems, we extend SAGD to the mini-batch setting as well. 
...
%SAGD can converge to a \emph{population stationary point} with high probability. 
%instead of an empirical 
%sample 
%stationary point 
%as is aimed in analysis by 
%which was the results of
%previous adaptive gradient methods. 
...
\begin{itemize}
    ...
    %, where the training data is used through a differential private mechanism.
...
%Next we consider SAGD with updating rule of RMSprop: $\phi_t(\tilde \g_1,...,\tilde \g_t) = \tilde \g_t$ and $ \v_t = \left(1-\beta_{2}\right) \sum_{i=1}^{t} \beta_{2}^{t-i} \tilde \g_{i}^{2}$. For this case, we present the best vaule of $\sigma$, $T$ for the trade-off between statistical rate and optimization rate which depends on number
...
into $B$ batches with $m$ samples for each batch. At each iteration $t$, Algorithm~\ref{algo: mini-StAda} uses DPG to access one batch to obtain a differential private gradient $\tilde \g_t$ (line 6) and then update $\w_t$ (line 7-8). %Note that the partitioning of data here is conceptual and the way of data pass does not change from SAGD in implementation. However, the performance can be improved by leveraging $m$. 
%The method Algorithm~\ref{algo: mini-StAda} uses mini-batches matches the  practical deep learning training process where analyst use optimization oracle to sequentially pass each mini-batch for $T$ epochs. 
%Note that the total number of parameter updates in mini-batch SAGD is $TB$. 
The theoretical guarantee is given below.

%Then
%Algorithm~\ref{algo: mini-StAda} updates $\w_t$ based on the gradient
%computed on each batch. 

%Mini-batch SGD terminates
%after a single pass over all batches. 
%The
%following theorem analyzes the risk bound of
%mini-batch SGD.
...
%Due to the space limits of the figures, we only present xxx algorithms and for the rest, we included them in the appendix.
...
\includegraphics[width = 0.52 \textwidth,trim={2cm 0 2cm 2cm},clip ]{figure/mnist_relu.png}
}\hspace{-0.1in}
\subfigure[MNIST, 2-layer Sigmoid]{
\includegraphics[width = 0.52 \textwidth, trim={2cm 0 2cm 2cm}, clip]{figure/mnist_sigmoid.png}
 }
 }
%\vspace{-0.15in}
...
%We can see that for ReLU and Sigmoid neural network, SAGD achieves the best test accuracy
...
%SGD performs slightly better than adaptive methods in terms of test loss. Regarding the test accuracy, SAGD display slight improvement over the other algorithms. 
...
%Regarding the test accuracy, the SAGD, Adam Padam, and SGD perform better than the other.
...
%loss, SAGD perform similarly to SGD. SAGD, SGD and AdaBound achieve the lowest test loss when $n \in [20000, 50000]$. 
%Regarding the test accuracy, we can see that when the training sample size is small, i.e., $n \in [50, 20000]$, SAGD achieves the best test accuracy among all the algorithms. When the sample size is large, i.e, $n \geq 20000$,  SAGD, SGD, and Adam perform better than the other algorithms.

%We use ResNet-18~\citep{hezh2016} and VGG-19~\citep{sizi2014} for the CIFAR-10 image classification task. We run 100 epochs and decay the learning rate by 0.1 every 30 epochs. 

%Results are reported in Figure~\ref{fig:cifar10}. Figure~\ref{fig:cifar10} (a) shows that SAGD and SGD perform better than the other algorithms in terms of the test loss. Regarding the test accuracy, the SAGD, Adam Padam, and SGD outperform the others. Figure~\ref{fig:cifar10} (b) reports the results on VGG-19. In terms of the test loss, SAGD perform similarly to SGD, and the lowest value is attained by SAGD, SGD and AdaBound when $n \in [20000, 50000]$. Regarding the test accuracy, we can see that when the training sample size is small, i.e., $n \in [50, 20000]$, SAGD achieves the best among all the algorithms. When the sample size is large, i.e, $n \geq 20000$,  SAGD, SGD, and Adam perform better than the others.
...
%VGG-19 & 92.63 $\pm$ 0.08 & \textbf{92.81 $\pm$ 0.04} & 90.12 $\pm$ 0.11 & 92.52 $\pm$ 0.03 & 90.02 $\pm$0.04 & 92.15 $\pm$0.01 & 92.04 $\pm$ 0.12 \\
%ResNet-18 & 91.48 $\pm$ 0.10  & 91.38 $\pm$ 0.14  & 89.87 $\pm$ 0.31 & 89.94 $\pm$ 0.02 & 62.33 $\pm$0.07 & 62.51 $\pm$0.11  & \textbf{91.98 $\pm$ 0.12} \\ 
%\toprule[1pt]
%\end{tabular}
%\caption{ Test accuracy of VGG-19 and ResNet-18 on CIFAR-10. Bold number indicates the best result.}
%\end{table*}
...
%We hope that it is a good alternative to current adaptive gradient methods.

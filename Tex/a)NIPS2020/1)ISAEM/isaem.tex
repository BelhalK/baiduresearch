...
In this paper, we propose a general class of methods called Two-Time-Scale EM Methods based on double levels of stochastic updates to tackle a growingly common large and nonconvex optimization task for latent data models.
...
Finally, we illustrate the advantages of our method in Section~\ref{sec:numerical} on two numerical experiments.\vspace{-.1cm}
...
&\emph{(\FISAEM\  )} &\StocEstep^{(k+1)} &= \overline{\StocEstep}^{(k)} + \big( \tilde{S}_{i_k}^{(k)}  - \tilde{S}_{i_k}^{(t_{i_k}^k)} \big) \label{eq:fisaem}\\
...
\beq
\tilde{S}_{i_k}^{(k)}=  \frac{1}{M_k} \sum_{m=1}^{M_k} S(z_{i_k,m}^{(k)}, y_{i_k}) \quad \textrm{with} \quad z_{i_k,m}^{(k)}, \sim p(z_{i_k}|y_{i_k};\theta^{(k)})
\eeq
...
\caption{Two-Time-Scale Noisy EM methods.}\label{alg:ttsem}
  ...
  \beq \label{eq:random}
   P( K = k ) = \frac{ \gamma_{k} }{\sum_{\ell=0}^{K_{\sf max}-1} \gamma_\ell} = \frac{\gamma_k}{{\sf P}_{\sf max}}.\vspace{-.2cm} 
  \eeq
  \FOR {$k=0,1,2,\dots, K$}
...
\eta_{i}^{(r)} \eqdef \tilde{S}_{i}^{(r)} -  \overline{\bss}_i(\vartheta^{(r)})\quad  \textrm{for all} \quad  i \in \inter, \, r > 0 \quad \textrm{and} \quad  \vartheta \in \Theta
\eeq
...
\end{Lemma}
The following main result for the \ISAEM\ algorithm, which proof can be found in Appendix~\ref{app:theoremisaem}, is derived under a control of the Monte Carlo fluctuations as described by assumption H~\ref{ass:mcerror}.
Typically, the controls exhibited abover are of interest when the number of MC samples $M_k$ increase with $k$.
...
\beq
\EE[ \| \grd V( \hs{K} ) \|^2 ]  = \frac{1}{{\sf P}_{\sf max}} \sum_{k=0}^{K_{\sf max}-1} \gamma_{k+1} \EE[ \| \grd V( \hs{k} ) \|^2 ] 
\eeq \vspace{-.2cm}
...
Note that in those two bounds, the quantities $\tilde{\eta}^{(k+1)} $ and $ \Xi^{(k+1)} $ depends only on the MC fluctuations $\EE \left[\norm{ \eta_{i_k}^{(k)}}^2 \right] $ and some constants.\vspace{-.2cm}
...
\textbf{Comparison using epochs credit}


\textbf{Comparison using number of training samples credit}


\subsection{PK Model with Absorption Lag Time}
This numerical example was conducted in order to characterize the pharmacokinetics (PK) of orally administered drug to simulated patients, using a population pharmacokinetic approach. $M = 50$ synthetic datasets were generated for $n = 500$ patients with $10$ observations (concentration measures) per patient.
The goal tis to model the evolution of the concentration of the absorbed drug using a nonlinear and latent data model. The fitting of that model is done using our two-time-scale class of algorithms.
...
The model for the $j$-th measured concentration, noted $y_{ij}$, for individual $i$ reads:
\begin{equation}\label{eq:pkmodel2}
...
Note that the {\sf MC-step} \eqref{eq:mcstep} is performed using a Metropolis Hastings procedure since the posterior distribution under the model $\theta$ noted $p(z_i | y_i, \theta)$ is intractable due to the nonlinearity of the model \eqref{eq:pkmodel} (see Appendix~\ref{app:pkmodel} for implementation details).
...
\subsubsection{Explicit Updates}
We recall that the complete model $(y,z)$ defined by \eqref{eq:pkmodel} and \eqref{eq:pkmodel2} belongs to the curved exponential family, which vector of sufficient statistics $S = \big(S_1(z),S_2(z),S_3(z) \big)$ read:
\beq \label{eq:suffstat_deformable}
\begin{split}
S_1(z)  = \frac{1}{n} \sum_{i=1}^n z_i  ,  \quad S_2(z) =\frac{1}{n} \sum_{i=1}^n z_i^\top z_i  , \quad S_3(z)  = \frac{1}{n}  \sum_{i=1}^n  \left(y_i - f(t_{i},z_i)\right)^2
\end{split}

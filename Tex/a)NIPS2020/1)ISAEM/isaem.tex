...
\begin{abstract}
Training latent data models using the Expectation-Maximization (EM) algorithm is the most popular choice for current learning tasks. 
...
Also if $\rho_{k}=1$, $\gamma_{k}=1$ and $\StocEstep^{(k)} = \tilde{S}^{(k)}$, then we recover the Monte Carlo EM algorithm \citep{wei1990monte}.
...
Moreover, for \ISAEM\ we initialize with $\StocEstep^{(0)} = \tilde{S}^{(0)}$; for \SAEMVR\, we set an epoch size of $m$ and define $\ell(k) \eqdef m \lfloor k/m \rfloor$ as the first iteration number in the epoch that iteration $k$ is in.
...
\end{Lemma}
Proof of this Lemma can be found in Appendix~\ref{app:prooflemmainc}.
...
\eeq
\end{Theorem} 
Proof of this Theorem can be found in Appendix~\ref{app:theoremisaem}.
...
\eeq
\end{Theorem} 
Proof of this Theorem can be found in Appendix~\ref{app:theoremvrsaem}.

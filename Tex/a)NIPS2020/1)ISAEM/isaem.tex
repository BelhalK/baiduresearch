...
We defined the mean square distance over the $M$ replicates
\begin{equation}\label{error}
E_k(\ell) = \frac{1}{M}\sum_{m=1}^{M}{\left(\theta_k^{(m)}(\ell) - \theta^* \right)^2}\eqs.
\end{equation}

Note that the MC simulation is performed using a Metropolis Hastings procedure since the posterior distribution under the model $\theta$ noted $p(z_i | y_i, \theta)$ is intractable due to the nonlinearity of the model \eqref{eq:pkmodel}.
Figure~\ref{fig:pk_tts} shows


\begin{figure}[H]
\begin{center}
\includegraphics[width=0.5\textwidth]{pic_paper/pk500.png}\vspace{-.2cm}
\caption{TO COMPLETE}\vspace{-.2cm}
\label{fig:pk_tts}
\end{center}
\end{figure}

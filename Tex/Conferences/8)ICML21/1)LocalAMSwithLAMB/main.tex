...
\begin{algorithm}[H]
\caption{\textsc{L\&D Local AMS for FL}} \label{alg:ldams}
\begin{algorithmic}[1]
%\small
...
\STATE $m_r = \beta_1 m_{r-1} + (1 - \beta_1) g_r$.
\STATE $m_{t}=m_{t} /\left(1-\beta_{1}^{r}\right)$.
\STATE $v_r = \beta_2 v_{r-1} + (1 - \beta_2) g_r^2$.
\STATE $v_r=v_r /\left(1-\beta_{2}^{r}\right)$.
\STATE $\hat{v}_r = \max( \hat{v}_{r-1} , v_r )$.
\STATE Compute ratio  $p_{r}=\frac{m_{r}}{\sqrt{v_{r}}+\epsilon}$.
\STATE Update local model:
$$\theta_{r+1}^{(i)}=\theta_{r}^{(i)}-\alpha_{r} \phi\left(\left\|x_{r}^{(i)}\right\|\right)/\left\|r_{r}^{(i)}+\lambda \theta_{r}^{(i)}\right\|\left(p_{r}^{(i)}+\lambda \theta_{r}^{(i)}\right)$$
\ENDFOR
\end{algorithmic}
\end{algorithm}

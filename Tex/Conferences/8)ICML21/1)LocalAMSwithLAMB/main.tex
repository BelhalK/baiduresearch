...
In the particular case of training Deep multilayer Neural Networks, under such settings, we propose in this paper, \algo, a novel Federated Learning method based on a Layerwise and Dimensionwise updates of the local models. 
...
\STATE Compute ratio  $p_{r,i}=\frac{m^{t}_{r,i}}{\sqrt{v^{t}_{r}}+\epsilon}$. \label{line:scale}
...
\ENDFOR
\STATE Server computes the averages of the local models $\bar{\theta}_r^\ell = \frac{1}{n} \sum_{i=1}^n \theta_{r,i}^{\ell,T}$ and send it back to the devices. \label{line:final}
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
based on classical result for stochastic nonconvex optimization, we characterize the convergence of our algorithm by upper bounding the second order moment of the objective function in \eqref{eq:opt}.
This suboptimality condition becomes smaller as the algorithm progresses and being able to upper bound it describes how fast our method reaches an $\epsilon$-stationary point.
Particularly, in our case, we will focus on the evolution of the following quantity: $\frac{1}{R} \sum_{r=1}^R \EE[\| \nabla f(\overline{\theta_r}) \|^2$ which is the average over a finite time of $R$ rounds of communication, evaluated at the periodically average vector of parameters noted $\overline{\theta_r}  = [\bar{\theta}_r^\ell ]_{\ell =1}^{\tot} $ where $\bar{\theta}_r^\ell $ is defined in Line~\ref{line:final} of Algorithm\ref{alg:ldams}.
...
\begin{figure}[H]
    \begin{center}
        \mbox{
        \includegraphics[width=0.25\textwidth]{figure/mnist_trainloss_cnn_ep1_client60_iid0.eps}
...
\begin{figure}[H]
    \begin{center}
        \mbox{
        \includegraphics[width=0.25\textwidth]{figure/cifar_testerror_cnn_ep1_client10_iid1.eps}
        \includegraphics[width=0.25\textwidth]{figure/cifar_testerror_cnn_ep1_client50_iid1.eps}
        }
        \mbox{
        \includegraphics[width=0.25\textwidth]{figure/cifar_testerror_cnn_ep3_client10_iid1.eps}

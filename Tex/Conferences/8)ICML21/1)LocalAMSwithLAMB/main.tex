...
\STATE Compute ratio  $p^j_{r,i}=\frac{m^{t}_{r,i}}{\sqrt{v^{t}_{r}}+\epsilon}$.
...
\textbf{ Term $A_1$:}
Since we have that $\|p_{r,i}^{\ell}\| \leq \sqrt{\frac{p_\ell}{1-\beta_2}}$ and $\sqrt{v^{t}_{r}} \leq M$, using H\ref{ass:boundgrad}, we develop the term $A_1$ as follows:
\begin{align}
...
\beq
\begin{split}
& -\alpha \phi_m \sqrt{\frac{\tot(1-\beta_2)}{M^2 p}}  \EE[ \| \overline{\nabla f}(\bar{\vartheta})\|^2]  -  \alpha \phi_M \sum_{\ell=1}^\tot \sum_{i = 1}^n \sum_{j=1}^{p_\ell} \frac{\sigma_{i}^{\ell, j}}{\sqrt{n}}  + \frac{\alpha \beta_1}{1-\beta_1}  \sqrt{(1-\beta_2)p} \frac{\tot M^2}{\sqrt{v_0}} \\

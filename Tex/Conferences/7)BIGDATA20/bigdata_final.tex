...
\begin{algorithm}[h]
\SetKwInOut{Input}{input}
\Input{Alg.~\ref{alg:diagnostic} {\tt SGDM}$(\theta, \gamma)$,  initial and minimum stepsize $\gamma_0$, $\gamma_{min}$, learning rate reduction $\rho \in (0,1)$ 
}
$\gamma \gets \gamma_0$ \\
\While{$\gamma > \gamma_{min}$}{
	$\theta \gets {\tt SGDM} ( \theta, \gamma )$ \\
	$\gamma \gets \rho \times \gamma$ \\
}
\Return{ $\theta$}

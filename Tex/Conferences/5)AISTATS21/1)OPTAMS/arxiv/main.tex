...
\begin{abstract}
We propose a new variant of AMSGrad~\citep{Proc:Reddi_ICLR18}, a popular adaptive gradient based optimization algorithm widely used for training deep neural networks. 
...
Deep learning models have been successful in several applications, from robotics (e.g.,~\citep{Article:Levine_JMLR16}), computer vision (e.g.,~\citep{Proc:He_CVPR16,Proc:Goodfellow_NIPS14}), reinforcement learning (e.g.,~\citep{mnih2013playing}) and natural language processing (e.g.,~\citep{Proc:Graves_ICASSP13}).
With the sheer size of modern datasets and the dimension of neural networks, speeding up training is of utmost importance.
To do so, several algorithms have been proposed in recent years, such as  \textsc{AMSGrad}~\citep{Proc:Reddi_ICLR18}, \textsc{Adam}~\citep{Proc:Kingma_ICLR15}, \textsc{RMSPROP}~\citep{Tieleman2012}, \textsc{AdADELTA}~\citep{zeiler2012adadelta},  \textsc{NADAM}~\citep{dozat2016incorporating}, 
\textsc{Local Adam}~\citep{Proc:Chen_FODS20}, \textsc{SAGD}~\citep{Proc:Zhou_NeurIPS20}, 
etc. 
All the prevalent algorithms for training deep networks mentioned above combine two ideas: the idea of adaptivity from \textsc{AdaGrad}~\citep{Proc:McMahan_COLT10,Proc:Duchi_JMLR11} and the idea of momentum from \textsc{Nesterov's Method}~\citep{Book:Nesterov_2004} or \textsc{Heavy ball} method~\citep{Article:Polyak_1964}.
...
On the other hand, \textsc{Nesterov's Method} or \textsc{Heavy ball} Method~\citep{Article:Polyak_1964} is an accelerated optimization algorithm which update not only depends on the current iterate and gradient but also depends on the past gradients (i.e., momentum). 
State-of-the-art algorithms such as \textsc{AMSGrad}~\citep{Proc:Reddi_ICLR18} and \textsc{Adam}~\citep{Proc:Kingma_ICLR15} leverage these ideas to accelerate the training of nonconvex objective functions, for instance deep neural networks losses.

In this paper, we propose an algorithm that goes beyond the hybrid of the adaptivity and momentum approach. 
Our algorithm is inspired by \textsc{Optimistic Online learning}~\citep{Proc:Chiang_COLT12,Proc:Rakhlin_NIPS13,Proc:Syrgkanis_NIPS15,Proc:Abernethy_COLT18,Proc:Mertikopoulos_ICLR19}, which assumes that, in each round of online learning, a \emph{predictable process} of the gradient of the loss function is available. 
...
\cite{Proc:Daskalakis_ICLR18} develop \textsc{Optimistic-Adam} leveraging optimistic online mirror descent~\citep{Proc:Rakhlin_NIPS13}.
Yet, \textsc{Optimistic-Adam} is specifically designed to optimize two-player games, e.g., GANs~\citep{Proc:Goodfellow_NIPS14} which is in particular a two-player zero-sum game. 
There have been some related works in \textsc{Optimistic Online learning}~\citep{Proc:Chiang_COLT12,Proc:Rakhlin_NIPS13,Proc:Syrgkanis_NIPS15} showing that if both players use an $\textsc{Optimistic}$ type of update, then accelerating the convergence to the equilibrium of the game is possible.
...
\textbf{Notations:} 
We follow the notations of adaptive optimization~\citep{Proc:Kingma_ICLR15,Proc:Reddi_ICLR18}. 
...
The idea of \textsc{Optimistic Online learning} (e.g.,~\citep{Proc:Chiang_COLT12,Proc:Rakhlin_NIPS13,Proc:Syrgkanis_NIPS15,Proc:Abernethy_COLT18}) is as follows.
...
Consider the \textsc{Follow-the-Regularized-Leader} (\textsc{FTRL},~\citep{hazan2019introduction}) online learning algorithm which update reads
\begin{equation} \notag
...
The update of its optimistic variant, called \textsc{Optimistic-FTRL} and developed in~\citep{Proc:Syrgkanis_NIPS15} reads
\begin{equation}\notag
...
Under the assumption that the loss functions are convex, it has been shown in~\citep{Proc:Syrgkanis_NIPS15} that the regret of \textsc{Optimistic-FTRL} is at most $\mathcal{O}(\sqrt{\sum_{t=1}^T \| g_t - m_t \|_*^2 } )$.

\vspace{0.1in}

\textit{Remark:} Note that the usual worst-case bound is preserved even when the predictors $\{m_{t}\}_{t>0}$ do not predict well the gradients. Indeed, if we take the example of \textsc{Optimistic-FTRL}, the bound reads $\sqrt{\sum_{t=1}^T \| g_t - m_t \|_*^2 } \leq 2 \max \limits_{w \in \Theta} \| \nabla \ell_t(w) \| \sqrt{T}$ which is equal to the usual bound up to a factor $2$~\citep{Proc:Rakhlin_NIPS13}, under certain boundedness assumptions on $\Theta$ detailed below.
...
\textsc{Adam}~\citep{Proc:Kingma_ICLR15}, a popular adaptive algorithm, combines momentum~\citep{Article:Polyak_1964} and anisotropic learning rate of \textsc{AdaGrad}~\citep{Proc:Duchi_JMLR11}.
...
This adaptive learning rate helps accelerating the convergence when the gradient vector is sparse~\citep{Proc:Duchi_JMLR11}, yet, when applying \textsc{AdaGrad} to train deep neural networks, it is observed that the learning rate might decay too fast, see~\citep{Proc:Kingma_ICLR15} for more details.
...
A variant, called \textsc{AMSGrad} and detailed in Algorithm~\ref{alg:amsgrad}, has been developed in~\citep{Proc:Reddi_ICLR18} to fix \textsc{Adam} failures.

\noindent{\centering
\begin{minipage}{.5\linewidth}
\begin{algorithm}[H]
\caption{\textsc{AMSGrad}~\citep{Proc:Reddi_ICLR18}} \label{alg:amsgrad}
\begin{algorithmic}[1]
%\small
...
The \textsc{AMSGrad} algorithm~\citep{Proc:Reddi_ICLR18} applies the $\texttt{max}$ operation on the second moment to guarantee a non-increasing learning rate $\eta_t / \sqrt{\hat{v}}_t $, which helps for the convergence (i.e., average regret $\mathcal{R}_T/T \rightarrow 0$).
...
Compared to \textsc{AMSGrad}, the algorithm is characterized by a \emph{two-level} update that interlinks some \emph{auxiliary state} $\tilde{w}_{t}$ and the model parameter state, $w_t$, similarly to the \textsc{Optimistic Mirror Descent} algorithm developed in~\citep{Proc:Rakhlin_NIPS13}.
...
Such two-levels method where the guess $m_t$ is equal to the last known gradient $g_{t-1}$ has been exhibited recently in~\citep{Proc:Chiang_COLT12}.
The gradient prediction process plays an important role as discussed in Section~\ref{sec:numerical}.
The proposed \textsc{OPT-AMSGrad} algorithm inherits three properties: \textsf{(i)} Adaptive learning rate of each dimension as \textsc{AdaGrad}~\citep{Proc:Duchi_JMLR11} (line 6, line 8 and line 9). \textsf{(ii)} Exponential moving average of the past gradients as \textsc{Nesterov's method}~\citep{Book:Nesterov_2004} and the \textsc{Heavy-Ball} method~\citep{Article:Polyak_1964} (line 5). \textsf{(iii)} Optimistic update that exploits \emph{prior knowledge} of the next gradient vector as in optimistic online learning algorithms~\citep{Proc:Chiang_COLT12,Proc:Rakhlin_NIPS13,Proc:Syrgkanis_NIPS15} (line 9).
...
In this section, we provide regret analysis of the proposed method and show that it can improve the bound of vanilla \textsc{AMSGrad} with a good guess of gradient.
The convex analysis is followed by a global non-asymptotic bound for nonconvex objective functions where the optimization problem tackled in this paper is casted as an offline and stochastic nonconvex optimization problem.
...
In the following, we assume convexity of $\{\ell_t\}_{t>0}$ and that $\Theta$ has a bounded diameter $D_{\infty}$, which is a standard assumption for adaptive methods~\citep{Proc:Reddi_ICLR18,Proc:Kingma_ICLR15} and is necessary in regret analysis.
...
\end{Corollary}
We can compare the bound of Corollary~\ref{cor:corollary} with that of \textsc{AMSGrad}~\citep{Proc:Reddi_ICLR18} with $\eta_t = \eta/\sqrt{t}$:
...
The construction of the predictions $\{m_t\}_{t>0}$ is thus of utmost importance for achieving optimal acceleration and can be learned through the iterations~\citep{Proc:Rakhlin_NIPS13}.
...
The random termination number \eqref{eq:random} is inspired by~\citep{Article:Ghadimi_SJOPT13} and is widely used to derive novel results in nonconvex optimization. 
...
\end{Theorem}
Firstly, the bound for our OPT-AMSGrad method matches the complexity bound of $\mathcal{O}( \sqrt{d/T_{\sf M}} + 1/T_{\sf M} )$ of~\citep{Article:Ghadimi_SJOPT13} for SGD considering the dependence of T only, and of~\citep{zhou2018convergence} for AMSGrad method.To see the influence of prediction quality, we can show that when $(1-\beta_1)(\beta_2-\beta_1^2-2L(1-\beta_1))-\frac{4L\beta_1^2(1+\beta_1^2)}{1-\gamma}<0$, $\tilde C_1$ and $\tilde C_2$ both decrease as $a_m$ approaches 1, i.e., as the prediction gets more accurate. Therefore, similar to the convex case, our nonconvex bound also improves with better gradient prediction.
...
Given the multilayer model \eqref{eq:dnnmodel}, assume the boundedness of the input data and of the loss function, \ie for any $\xi \in \rset^p$ and $y \in \rset$ there is a constant $T >0$ such that $\norm{\xi} \leq 1 \quad \textrm{a.s.}$ and $|\mathcal{L}'(\cdot, y)| \leq T$ where $\mathcal{L}'(\cdot, y)$ denotes its derivative \wrt the parameter. Then for each layer $\ell \in [1,L]$, there exists a constant $A_{(\ell)}$ such that $\norm{w^{(\ell)}} \leq A_{(\ell)}$
\end{Lemma}


\section{Comparison to related methods} \label{sec:related}
We give in this section some comparable methods to our \textsc{OPT-AMSGrad} algorithm such as AO-FTRL~\citep{Proc:Mohri_AISTATS16} or \textsc{Optimistic-Adam}~\citep{Proc:Daskalakis_ICLR18}.

 \textbf{Comparison to nonconvex optimization works.}\hspace{0.1in}
Recently,~\citep{Proc:Zaheer_NeurIPS18,Proc:Chen_ICLR19,Proc:Ward_ICML19,zhou2018convergence,zou2018convergence,Proc:Li_AISTATS19} provide some theoretical analysis of \textsc{Adam}-type algorithms when applying them to smooth nonconvex optimization problems. 
For example,~\citep{Proc:Chen_ICLR19} provide the following bound $\min_{t \in [T]} \mathbb{E}[\| \nabla f(w_t) \|^2 ] = \mathcal{O}(\log T / \sqrt{T}) $.
...
To get some adaptive data dependent bound written in terms of the gradient norms observed along the trajectory when applying  \textsc{OPT-AMSGrad} to nonconvex optimization, one can follow the approach of~\citep{Proc:Agarwal_ICML19} or~\citep{Proc:Chen_Yuan_ICLR19}.
...
\vspace{0.1in}
\textbf{Comparison to AO-FTRL~\citep{Proc:Mohri_AISTATS16}.}\hspace{0.1in}In~\citep{Proc:Mohri_AISTATS16}, the authors propose \textsc{AO-FTRL}, which update reads $w_{t+1} = \arg\min_{{w \in \Theta}} ( \sum_{s=1}^t g_s )^{\top}  w + m_{t+1}^\top w + r_{0:t}(w) $, where $r_{0:t}(\cdot)$ is a 1-strongly convex loss function with respect to some norm $\| \cdot\|_{(t)}$ that may be different for different iteration $t$. 
Data dependent regret bound provided in~\citep{Proc:Mohri_AISTATS16} reads $r_{{0:T}}(w^*) + \sum_{t=1}^T \| g_t - m_t \|_{(t)^*}$ for any benchmark $w^{*} \in \Theta$. 
...
However, no experiments were provided in~\citep{Proc:Mohri_AISTATS16} to back those findings.




\vspace{0.1in}
\textbf{Comparison to \textsc{Optimistic-Adam}~\citep{Proc:Daskalakis_ICLR18}.} This is an optimistic variant of ADAM, namely \textsc{Optimistic-Adam}. 
...
\textsc{Optimistic-Adam} is designed to optimize two-player games (e.g., GANs~\citep{Proc:Goodfellow_NIPS14}), while our proposed algorithm \textsc{OPT-AMSGrad} is designed to accelerate optimization (e.g., solving empirical risk minimization).
\citep{Proc:Daskalakis_ICLR18} focuses on training GANs~\citep{Proc:Goodfellow_NIPS14} as a two-player zero-sum game. 
\citep{Proc:Daskalakis_ICLR18} was inspired by these related works and showed that \textsc{Optimistic-Mirror-Descent} can avoid the cycle behavior in a bilinear zero-sum game, which accelerates the convergence. 
...
\begin{algorithm}[H]
\begin{algorithmic}[1]
\caption{\textsc{Optimistic-Adam~\citep{Proc:Daskalakis_ICLR18}+$\hat{v}_t$}. \label{OPT-DISZ}}
\STATE \textbf{Required}: parameter $\beta_1$, $\beta_2$, and $\eta_t$.
...
Some classical works in gradient prediction methods include \textsc{Anderson} acceleration~\citep{Article:Walker_SJNA11}, \textsc{Minimal Polynomial Extrapolation}~\citep{cabay1976polynomial} and  \textsc{Reduced Rank Extrapolation}~\citep{eddy1979extrapolating}.
...
\end{equation}
where $e_t$ is a second order term satisfying $\| e_t \|_2  = \mathcal{O}( \| g_{t-1} - g^* \|_2^2)$, see~\citep{Article:Scieur_MP20} for details and results.
...
\begin{algorithm}[H]
\begin{algorithmic}[1] 
\caption{Regularized Approximated Minimal Polynomial Extrapolation~\citep{Article:Scieur_MP20} } \label{alg:algex}
...
%
%Now let us consider another problem: an online learning problem proposed in~\citep{Proc:Reddi_ICLR18}
%\footnote{\citep{Proc:Reddi_ICLR18} uses this example to show that \textsc{Adam}~\citep{Proc:Kingma_ICLR15} fails to converge.}.
...
The hyper-parameters are set to be $\beta_1 = 0.9$ and $\beta_2 = 0.999$, see~\citep{Proc:Reddi_ICLR18}. 
The other benchmark method is the \textsc{Optimistic-Adam$+\hat{v}_t$}~\citep{Proc:Daskalakis_ICLR18}, which described Algorithm~\ref{OPT-DISZ}. 
...
For convolutional networks, we adopt a simple four layer CNN which has 2 convolutional layers following by a fully connected layer. In addition, we also apply residual networks, Resnet-18 and Resnet-50~\citep{Proc:He_CVPR16}, which have achieved state-of-the-art results.
For the texture \textit{IMDB} dataset, we consider a Long-Short Term Memory (LSTM) network~\citep{Article:Gers_NC00}.
...
Following~\citep{Proc:Yan_IJCAI18} and their study of the SGD with Momentum we denote for any $t >0$:
...
\eeq
where $\mathcal{L}'(\cdot, y)$ denotes its derivative \wrt the parameter. Then for each layer $\ell \in [1,L]$, there exists a constant $A_{(\ell)}$ such that:
$$

...
In classical stochastic nonconvex optimization, adaptive methods have proven to be the spearhead in many applications.
...
Before our investigation of the theoretical behavior of our method, in the sequel, we recall few important notations used throughout our paper.
\begin{table}[htbp]
...
\textbf{LAMB bound in \citep{you2019large}: }
...
\textbf{Local-AMS bound in \citep{chen2020toward}: }
...
\begin{split}
& \frac{1}{R}\sum_{r=1}^R  \EE\left[ \left\| \frac{\nabla f(\overline{\theta_r})}{\sqrt{ v_r^t}}   \right \|^2 \right]  \\
&   \leq  8 \sqrt{\frac{p}{Rn}} \left[ f(\bar{\vartheta}_1)  - \EE[ f(\bar{\vartheta}_{R+1})] \right]+    8 L_{s} \sqrt{\frac{p}{Rn}}  \frac{  \sigma^2}{\epsilon} + cst.
 \end{split}
\eeq
...
Multiple edges of periodic averaging, adaptive optimization methods and layerwise updates are displayed in our bounds.
...
\beq 
\frac{1}{R} \EE\left[ \left\| \nabla f(\overline{\theta_r})   \right \|^2 \right] \leq \mathcal{O}\left(\sqrt{\frac{M^2 p}{ n}} \frac{1}{\tot R } \right)
\eeq
\end{Corollary*}

\begin{proof}
From the bound in Theorem~\ref{th:main} and with assumption \textbf{H\ref{ass:boundv}} we obtain:
\end{proof}

\subsection{Multiple local updates}

In previous proof, we change all index $r$ to iteration $t$. We can write \eqref{eqn1} as
\begin{align}
...
\begin{align}
    \langle \nabla f(\bar \vartheta_t),\frac{\bar g_t}{\sqrt{\hat v_t}} \rangle&=\frac{1}{2}\Vert \frac{\nabla f(\bar\vartheta_t) }{\hat v^{1/4}}\Vert^2+\frac{1}{2}\Vert \frac{\nabla f(\bar g_t) }{\hat v^{1/4}}\Vert^2-\Vert \frac{\nabla f(\bar\vartheta_t)-\bar g_t }{\hat v^{1/4}}\Vert^2.
\end{align}
To bound the last term on the RHS, we have
\begin{align}
    \Vert \frac{\nabla f(\bar\vartheta_t)-\bar g_t }{\hat v^{1/4}}\Vert^2&=\Vert \frac{\frac{1}{n}\sum_{i=1}^n (\nabla f_i(\bar\vartheta_t)-\bar g_{t,i})}{\hat v^{1/4}} \Vert^2\\
    &\leq \frac{1}{n}\sum_{i=1}^n\Vert \frac{\nabla f_i(\bar\vartheta_t)-\bar g_{t,i}}{\hat v^{1/4}} \Vert^2\\
    &\leq \frac{2}{n}\sum_{i=1}^n \Big(\Vert \frac{\nabla f_i(\bar\vartheta_t)-\nabla f_i(\bar\theta_t)}{\hat v^{1/4}} \Vert^2+\Vert \frac{\nabla f_i(\bar\vartheta_t)-\bar g_{t,i}}{\hat v^{1/4}} \Vert^2  \Big).
\end{align}
By Lipschitz smoothness of the loss function, the first term admits
\begin{align}
    \frac{2}{n}\sum_{i=1}^n\Vert \frac{\nabla f_i(\bar\vartheta_t)-\nabla f_i(\bar\theta_t)}{\hat v^{1/4}} \Vert^2&\leq \frac{2}{n \sqrt{v_0}}\sum_{i=1}^n L\Vert \bar\vartheta_t-\bar\theta_t\Vert^2  \\
    &=\frac{2L_\ell}{n \sqrt{v_0}}\frac{\beta_1^2}{(1-\beta_1)^2}\sum_{i=1}^n \Vert \bar\theta_t-\bar\theta_{t-1}\Vert ^2\\
    &\leq \frac{2L_\ell }{n \sqrt{v_0}}\frac{\beta_1^2}{(1-\beta_1)^2} \sum_{i=1}^n \sum_{l=1}^L\Vert \frac{\phi(\Vert \theta_{t,i}^l\Vert)}{\Vert p_{t,i}^l\Vert}p_{t,i}^l \Vert^2\\
    &\leq \frac{2L_\ell LM^2}{ \sqrt{v_0}}\frac{\beta_1^2}{(1-\beta_1)^2}.
\end{align}
For the second term,

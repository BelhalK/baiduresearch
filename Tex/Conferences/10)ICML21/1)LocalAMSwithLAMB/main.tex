...
Assume \textbf{H\ref{ass:smooth}-H\ref{ass:phi}}. Consider $\{\overline{\theta_r}\}_{r>0}$, the sequence of parameters obtained running Algorithm~\ref{alg:ldams}. Then, if the number of local epochs is set to $T=1$ and $\epsilon = \lambda = 0$, we have:
\beq \label{bound1}
\begin{split}
...
\textcolor{red}{add some interpretation of the bound}
...
\textcolor{red}{Continue comparison}



In light of the prior remarks, we now give a simple variant of Theorem~\ref{th:main} in the following Corollary where we assume an upperbound on the nomr of the second moment approximate $\sqrt{ v_r^t}$:
...
\beq \label{bound1}
\frac{1}{R} \EE\left[ \left\| \nabla f(\overline{\theta_r})   \right \|^2 \right] \leq \mathcal{O}\left(\sqrt{\frac{ p}{ n}} \frac{1}{\tot R } \right)
\eeq
\end{Corollary}
...
\noindent\textbf{Settings.} In our experiment, we will evaluate three federated learning algorithms:

\noindent\textbf{Models.} We test the performance of different federated learning algorithms on MNIST
...
\beq
\begin{split}
   \frac{1}{R}\sum_{r=1}^R  \EE\left[ \left\| \frac{\nabla f(\overline{\theta_r})}{\sqrt{ v_r^t}}   \right \|^2 \right] & \leq  \sqrt{\frac{M^2 p}{n}} \frac{ f(\bar{\vartheta}_1)  - \EE[ f(\bar{\vartheta}_{R+1})]}{\tot R}+   \frac{1}{n^2}  \sum_{r=1}^R  \sum_{i = 1}^n  \sigma_i^2 \EE\left[ \left\|\frac{\phi(\|\theta_{r,i}^{\ell}\|)}{\sqrt{v^{t}_{r}} \|p_{r,i}^{\ell}\|} \right\|^2 \right]\\

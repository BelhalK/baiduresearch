...
\emph{Bounded variance:} For the given $m<d$, $t=\mathcal{O}\left(\ln \left(\frac{d}{\delta}\right)\right)$ \textcolor{blue}{with probability $1-\delta$ we have:}
    ...
    \end{align}
{\color{blue} where $e < c \leq 3$ is a positive constant.}
...
In contrast, without any extra assumptions, our results in Theorem~\ref{thm:hetreg_case} for \texttt{PRIVIX} and \texttt{HEAPRIX} are respectively $BR=O(\frac{(d+m)}{\epsilon}\log(\frac{{(\frac{d}{m})}^2+d}{\epsilon\delta}\log(\frac{1}{\epsilon})))$ and $BR=O(\frac{d}{\epsilon}\log(\frac{d^2}{\epsilon m^2\delta}\log(\frac{1}{\epsilon})))$ which improves the total communication cost of Theorem 1 in~\citep{rothchild2020fetchsgd} under regimes such that $\frac{1}{\epsilon}\geq d$ or $d>>m$. 
...
We also provide bounds for PL, convex and non-convex objectives unlike~\citep{rothchild2020fetchsgd}. Finally, we note that algorithms in~\citep{rothchild2020fetchsgd} are using momentum at sever. While we do not use momentum at the server explicitly, we note that we can modify our algorithms to easily add momentum.}
...
Further comparison can be found in Section~\ref{sec:gen-proof} in Appendix.} 

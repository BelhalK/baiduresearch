...
\emph{Unbiased estimation}: As in~\citet{li2019privacy}, we have:
    \begin{align}\notag
...
We note that $\Delta=1+c\frac{d}{m}$ implies that if $m\rightarrow d$, then $\Delta\rightarrow 1+c$, indicating a noisy reconstruction. Exploiting this noisy reconstruction, \citet{li2019privacy} show that if the data is normally distributed, \texttt{PRIVIX} is differential private~\citep{DBLP:conf/icalp/Dwork06}, up to additional assumptions and algorithmic design.  }
...
In \texttt{FedSKETCH}, the number of local updates, between two consecutive communication rounds, at device $j$ is denoted by $\tau$.
 unlike~\citet{haddadpour2020federated}, server node does not store any global model, instead device $j$ has two models, $\boldsymbol{x}^{(r)}$ and $\boldsymbol{x}^{(\ell,r)}_j$, respectively local and global models. 
...
This algorithm leverages the idea of gradient tracking applied in~\citet{haddadpour2020federated} (with compression) and a special case of $\gamma=1$ without compression~\citep{liang2019variance}. 
...
\end{remark}
{\color{blue} We note that in both \texttt{FedSKETCH} and \texttt{FedSKETCHGATE} algorithms, unlike \texttt{PRIVIX}, \texttt{HEAPRIX} variant requires a second round of communication. 
...
It is shown in~\citet{karimi2016linear} that PL condition implies strong convexity property with same module (PL objectives can also be non-convex, hence strong convexity does not imply PL condition necessarily).
...
We note that while reducing communication cost, our scheme requires {\color{blue}$\tau=O(d/m(k(\frac{ d}{mk}+1)\epsilon))>1$}, while it scales down with the number of sampled devices, $k$. 
...
\vspace{0.05in}\paragraph{Comparison  with~\citet{haddadpour2020federated}}\label{rmrk:bidirect}
...
Further comparison can be found in the Appendix.} 
...
Note that in Algorithm~\ref{Alg:PFLHom}, FS-PRIVIX with global learning rate $\gamma=1$ is equivalent to the DiffSketch algorithm proposed in~\citet{li2018federated}. 
...
The slow convergence of federated SGD in non-i.i.d. data distribution case has also been reported in~\citet{mcmahan2016communication,chen2020toward}. 
...
\label{table:1}
    ...
    \makecell{\textbf{Theorem~\ref{thm:homog_case}}} %& \makecell[l]{$\boldsymbol{R=O\left(\frac{1}{\epsilon}\right)}$ \\[3pt] $\boldsymbol{\tau=O\left(\frac{\mu^2d}{k\epsilon}\right)}$\\[3pt]
       ...
       $\boldsymbol{kBR=O\left({m}\kappa(d-m+mk)\log\frac{1}{\epsilon}\log\left(\frac{\kappa(d\frac{d-m}{mk}+d)\log\frac{1}{\epsilon}}{\delta}\right)\right)}$}                                                                                   & \makecell{\ding{52}} & \makecell{\ding{52}}   \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
%}                                                                            & \makecell{\ding{52}} & \makecell{\ding{52}}
   \\
       ...
       $\boldsymbol{BR=O\left(\frac{d}{\epsilon}\log\left(\frac{d^2}{m\epsilon\delta}\log\left(\frac{1}{\epsilon}\right)\right)\right)}$}   & \makecell[l]{$\boldsymbol{R\!=\!O\left(\frac{d}{m\epsilon}{\color{black}\log\left(\frac{1}{\epsilon}\right)}\right)}$\\[3pt]
       $\boldsymbol{\tau\!=\!O\left(\frac{1}{p\epsilon^2}\right)}$\\[3pt]
       $\boldsymbol{B=O\left(\log\left(\frac{d^2}{m\epsilon\delta}\right)\right)}$}                                                                            & \makecell{\ding{52}} & \makecell{\ding{52}}
   \\
        \bottomrule
    \end{tabular}
    }

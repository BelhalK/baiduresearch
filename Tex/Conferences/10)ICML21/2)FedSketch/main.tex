...
Since locally generated data in each device may come from different distribution, local computations involved in Federated Learning setting can lead to poor convergence error in practice~\citep{li2019federated,liang2019variance}. 
...
\emph{Unbiased estimation}: As in~\citep{li2019privacy}, we have:
    ...
    \end{align}
\emph{Bounded variance:} if $m=\mathcal{O}\left(\frac{e}{\mu^2}\right)$, $t=\mathcal{O}\left(\ln \left(\frac{d}{\delta}\right)\right)$ \textcolor{blue}{ then , with probability $1-\delta$ we have:}
    \begin{align}\notag
        \mathbb{E}_{\mathbf{S}}\left[\left\|\texttt{PRIVIX}\left[\mathbf{S}\left(\mathbf{x}\right)\right]-\mathbf{x}\right\|_2^2\right]\leq \mu^2 d\left\|\mathbf{x}\right\|_2^2{\color{blue}\leq \frac{d}{m}\left\|\mathbf{x}\right\|_2^2}\ , 
    \end{align}
{\color{blue} where $2\geq c$ is a positive constant.}

{\color{red} you mean  $ 0 < c \leq 2 $?}

{\color{red} What is $\mu$? it is not defined prior}
...
\STATE \textbf{Output:}
$\texttt{HEAVYMIX}\left[\mathbf{S}\left(\mathbf{x}\right)\right]+\texttt{PRIVIX}\left[\mathbf{S}\left(\mathbf{x}-\texttt{HEAVYMIX}\left[\mathbf{S}\left(\mathbf{x}\right)\right]\right)\right]$
%\vspace{- 0.1cm}
\end{algorithmic}
\end{algorithm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\paragraph{Comparison  with~\citep{haddadpour2020federated}}\label{rmrk:bidirect}
...
\vspace{0.05in}\noindent\textbf{Comparison with~\citep{haddadpour2020federated}}\textcolor{blue}{TBA...}


\vspace{0.05in}\noindent\textbf{Comparison with~\citep{karimireddy2019scaffold}}\textcolor{blue}{TBA...}
 
 
\vspace{0.05in}\noindent\textbf{Comparison with~\citep{basu2019qsparse}}\textcolor{blue}{TBA...}

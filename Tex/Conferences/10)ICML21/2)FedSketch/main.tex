...
In this paper, we exploit commonly used \texttt{Count Sketch} ~\citep{DBLP:journals/tcs/CharikarCF04} which uses two sets of functions that encode any input vector $\boldsymbol{x}$ \textbf{into a hash table} $\boldsymbol{S}_{m\times t}(\boldsymbol{x})$. 
...
\textit{Unbiased estimation}: As in~\citet{li2019privacy}, we have:
    ...
    \end{align}
\textit{Bounded variance:} For the given $m<d$, $t=\mathcal{O}\left(\ln \left(\frac{d}{\delta}\right)\right)$ \textcolor{blue}{with probability $1-\delta$ we have:}
    \begin{align}\notag
        \mathbb{E}_{\mathbf{S}}\left[\left\|\texttt{PRIVIX}\left[\mathbf{S}\left(\boldsymbol{x}\right)\right]-\boldsymbol{x}\right\|_2^2\right] {\color{blue}\leq c\frac{d}{m}\left\|\boldsymbol{x}\right\|_2^2}\ , 
    \end{align}
...
Note that $\Delta=1+c\frac{d}{m}$ implies that if $m\rightarrow d$, then $\Delta\rightarrow 1+c$, indicating a noisy reconstruction. 
...
\STATE \textbf{Approximate $\mathbf{S}(\boldsymbol{x})$ using \texttt{HEAVYMIX} }
\STATE \textbf{Approximate $\mathbf{S}\left(x - \texttt{HEAVYMIX}[\mathbf{S}(\boldsymbol{x})]\right)$ using \texttt{PRIVIX} }
\STATE \textbf{Output:}
...
\end{algorithm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We note if $m\rightarrow d$, then $C(\boldsymbol{x})\rightarrow \boldsymbol{x}$, meaning that the algorithm convergence can be improved by decreasing the size of compression $m$. 
\begin{corollary}\label{cor:small}
Based on~Theorem 3 of~\citep{horvath2020better}, \texttt{HEAPRIX} in Algorithm~\ref{alg:heaprix} satisfies \textcolor{blue}{$C(\boldsymbol{x})\in \mathbb{U}(c \frac{d}{m})$}. 
\end{corollary}
...
{\color{blue}The bounds in Theorem~\ref{thm:homog_case} suggest that in homogeneous setting if we set $d=m$ (no compression), the number of communication rounds to achieve the $\epsilon$ error matches with the number of iterations required to achieve the same error under a centralized setting. 
...
We note that while reducing communication cost, our scheme requires {\color{blue}$\tau=O(d/m(k(\frac{ d}{mk}+1)\epsilon))>1$}, which scales down with the number of sampled devices, $k$. 
...
{\color{blue}Before comparing with prior works, we highlight that privacy is another purpose of using unbiased sketching in addition to communication efficiency. 
Therefore, our main competing schemes are distributed algorithms based on sketching. 
Yet, for the sake of showing the effectiveness of our algorithms, we also compare with prior non-sketching based distributed algorithms (\cite{karimireddy2019scaffold,basu2019qsparse,haddadpour2020federated}) in Section~\ref{app:comparison} of Appendix.}
...
Under that assumption, the total communication cost is shown to be $BR=O\left(\frac{m\max(I^{2/3},2-\alpha)}{\epsilon^3\alpha}\log\left(\frac{d\max(I^{2/3},2-\alpha)}{\epsilon^3\delta}\right)\right)$ where $I$ is a constant related to the window of gradients.
Our result improves this bound under weaker assumptions in a regime where $\frac{I^{2/3}}{\epsilon^2}\geq d$. 
...
While we do not use it explicitly, we note that we can modify our algorithms to include momentum easily.}
...
\textbf{Homogeneous case.} In Figure~\ref{fig:MNIST-iid1}, we provide the training loss and test accuracy with different number of local EP and sketch size, $(t,k)=(20,40)$ and $(50,100)$. 
...
Nevertheless, when the sketch size is not too small, i.e. $(50,100)$, \texttt{FS-HEAPRIX} can still provide comparable test accuracy as \texttt{FedSGD} with multiple local epochs.

...
%%%%%%%%% ABSTRACT
\begin{abstract}
We propose in this paper, \algo, a \textbf{S}tochastic gradien\textbf{T} \textbf{AN}isotropic \textbf{L}ang\textbf{E}vin d\textbf{Y}namics, for sampling high dimensional data.
...
\textcolor{blue}{Recently, EBMs parameterized by modern neural networks have drawn much attention from the computer vision and machine learning communities. Successful applications with EBMs include generations of images \cite{xie2016theory,gao2018learning,xie2018cooperative,du2019implicit}, videos \cite{XieCVPR17,xie2019learning}, 3D volumetric shapes \cite{xie2018learning,xie2020generative}, unordered point clouds \cite{xie2021GPointNet}, 
texts \cite{deng2020residual}, 
molecules, etc., \cite{ingraham2018learning,du2019energy}, as well as image-to-image translation \cite{xie2021cooperative,xie2021cycleCoopNets}, 
% unpaired cross-domain image translation \citep{xie2021cycleCoopNets}, 
out-of-distribution detection \cite{ebmood2020} and inverse optimal control \cite{xu2019energy}.}
...
Yet, in both of our main results, the ergodicity and the convergence rate, including the underlying drift condition, are established uniformly on the parameter space $\Theta$.
...
Assume H\ref{ass:bounded}-H\ref{ass:V2}.
A direct consequence of Theorem~\ref{thm:thm1} is that the family of transition kernels $(\Pi_\theta)_{\theta \in \Theta}$ are uniformly ergodic,\ie\ for any compact $\mathcal{C} \subset \zset$, there exist constants $\rho \in ]0,1[$ and $e >0$ such for any iteration $k > 0$,we have:
\beq\label{coro:main}
\sup \limits_{z \in \mathcal{C}} \| \Pi_\theta^k f(\cdot) - \pi_\theta f(\cdot) \|_{V} \leq e \rho^k \| f \|_{V_\theta} \eqsp,
\eeq
...
\eeq
where $\sigma_1$ and $\sigma_2$ are the corresponding standard deviations of the two Gaussian distributions $n_{\sigma_1}$ and $n_{\sigma_2}$. 
...
%\end{center}
%	\caption{(CIFAR-10 Dataset). FID values per method through 100k iterations}
%	\label{fig:cifarfid}
%\end{figure}

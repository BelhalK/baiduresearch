...
The transition kernel from state $z$ to $z'$ is denoted by $\Pi_\theta(z, z')$.
...
& + \mathsf{1}_{\bset(z)}\int_{\zset} (1 - \alpha_\theta(z, y)) \prop{\theta}(z,y) \textrm{d}y
\end{split}
\eeq
...
& \geq \textrm{min}(1, \delta) m \int_{\bset} \mathsf{1}_\xset(z)  \textrm{d}y
\end{split}
\eeq

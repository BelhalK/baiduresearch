...
Let $\mathbf{A}=(a_{ij})$ be a given $n\times n$ matrix of which the entries are non-negative.
...
For any $n$ and $p$ with $n\geq p\geq 4$, there exist $\alpha \in (1,2)$, $\delta>0$ and $n\times p$ matrix  $\mathbf{A}=(a_{ij})$ with $a_{ij}\geq 0$ for all $1\leq i\leq n$ and $1\leq  j \leq p$ such that the following holds:

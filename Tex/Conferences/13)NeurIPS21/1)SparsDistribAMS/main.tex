...
Extensive literature in distributed (synch or asynch) SGD.
Some distributed Adaptive method (DAMS and FODS (for FL) papers) but not much.
Even less sparsified variants of them.

\paragraph{Distributed Learning. } 
When a large number of compute engines is available 
...
A recent work \citep{chen2020quantized}, which focuses on adaptive gradient method, namely the Adam \citep{kingma2014adam} optimization method, develops a decentralized variant of gradient based and adaptive methods in the context of gossip protocols.

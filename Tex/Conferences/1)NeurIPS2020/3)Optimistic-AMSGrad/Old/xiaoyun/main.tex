...
There are rich literatures on gradient predicting methods, with some classical works including \textsc{Anderson} acceleration \citep{WN11}, \textsc{Minimal Polynomial Extrapolation} \citep{CJ76},  \textsc{Reduced Rank Extrapolation} \citep{E79}. 
...
\end{equation}
where $e_t$ is a second order term satisfying $\| e_t \|_2  = O( \| g_{t-1} - g^* \|_2^2)$ and $A \in \mathbb R^{d \times d}$ is an unknown matrix, see \citep{SAB16} for details and results. In our numerical experiments, we propose to use the extrapolation algorithm of \citep{SAB16}  to construct the sequence $\{m_t\}_{t>0}$ based on estimating the limit of a sequence using the last iterates \citep{BZ13}. 
...
The other competing method is the aforementioned \textsc{OPT-Adam$+\hat{v}_t$} method \citep{DISZ18}, whose details are described in the Supplementary Material. 
%The key difference is that it uses previous gradient as the gradient prediction of the next iteration. 
We also report the best result achieved by tuning the step size $\eta$ for \textsc{OPT-Adam$+\hat{v}_t$}.
For \textsc{OPT-AMSGrad}, we use the same $\beta_1$, $\beta_2$ and the best step size $\eta$ of \textsc{AMSGrad} for a fair evaluation of the improvement brought by the extra optimistic step. Algorithm \ref{alg:algex} has an additional parameter $r$ that controls the number of previous gradients used for gradient prediction. Empirically, we observe similar performance of \textsc{OPT-AMSGrad} with different values of $r$. 
Hence, we report the result for $r=5$. More details on $r$ will be discussed at the end of this section. In all experiments, the optimization algorithms have same initialization and the results are averaged over 5 repetitions.

\textbf{Datasets.}\hspace{0.1in}
% Following \citep{RKK18} and \citep{KB15}, 
We compare different algorithms on \textit{MNIST}, \textit{CIFAR10},
\textit{CIFAR100}, and \textit{IMDB} datasets. 
For \textit{MNIST}, we use two noisy variants named as \textit{MNIST-back-rand} and \textit{MNIST-back-image} from \citep{MNIST07}. They both have 12000 training samples and 50000 test samples, where random background is inserted to the original \textit{MNIST} hand written digit images. 
...
\textit{CIFAR10} and \textit{CIFAR100} are popular computer-vision datasets, and \textit{IMDB} movie review dataset is a popular dataset for text classification.

\textbf{Network architecture.}\hspace{0.1in}We adopt a multi-layer fully-connected ReLU neural network with input layer followed by a hidden layer with $200$ nodes, which is connected to another layer with $100$ nodes before the output layer. This network is tested on \textit{MNIST} variants. For CIFAR datasets, we adopt ALL-CNN network proposed by \citep{CNN15}. In addition, we also apply residual networks, Resnet-18 and Resnet-50~\citep{Rnet16}, which have achieved many state-of-the-art results. For the texture \textit{IMDB} dataset, we consider training a Long-Short Term Memory (LSTM) network. The network includes a word embedding layer with 5000 input entries representing most frequent words in the dataset, and each word is embedded into a 32 dimensional space. The output of the embedding layer is passed to 100 LSTM units, which is then connected to 100 fully connected ReLu's before the output layer. A mini-batch size of $128$ is used to compute the stochastic gradients for all experiments.

\textbf{Results.}\hspace{0.1in}Firstly, to illustrate the acceleration effect of \textsc{OPT-AMSGrad} at early stage, we provide the training loss against number of iterations in Figure~\ref{fig:train_loss}. We clearly observe that on all datasets, the proposed \textsc{OPT-AMSGrad} converges faster than the other competing methods, right after the training begins. In other words, we need fewer iterations (samples) to achieve the same training loss. This validates one of the main advantages of \textsc{OPT-AMSGrad}, which is a higher sample efficiency.
We are also curious about the long-term performance and generalization of the proposed method in test phase. In Figure~\ref{fig:testandtrain}, we plot the corresponding results when the model is trained to the state with stable test accuracy. We observe: 1) In the long term, \textsc{OPT-AMSGrad} algorithm may converge to a better point with lower loss, and 2) In this three applications, the proposed \textsc{OPT-AMSGrad} also outperforms the competing methods in terms of test accuracy.

\subsection{Choice of parameter $r$}
\begin{wrapfigure}[12]{r}{1.7in}\vspace{-0.4in}
\begin{center}
\mbox{
...
\end{wrapfigure}
\vspace{-0.1in}
Figure~\ref{fig:compare} compares the learning performance with different $r=3,5,10$. We observe that the choice of $r$ does not have significant impact on the training loss.
Taking into consideration both quality of gradient prediction and computational cost, we recommand setting $r=5$ in practice. As shown previously, \textsc{OPT-AMSGrad} with $r=5$ outperforms baseline methods on all the datasets.
\section{Conclusion}

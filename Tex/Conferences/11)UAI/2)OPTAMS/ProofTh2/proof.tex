...
{\color{red} Here, if we assume $\min_{1,...,T_M} a_t=a_m$ and constant lr $\eta$, we have from (13)
\begin{align*}
     \frac{\eta}{M}\sum_{1}^{T_M}((1+\beta_1)+(1-\beta_1)a_t)\|\nabla f(w_t)\|^2\geq \frac{\eta }{M}((1+\beta_1)+(1-\beta_1)a_m)\sum_{1}^{T_M}\|\nabla f(w_t)\|^2
\end{align*}
Then 
\begin{align*}
    \EE[\|\nabla f(w_T)\|^2]=\frac{\sum_{1}^{T_M}\|\nabla f(w_t)\|^2}{T_M}\leq \frac{M}{T_M \eta((1+\beta_1)+(1-\beta_1)a_m)} \times (13) RHS \leq ...
\end{align*}
Say if $\eta$ is still $\frac{1}{\sqrt{dT_M}}$, the order of the bound should be the same.
}

...
\State{\textbf{Initialize}: $\theta_{0} \in \Theta \subseteq \mathbb R^d $, as the global model and $\hat v_0=v_{0} = \epsilon \mathsf{1} \in \mathbb R^{d}$ and $\bar{\theta}_0 =  \frac{1}{n} \sum_{i=1}^n \theta_0$.}
\vspace{0.05in}
...
\begin{split}
&  \frac{1}{R}\sum_{r=1}^R  \EE\left[ \left\| \frac{\nabla f(\overline{\theta_r})}{\hat v_t^{1/4}}   \right \|^2 \right] \leq    \sqrt{\frac{M^2 p}{n}}  \frac{ \triangle}{\tot \alpha R}+4\alpha \left[ \frac{\alpha^2 L_\ell}{\sqrt{v_0}} M^2 (T-1)^2 \phi_M^2 (1-\beta_2)p +\phi_M^2\sqrt{M^2+p\sigma^2} \right]  \\
...
% As a side note, the poor performance of Fed-SGD is, to some extent, consistent with prior literature and practical numerical experiments showing that the standard periodical averaging of SGD usually performs worse with highly non-iid data~\cite{chen2020toward,reddi2020adaptive}.


We test the algorithms on CIFAR-10 using a ResNet-9 model. 
Similarly, we observe in Figure~\ref{fig:noniid} that \algo\ improves the performance of vanilla Fed-AMS method under various settings with different number of clients and number local epochs, as shown by its remarkable improvement in both the convergence speed and the test accuracy over both baseline methods. In all our empirical results, due to the faster convergence, \algo\ requires much fewer communication rounds to achieve satisfactory learning performance, validating its key advantage in practical training systems, and \algo\ seems to work in particularly well for the non-iid data distribution case, which is a very important scenario of FL in practice.
...
TinyImagenet & 67.58 $\pm$ 0.21  & 64.86 $\pm$ 0.83 & ?? $\pm$ ??  & 76.00 $\pm$ 0.26 \\
\toprule[1pt]
\end{tabular}
}
\end{table}
...
% CIFAR-10 & 90.75 $\pm$ 0.48  & 90.93 $\pm$ 0.22 & 91.57 $\pm$ 0.38  & 92.44 $\pm$ 0.53 \\
% % TinyImageNet & 79.64 $\pm$ 0.21  & 75.94 $\pm$ 0.83  & 93.50 $\pm$ 0.26 \\
% TinyImagenet & 79.64 $\pm$ 0.21  & 75.94 $\pm$ 0.83 & 83.64 $\pm$ 0.43  & 93.50 $\pm$ 0.26 \\
% \toprule[1pt]
% \end{tabular}
% }
% \end{table}
...
%        }
%    \end{center}
%	\caption{\textbf{non-i.i.d. setting.}. Test accuracy on CIFAR-10+Resnet18 and TinyImagenet+Resnet18, with non-iid data distribution.
%	}
...
%\begin{tabular}{llll}
%\toprule[1pt]
% & Fed-SGD      & Fed-AMS         & \textbf{\algo\ }       \\ \hline
%CIFAR-10 + Resnet18 & 90.75 $\pm$ 0.48  & 90.93 $\pm$ 0.22  & 92.44 $\pm$ 0.53 \\
%TinyImagenet + Resnet18 & 67.58 $\pm$ 0.21  & 64.86 $\pm$ 0.83  & 76.00 $\pm$ 0.26 \\
%\toprule[1pt]
%\end{tabular}
%}\vspace{-0.1in}
%\end{table}
...
\begin{figure}[t]
    \begin{center}
        \mbox{
        \hspace{-0.05in}\includegraphics[width=0.4\textwidth]{new_fmnist_mnist_fig/cifar_testerror_resnet18_ep1_client2_iid0_reddi.pdf}
...
for the theoretical results of the main paper, including the intermediary Lemmas and the main convergence result, Theorem~\ref{th:multiple update}.

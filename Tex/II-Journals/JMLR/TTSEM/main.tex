...
Note that the proposed \FISAEM\ (Fast Incremental SAEM) update, Line~\ref{line:fisaem} in Table~\ref{alg:prox}, draws {two} {independent} and {uniform} indices $(i_k, j_k) \in \inter$. 
...
The stepsize in~\eqref{eq:sestep} is set to $\rho_{k+1} = 1$ for the \ISAEM\ (Incremental SAEM) method initializing with $\StocEstep^{(0)} = \tilde{S}^{(0)}$; $\rho_{k+1} = \rho$ is  constant for the \SAEMVR\ (Variance Reduced SAEM) and \FISAEM. 
...
&-\frac{1}{n} \EE[\EE[ \tilde{S}_i^{(\tau_i^k)}-  \overline{\bss}_{i_k}(\param^{(k+1)})  | \mathcal{F}_{k} ]] + \frac{1}{n} \EE[\eta_{i_k}^{(k+1)}] \eqsp.
\end{split}
\eeq

...
\runtitle{Two-Timescale Stochastic EM Algorithms}

\begin{aug}
\author{\fnms{Belhal} \snm{Karimi}\thanksref{}\ead[label=e1]{belhalkarimi@baidu.com}}
\and
\author{\fnms{Ping} \snm{Li}\thanksref{}\ead[label=e2]{liping11@baidu.com}}

\address{ Cognitive Computing Lab \\
    Baidu Research \\
...
\begin{keyword}
\kwd{ two-timescale, stochastic, em, nonconvex, mcmc, monte carlo, latent variable}
\end{keyword}
\tableofcontents
\end{frontmatter}
...
For all $ i \in \inter$, draw $M$ samples, noted $\{z_{i,m} \sim p(z_i|y_i;\param)\}_{m=1}^{M}$, and compute the MC integration, noted $\tilde{S}$, of $\overline{\bss}(\param)$ defined by~\eqref{eq:definition-overline-bss}:
\beq\label{eq:mcstep}
...
\medskip
\noindent {Remarks on Table~\ref{alg:prox}:} For all methods, we define a random index noted $i_k \in \inter$ and drawn at iteration $k$, and $\tau_i^k = \max \{ k' : i_{k'} = i,~k' < k \}$ as the iteration index where $i \in \inter$ is last drawn prior to iteration $k$.
...
\beq \label{eq:smooth}
\begin{split}
& \| \overline{\bss}_i ( \overline{\param} ({\bss})) - \overline{\bss}_i ( \overline{\param} ({\bss}' )) \| \leq \Lip{{\bss}} \| {\bss} - {\bss}' \| \, , \\
&\| \grd  V ( {\bss} ) - \grd  V ( {\bss}' ) \| \leq \Lip{V} \| {\bss} - {\bss}' \|\eqs,
\end{split}
\eeq
...
&\| {\bss} - \os( \op ({\bss})) \|^2\geq \upsilon_{\max}^{-2} \| \grd V ( {\bss} ) \|^2\eqs.
\end{split}
\eeq
\end{lemmacoloured}
...
\beq\notag
\begin{split}
&\EE[\stt^{(k+1)} - \hat{\bss}^{(k)}]= \EE[\overline{\bss}^{(k)} - \hat{\bss}^{(k)}] \\
&+ (1 - 1/n ) \EE[\frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^k)}- \overline{\bss}^{(k)}]+\frac{1}{n}\EE[\eta_{i_k}^{(k+1)}]\eqsp,
\end{split}
\eeq
...
\begin{split}
&\stt^{(k+1)} - \hat{\bss}^{(k)}  \\
 =& \stt^{(k)} - \hat{\bss}^{(k)} +\frac{1}{n}( \tilde{S}_{i_k}^{(k+1)} - \tilde{S}_{i_k}^{(\tau_i^k)}  ) \\
...
\beq\notag
\begin{split}
\stt^{(k+1)} - \hat{\bss}^{(k)} = &\overline{\bss}^{(k)} - \hat{\bss}^{(k)} + \stt^{(k)}- \overline{\bss}^{(k)} \\
& - \frac{1}{n}( \tilde{S}_{i_k}^{(\tau_i^k)} -  \overline{\bss}_{i_k}(\param^{(k)})   ) + \frac{1}{n}\eta_{i_k}^{(k+1)}\eqsp .
\end{split}
\eeq
...
\end{split}
\eeq
Since we have $\EE[ \tilde{S}_i^{(\tau_i^k)} | \mathcal{F}_{k} ] =\frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^k)}$ and \\
$\EE[  \overline{\bss}_{i_k}(\param^{(k)})  | \mathcal{F}_{k} ]= \overline{\bss}^{(k)}$, we conclude the proof.
\end{proof}
...
\beq\notag
\begin{split}
\frac{2}{n^2}\EE[ \| \os_{i_k}^{(k)} - \os_{i_k}^{(t_{i_k}^k)} \|^2 ] &= \frac{2}{n^3} \sum_{i=1}^n \EE[ \| \os_i^{(k)} - \os_i^{(t_i^k)} \|^2 ] \\
&\overset{(a)}{\leq} \frac{2\Lip{\bss}^2}{n^3}
...
& \EE [\pscal{  \stt^{(k+1)}  - \hs{k}}{ \grd V( \hs{k} ) } ]\\
 \leq & (\upsilon^2_{\max}\frac{\beta(n-1) + 1}{2n}-\upsilon_{\min}) \EE [\|  \overline{\bss}^{(k)}  - \hs{k}\|^2  ] \\
 & + (1 - \frac{1}{n})/(2\beta)\EE[\| \frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^k)}-  \overline{\bss}^{(k)}\|^2] \\
...
& \EE[ \| \hs{k+1} - \hs{t_i^k} \|^2 ]\\
 \leq  &\EE [ \| \hs{k+1} - \hs{k} \|^2 + \| \hs{k} - \hs{\tau_i^k} \|^2 \\
 &+  \frac{\gamma_{k+1}}{\beta} \| \hs{k} - \stt^{(k+1)}\|^2 + \gamma_{k+1} \beta \| \hs{k}- \hs{\tau_i^k} \|^2 ]\eqsp,
\end{split}
\eeq
...
\begin{split}
& \frac{1}{n} \sum_{i=1}^n \EE[ \| \hs{k+1} - \hs{\tau_i^{k+1}} \|^2 ] \\
 \leq & 4(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  )\EE [  \|   \os^{(k)} - \hs{k}  \|^2  ] \\
 &+ 2(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  )\EE [\| \eta_{i_k}^{(k)}\|^2 ]\\
...
%\begin{split}
%&\Delta^{(k+1)}\\
%  \leq & 4 \sum_{ \ell = 0 }^k \prod_{j = \ell +1}^k ( 1 -  \Lambda_{(j)} ) (\gamma_{\ell+1}^2 +\frac{\gamma_{\ell+1}}{\beta}  )  \EE[  \|  \os^{(\ell)} - \hs{\ell}  \|^2 ] \\
...
Denote $ \Lambda_{(k+1)} =\frac{1}{n} - \gamma_{k+1} \beta - \frac{2\gamma_{k+1} \Lip{\bss}^2}{n^2}(\gamma_{k+1} +\frac{1}{\beta}) $ and note that $\Delta^{(0)} = 0$, thus the telescoping sum yields and for any $k >0$, $\tilde{\alpha}_k \geq 0$, we have by Lemma~\ref{lem:growth} that:
\beq\notag
...
\eeq
which yields an upper bound on the gradient of the Lyapunov function $V$ and concludes the proof.
...
\begin{split}
 & \EE [\| \hs{k} - \stt^{(k+1)}\|^2 ] \\
 \leq & 2\rho^2 \EE[ \| \hs{k} - \os^{(k)} \|^2] +  2\rho^2\Lip{\bss}^2 \EE[ \| \hs{k} - \hs{\ell(k)} \|^2 ]\\
...
\begin{split}
&\EE[ \| \hs{k} -\stt^{(k+1)} \|^2 ] \\
\leq &2\rho^2 \EE[ \| \hs{k} - \os^{(k)} \|^2] + 2\rho^2 \EE[ \| \os^{(k)} - \StocEstep^{(k+1)} \|^2 ]\\
...
\beq\notag
\overline{b}_i = \overline{b}_{i+1} (1 + \gamma_{k+1} \beta + 2 \gamma_{k+1}^2\rho^2 \Lip{\bss}^2 ) + \gamma_{k+1}^2\rho^2 \Lip{V} \Lip{\bss}^2\eqsp,
\eeq
for $i=0,1,\dots,m-1$ and $\overline{b}_m = 0$.
Note that $\overline{b}_i$ is decreasing with $i$ and this implies
\beq\notag
\overline{b}_i \leq \overline{b}_0 = \gamma_{k+1}^2\rho^2 \Lip{V} \Lip{\bss}^2 \frac{ (1 + \gamma_{k+1} \beta + 2 \gamma_{k+1}^2 \rho^2\Lip{\bss}^2 )^m - 1 }{ \gamma_{k+1} \beta + 2 \gamma_{k+1}^2 \rho^2\Lip{\bss}^2 } \eqsp,
\eeq
for $i=1,2,\dots,m$.
...
\begin{split}
&R_{k+1 } 
 \leq   \EE [ V( \hs{k} ) ]+ \tilde{\eta}^{(k+1)}+ \tilde{\chi}^{(k+1)}\\
& - \gamma_{k+1}( - \gamma_{k+1}\rho^2 \Lip{V} - b_{k+1}(\frac{\rho}{\beta}+ 2\gamma_{k+1} \rho^2) ) \EE[ \|  \hmean_{k} \|^2 ] \\
&- \gamma_{k+1}(  \rho \upsilon_{\min} +   \upsilon_{\max}^2  )  \EE[ \|  \hmean_{k} \|^2 ] \\
& + (  \underbrace{b_{k+1} (1 + \gamma \beta + 2 \gamma^2\rho^2 \Lip{\bss}^2 ) + \gamma^2\rho^2 \Lip{V} \Lip{\bss}^2}_{=b_k~~\text{since $k+1 \leq \ell(k)+m$}} ) \EE[  \| \hs{k} - \hs{\ell(k)} \|^2 ] \\
&\eqsp,
\end{split}
\eeq
}
...
\beq\notag
\frac{\mu}{n} \leq \gamma \beta + 2 \gamma^2 \Lip{\bss}^2 \leq \frac{\mu}{n} + \frac{2 \mu^2}{c_1^2 n^{\frac{4}{3}}} \leq \frac{\mu c_1^2 + 2 \mu^2}{c_1^2} \frac{1}{n} \eqs,
\eeq
and $(1 + \gamma \beta + 2 \gamma^2 \Lip{\bss}^2 )^m \leq \rme-1$ where the required $\mu$ in (b) can be found by solving the quadratic equation.
...
{\small\beq\notag
\begin{split}
 \hs{k} -  \stt^{(k+1)}= & \rho (\hs{k} - \overline{\bss}^{(k)})  + \rho \eta_{i_k}^{(k+1)}+ \rho [(\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)})  \\
 &- \EE[\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)}] ]+ (1-\rho)\left( \hs{k} - \tilde{S}^{(k)}\right)\eqsp,
\end{split}
\eeq
}
...
\beq\notag
\begin{split}
 \stt^{(k+1)} - \hs{k}  = & \rho (\overline{\bss}^{(k)}-\hs{k}) + \rho \eta_{i_k}^{(k+1)} - \rho [(\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)}) \\
 &- \EE[\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)}] ]  + (1-\rho)\left(\stt^{(k)} - \hs{k}\right) \eqsp,
\end{split}
\eeq
...
\begin{split}
&  \EE [\| \hs{k} - \stt^{(k+1)}\|^2 ] \\
\leq & 2\rho^2 \EE[ \| \hs{k} - \os^{(k)} \|^2] +  2\rho^2\frac{\Lip{\bss}^2}{n}
...
\begin{split}
\hs{k+1} - \hs{k}  = & -\gamma_{k+1} ((1-\rho)[\hs{k} - \stt^{(k)} ] \\
&+\rho[\hs{k} - \overline{\StocEstep}^{(k)} - ( \tilde{S}_{i_k}^{(k)}  -  \tilde{S}_{i_k}^{(t_{i_k}^k)}  )] ) \eqsp.
\end{split}
\eeq
We observe, using the identity~\eqref{eq:fisaem_drift_main}, that
\beq \label{eq:auxlemfisaem_main}
\begin{split}
\EE[ \| \hs{k} -\stt^{(k+1)} \|^2 ] \leq &2\rho^2 \EE[ \| \hs{k} - \os^{(k)} \|^2] \\
&+ 2\rho^2 \EE[ \| \os^{(k)} - \StocEstep^{(k+1)} \|^2 ]\\
...
\begin{split}
&\EE[ \| \os^{(k)} - \StocEstep^{(k+1)} \|^2 ] \\
 = &\EE [ \| \frac{1}{n} \sum_{i=1}^n ( \os_i^{(k)} -\overline{\StocEstep}_i^{(k)} ) - ( \tilde{S}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)} ) \|^2 ] \\
...
\beq\notag
\begin{split}
\EE[ \| \os_{i_k}^{(k)} - \os_{i_k}^{(t_{i_k}^k)} \|^2 ] = & \frac{1}{n} \sum_{i=1}^n \EE[ \| \os_i^{(k)} - \os_i^{(t_i^k)} \|^2 ] \\
&\overset{(a)}{\leq} \frac{\Lip{\bss}^2}{n}
\sum_{i=1}^n \EE[ \| \hs{k} - \hs{t_i^k} \|^2 ]\eqsp.
\end{split}
\eeq
...
\begin{split}
V( \hs{k+1} )  \leq& V( \hs{k} ) - \gamma_{k+1} \pscal{  \hs{k} - \stt^{(k+1)} }{ \grd V( \hs{k} ) } \\
&+ \frac{\gamma_{k+1}^2 \Lip{V}}{2} \| \hs{k}  -  \stt^{(k+1)}\|^2\eqsp.
\end{split}
\eeq
...
& \EE[V( \hs{k+1} )]  
 ...
 &-\frac{\gamma_{k+1}\rho^2}{2} \xi^{(k+1)} - \frac{\gamma_{k+1}(1-\rho)^2}{2} \EE[\| \hs{k} - \tilde{S}^{(k)}\|^2]\\
...
\beq\notag
\begin{split}
 \Delta^{(k+1)} \leq & \left( 1 - \frac{1}{n} +\gamma_{k+1}\beta+\gamma_{k+1}^2\rho^2 \Lip{\bss}^2\right) \Delta^{(k)}\\
& + \gamma_{k+1} (1-\rho)^2 \left( 2\gamma_{k+1} + \frac{1}{\beta} \right)\EE[ \|\hs{k} - \tilde{S}^{(k)}\|^2]  \\
 & + \left(2 \gamma_{k+1}^2 \rho^2 + \frac{\gamma_{k+1} \rho^2}{\beta}\right) \EE[\| \overline{\bss}^{(k)}-\hs{k}\|^2 ]\\
& + \gamma_{k+1}\left(2 \gamma_{k+1} + \frac{ \rho^2}{\beta} \right)\EE[\norm{\eta_{i_k}^{(k+1)}}^2 ]\eqsp.
 \end{split}
\eeq
...
1 - \frac{1}{n} +\gamma_{k+1}\beta+\gamma_{k+1}^2\rho^2 \Lip{\bss}^2
 &\leq 1 - \frac{1}{n} + \frac{1}{\alpha kn} + \frac{ 1 }{ \alpha^2 c_1^2 k^2 n^{\frac{4}{3}} } \\
 &\leq 1 - \frac{c_1(k\alpha  - 1) - 1}{k\alpha n c_1 }\\
 & \leq 1 - \frac{1}{k\alpha n c_1 }
  \end{split}
\eeq
}
...
& +\sum_{ \ell = 0 }^k \omega_{k, \ell} \gamma_{\ell+1} (1-\rho)^2 \left( 2\gamma_{\ell+1} +\frac{1}{\beta} \right)\EE[ \norm{\tilde{S}^{(\ell)} - \hs{\ell}}^2] \\
&+ \sum_{ \ell = 0 }^k \omega_{k, \ell}\gamma_{\ell+1} \tilde{\epsilon}^{(\ell+1)}  \eqsp,
\end{split}
\eeq
}
where $ \omega_{k, \ell} =  \prod_{j = \ell +1}^k ( 1 -  \Lambda_{(j)} )$ and \\
$\tilde{\epsilon}^{(\ell+1)}   = \left(2 \gamma_{k+1} + \frac{ \rho^2}{\beta} \right)\EE[\norm{\eta_{i_k}^{(k+1)}}^2 ]$.
...
\begin{split}
& s_{i,m}^{(1)} = \indiacc{m}(z_i), \quad  s_{i,m}^{(2)} =   \indiacc{m}(z_i) y_i, \quad \phi^{(2)}_m(\param) =  {\mu_m} \eqsp,\\
&\phi_m^{(1)}(\param) =   \left\{\log(\omega_m) -\frac{\mu_m^2}{2}\right\} - \left\{\log(1 - {\textstyle  \sum_{j=1}^{M-1}} \omega_j) - \frac{\mu_M^2}{2}\right\} \eqsp,\\
& s_i^{(3)} = y_i, \quad \phi^{(3)}(\param) = \mu_M \eqsp,
\end{split}
\eeq
}
and $\psi(\param) =   - \left\{\log(1 - \sum_{m=1}^{M-1} \omega_m) - \frac{\mu_M^2}{2 \sigma^2}\right\}$.
We also define for $m \in [M]$,  $j \in \{1, 2, 3 \}$, $s_{m}^{(j)} = n^{-1}\sum_{i=1}^n s_{i,m}^{(j)}$. 
Consider the sample used to compute an approximation of the conditional expectation $\EE_{\param}[ 1_{\{z_i=m\}} | y= y_{i} ]$:
\beq \label{eq:cexp}
...
{\small
\beq\label{eq:stat_gmm}
 \tilde{S}_{i_k}^{(k)} = ( \underbrace{ \{ \indiacc{m}(z_{i_k,m})\}_{m}}_{\eqdef \tilde{s}_{i_k}^{(1)}} , \underbrace{ \{\indiacc{m}(z_{i_k,m})y_{i_k}\}_{m}}_{\eqdef \tilde{s}_{i_k}^{(2)}}   , \underbrace{y_{i_k}}_{\eqdef \overline{\bss}_{i_k}^{(3)}( \param^{(k)} )} )^\top.
\eeq
}
Recall the regularizer $ \frac{\delta}{2}\sum_{m=1}^M \mu_m^2 - \log \Dir(\bomega; M, \epsilon)$ we used, which also reads:
{\small\beq \textstyle \label{eq:regu}
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=3in]{fig2/figgmm.eps}
\end{center}
...
\beq\notag
\begin{split}
&\Phi_{i}=\mathbf{K}_{\mathbf{g}} z_{i} \quad \textrm{where} \\
& \left(\mathbf{K}_{\mathbf{g}} z_{i}\right)(x)=\sum_{k=1}^{k_{s}} \mathbf{K}_{\mathbf{g}}\left(x, g_{k}\right)\left(z_{i}^{(1)}(k), z_{i}^{(2)}(k)\right)\eqs,
\end{split}
\eeq
...
\vspace{0.2in}
\begin{figure}[H]
\includegraphics[width=0.5\textwidth]{fig/variancedigit.png}
...
Lognormal distributions are used for the four PK parameters, for eg. $\log(T_i^{\textrm{lag}}) \sim \mathcal{N}(\log(T^{\textrm{lag}}_{\rm pop}), \omega^2_{T^{\textrm{lag}}} )$.
%{\small\begin{align}
...
S_1(z)  &= \frac{1}{n} \sum_{i=1}^n z_i  \\
 S_2(z) &=\frac{1}{n} \sum_{i=1}^n z_i^\top z_i \\
  S_3(z)  &= \frac{1}{n}  \sum_{i=1}^n  \left(y_i - f(t_{i},z_i)\right)^2 \eqs ,
\end{split}
\eeq
...
%\right) \eqsp,
\eeq
where $z_{\rm pop}$ denotes the vector of fixed effects \\
$(T^{\textrm{lag}}_{\rm pop}, ka_{\rm pop}, V_{\rm pop}, k_{\rm pop})$.
...
\begin{figure}[t]
\centering
\includegraphics[width=3in]{fig2/figpk.eps}

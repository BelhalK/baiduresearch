...
saemix.fi<-function(model,data,control=list()) {
...
phiM0 <- list(phiM = phiM)
phiMs <- rep(list(phiM0),Dargs$N)
...
for (kiter in 1:saemix.options$nbiter.tot) { # Iterative portion of algorithm
  ...
  # Burn-in - resetting sufficient statistics
    if(opt$flag.fmin && kiter==saemix.options$nbiter.sa) {
  	...
  	# E-step
    if(saemix.options$algo=="fi"){
        xmcmc<-estep.fi(kiter, Uargs, Dargs, opt, structural.model, mean.phi, varList, DYF, phiM,saemixObject)
      } else{
  ...
  ############# Stochastic Approximation
      ...
      if(saemix.options$algo=="vr"){
        ...
        xstoch<-mstep.fi2(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, 
          varList, phi, betas, suffStat,saemix.options,phiMs,h.suffStat,suffStat.fi, indchosen, indchosen.j)

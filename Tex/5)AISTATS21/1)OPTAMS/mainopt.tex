...
\aistatstitle{An Optimistic Acceleration of AMSGrad for Nonconvex Optimization}
...
\begin{abstract}
In this paper, we propose a new variant of AMSGrad \citep{RKK18}, a popular adaptive gradient based optimization algorithm widely used for training deep neural networks. 
...
In the nonconvex case, we establish a non-asymptotic convergence bound independently of the initialization of the method.
...
\captionsetup{justification=centering}
    %\hspace{-0.15in}
    \includegraphics[width=3.2in]{plot.pdf}
...
In Section~\ref{sec:numerical}, we derive a basic, yet effective, gradients prediction algorithm, see Algorithm~\ref{alg:algex}, embedded in \textsc{OPT-AMSGrad}.

\subsection{Finite-Time Analysis in the Nonconvex Case}
...
\end{assumption}
We assume that the optimistic guess is in general reasonable:
\begin{assumption}\label{ass:guessbound}
For any $t >0$, $0 < \pscal{m_t}{ g_t} = a_t \|g_t\|^2$ with some $0<a_t\leq 1$, and $\Vert m_t\Vert\leq \Vert g_t\Vert$, where $\pscal{}{}$ denotes the inner product.
\end{assumption}
H\ref{ass:guessbound} says that the predicted gradient has acute angle with the true gradient and bounded norm, as the shadowed area in Figure \ref{fig:assumption}. We bound it by $\Vert g_t\Vert$ mainly for convenience, and our result will hold with arbitrary norm bound (by also changing the range of $a_t$ properly), and the scale of $m_t$ can indeed be offset by adjusting the learning rate. Lastly, We make a classical assumption in nonconvex optimization \citep{ghadimi2013stochastic} on the magnitude of the gradient:
...
\captionsetup{justification=centering}
    %\hspace{-0.15in}
    \includegraphics[width=3.2in]{plot_prediction.pdf}
...
\begin{align*}
&\tilde{C}_1 =\frac{\major \left[ \frac{a(1 - \beta_1)^2}{1-\beta_2} + 2L \frac{1}{1-\beta_2} +  \Delta f  +   \frac{4L \beta_1^2(1 + \beta_1^2) }{(1 - \beta_1)(1 - \beta_2)(1-\gamma)} \right]}{(1 - a\beta_1) + (\beta_1 + a)}\\
&\tilde{C}_2 = \frac{ (a\beta_1^2 -2 a \beta_1 + \beta_1)\major^2 }{(1 - \beta_1) \left((1 - a\beta_1) + (\beta_1 + a)\right)}  \EE\left[ \norm{\hat{v}_{0}^{-1/2}}    \right]  \eqsp,
\end{align*}
where $\Delta f = f(\overline{w}_{1}) - f(\overline{w}_{T_{\sf M}+1})$
\end{Theorem}
The bound for our OPT-AMSGrad method matches the complexity bound of $\mathcal{O}( \sqrt{d/T_{\sf M}} + 1/T_{\sf M} )$ of \citep{ghadimi2013stochastic} for SGD considering the dependence of T only, and of \citep{ZTYCG18} for AMSGrad method.
...
\textbf{Comparison to \textsc{Optimistic-Adam}~\citep{DISZ18}.}\hspace{0.1in} \cite{DISZ18} develop an optimistic variant of ADAM, namely \textsc{Optimistic-Adam}. 
...
\centering
\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{simulation/fig2/M_rand_train_loss_no1.eps}\hspace{-0.1in}
\includegraphics[width=1.7in]{simulation/fig2/M_image_train_loss_no1.eps}\hspace{-0.1in}
}

\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{simulation/fig2/cifar_cnn_train_loss_no1.eps}\hspace{-0.1in}
\includegraphics[width=1.7in]{simulation/fig2/imdb_lstm_train_loss_no1.eps}
}

\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{simulation/fig2/cifar10_resnet_train_loss.eps}\hspace{-0.1in}
\includegraphics[width=1.7in]{simulation/fig2/cifar100_resnet_train_loss.eps}
}
...
\begin{figure*}
\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/new_mnist_img_figure/mnist_img_train_loss_disz_2.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/new_mnist_img_figure/mnist_img_train_acc_disz_2.eps}\hspace{-0.12in}
%}
%\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/mnist_img_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/mnist_img_test_acc_disz.eps}
}
\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/cifar10_train_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/cifar10_train_acc_disz.eps}\hspace{-0.12in}
%}
%\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/cifar10_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/cifar10_test_acc_disz.eps}
}
\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/cifar100_train_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/cifar100_train_acc_disz.eps}\hspace{-0.12in}
%}
%\mbox{\hspace{-0.2in}
\includegraphics[width=1.7in]{new_figure/cifar100_test_loss_disz.eps}\hspace{-0.12in}
\includegraphics[width=1.7in]{new_figure/cifar100_test_acc_disz.eps}
}
...
\\  = &\sum_{t=1}^T \langle  w_t - \tilde{w}_{t+1} , g_t - \tilde{m}_t \rangle + \langle w_t - \tilde{w}_{t+1}, \tilde{m}_t \rangle \\
&+  \langle \tilde{w}_{t+1} - w^*, \tilde{g}_t  \rangle+ \langle \tilde{w}_{t+1} - w^*,g_t - \tilde{g}_t  \rangle \eqsp.
\end{aligned}
\end{equation}
...
\begin{aligned}
&  \langle \tilde{w}_{t+1} - w^*, & \tilde{g}_t \rangle \\
& \leq \frac{1}{\eta_t}\big[ B_{\psi_t}( w^*, \tilde{w}_{t}) -B_{\psi_t}(w^*,  \tilde{w}_{t+1} ) - B_{\psi_t}(\tilde{w}_{t+1}, \tilde{w}_{t}) \big] \eqsp.
\end{aligned}
\end{equation}
...
\begin{aligned}
& \langle w_t -\tilde{w}_{t+1}, \tilde{m}_t  \rangle \\
& \leq \frac{1}{\eta_t}\big[ B_{\psi_{t-1}}(\tilde{w}_{t+1}, \tilde{w}_{t}) - B_{\psi_{t-1}}(\tilde{w}_{t+1}, w_t ) - B_{\psi_{t-1}}(w_{t}, \tilde{w}_{t}) \big] \eqsp.
\end{aligned}
\end{equation}
By (\ref{nn1}), (\ref{nn2}), and (\ref{nn3}), we obtain
\begin{equation} \label{nnn}
\begin{aligned}
 \mathcal{R}_T  \overset{(\ref{nn1})}{\leq} & \sum_{t=1}^T \langle  w_t - \tilde{w}_{t+1} , g_t - \tilde{m}_t \rangle + \langle w_t - \tilde{w}_{t+1}, \tilde{m}_t \rangle \\
&+  \langle \tilde{w}_{t+1} - w^*, \tilde{g}_t  \rangle+ \langle \tilde{w}_{t+1} - w^*,g_t - \tilde{g}_t  \rangle \\
 \overset{(\ref{nn2}), (\ref{nn3})}{\leq}&  \sum_{t=1}^T \| w_t - \tilde{w}_{t+1} \|_{\psi_{t-1}} \| g_t - \tilde{m}_t  \|_{\psi_{t-1}^*} \\
&+ \|  \tilde{w}_{t+1} - w^* \|_{\psi_{t-1}} \| g_t - \tilde{g}_t  \|_{\psi_{t-1}^*}\\
...
\begin{aligned}
  &\mathcal{R}_T \\
  &\leq \sum_{t=1}^T \Big\{   \frac{1}{2 \eta_t} \| w_t - \tilde{w}_{t+1} \|_{\psi_{t-1}}^2 + \frac{\eta_t}{2} \| g_t - m_t  \|_{\psi_{t-1}^*}^2 \\
  & + \|  \tilde{w}_{t+1} - w^* \|_{\psi_{t-1}} \| g_t - \tilde{g}_t  \|_{\psi_{t-1}^*}\\\
...
\begin{aligned}
A_2 & := B_{\psi_t}( w^*, \tilde{w}_{t}) -B_{\psi_t}(w^*,  \tilde{w}_{t+1} ) \\
 &= \langle w^* - \tilde{w}_{t+1}  , \text{diag}(\hat{v}_{t+1}^{1/2} -\hat{v}_t^{1/2} ) ( w^* - \tilde{w}_{t+1}  ) \rangle\\
...
& + \eta \sum_{i=1}^d \frac{ (g_{T}[i] - m_{T}[i])^2 }{ \sqrt{ T \big( (1-\beta_2) \sum_{s=1}^{T-1} \beta_2^{T-1-s} (g_{s}[i] - m_{s}[i])^2 } \big) }\\
...
\begin{align*}
    ...
    \frac{\eta}{\sqrt{1 - \beta_2}} \sum_{i=1}^d \sum_{t=1}^{T} \frac{ | g_{t}[i] - m_{t}[i] | }{ \sqrt{t} }\\
    & \leq \frac{\eta \sqrt{1 + \log T}}{\sqrt{1 - \beta_2}} \sum_{i=1}^d \| (g-m)_{1:T}[i] \|_2 ,
\end{align*}
...
\overline{w}_{t+1} - \overline{w}_t =  &\frac{1}{1 - \beta_1} ( w_{t+1} - \tilde{w}_t)  -  \frac{\beta_1}{1 - \beta_1}( w_{t} - \tilde{w}_{t-1})\\
 = &- \frac{1}{1 - \beta_1} \eta_{t} \hat{v}_{t}^{-1/2} (\theta_t + h_{t+1})  \\
&+   \frac{\beta_1}{1 - \beta_1}\eta_{t-1} \hat{v}_{t-1}^{-1/2} (\theta_{t-1} + h_{t})\\
 = &- \frac{1}{1 - \beta_1}  \eta_{t} \hat{v}_{t}^{-1/2} (\theta_t + \beta_1 \theta_{t-1}) \\
&-  \frac{1}{1 - \beta_1}  \eta_{t} \hat{v}_{t}^{-1/2} (1- \beta_1) m_{t+1}\\
 &+ \frac{\beta_1}{1 - \beta_1} \eta_{t-1} \hat{v}_{t-1}^{-1/2} (\theta_{t-1} + \beta_1 \theta_{t-2}) \\
&+ \frac{\beta_1}{1 - \beta_1}  \eta_{t-1} \hat{v}_{t-1}^{-1/2} (1- \beta_1) m_{t} \eqsp.
\end{split}
\eeq
...
\beq
\begin{split}
\overline{w}_{t+1} - \overline{w}_t \leq &  \frac{\beta_1}{1- \beta_1} \tilde{\theta}_{t-1} \left[ \eta_{t-1} \hat{v}_{t-1}^{-1/2} - \eta_{t} \hat{v}_{t}^{-1/2} \right] \\
& - \eta_t \hat{v}_t^{-1/2} \tilde{g}_t \eqsp.
\end{split}
\eeq
\end{proof}
...
\beq
\begin{split}
&\eta_{t}^{2} \EE \left[\left\|\hat{v}_{t}^{-1/2} \theta_{t}\right\|_{2}^{2}\right] \\
& \leq \frac{\eta_{t}^{2} (1- \beta_1)^2}{1 - \beta_2}  \mathbb{E}\left[\sum_{i=1}^{d} \frac{( \sum_{r=1}^t \beta_1^{t-r} g_{r,p})^{2}}{ \sum_{r=1}^t \beta_2^{t-r} g^2_{r,p}}\right] \\
...
\beq
\begin{split}
&\tilde{C}_1 = C_1 +  \frac{\major\left[ \frac{a(1 - \beta_1)^2}{1-\beta_2} + 2L \frac{1}{1-\beta_2}  \right]}{(1 - a\beta_1) + (\beta_1 + a)} \\
&C_1 = \frac{\major}{(1 - a\beta_1) + (\beta_1 + a)}  \Delta f \\
& + \frac{ 4L \left(\frac{\beta_1}{1 - \beta_1}\right)^2 \major}{(1 - a\beta_1) + (\beta_1 + a)} \frac{(1 + \beta_1^2) (1- \beta_1)}{(1 - \beta_2)(1-\gamma)}\\
...
&\nabla f(w_t)^\top (\overline{w}_{t+1} - \overline{w}_t) \\
 \leq &\nabla f(w_t)^\top \left[\frac{\beta_1 \tilde{\theta}_{t-1} \left[ \eta_{t-1} \hat{v}_{t-1}^{-1/2} - \eta_{t} \hat{v}_{t}^{-1/2}\right] }{1 - \beta_1}- \eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t \right]\\
...
\begin{split}
& \nabla f(w_t)^\top (\overline{w}_{t+1} - \overline{w}_t)  \\
\leq&  \frac{\beta_1 (1+\beta_1)}{1 - \beta_1} \major^2 [ \|\eta_{t-1} \hat{v}_{t-1}^{-1/2}\| - \|\eta_{t} \hat{v}_{t}^{-1/2} \|]\\
 - & \nabla f(w_t)^\top\eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t  \eqsp,
\end{split}
\eeq
...
&\nabla f(w_t)^\top (\overline{w}_{t+1} - \overline{w}_t)\\
 \leq &  - \nabla f(w_t)^\top\eta_{t-1} \hat{v}_{t-1}^{-1/2} \bar{g}_t \\
 &+ \frac{1}{1 - \beta_1} (a\beta_1^2 -2 a \beta_1 + \beta 1)\major^2 [ \|\eta_{t-1} \hat{v}_{t-1}^{-1/2}\| - \|\eta_{t} \hat{v}_{t}^{-1/2} \|] \\
&  - \nabla f(w_t)^\top\eta_{t} \hat{v}_{t}^{-1/2} (\beta_1 g_{t-1} + m_{t+1}) \eqsp .
\end{split}
\eeq
...
By Lemma~\ref{lem:momentum} we also have:
 \beq
 \begin{split}
& \overline{w}_{t+1} - \overline{w}_t\\
  = &\frac{\beta_1}{1 - \beta_1} \tilde{\theta}_{t-1} \left[ \eta_{t-1} \hat{v}_{t-1}^{-1/2} - \eta_{t} \hat{v}_{t}^{-1/2}\right] \\
 &- \eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t \\
= &\frac{\beta_1}{1 - \beta_1} \tilde{\theta}_{t-1}\eta_{t-1} \hat{v}_{t-1}^{-1/2} \left[ I - (\eta_{t} \hat{v}_{t}^{-1/2}) (\eta_{t-1} \hat{v}_{t-1}^{-1/2})^{-1} \right] \\
&- \eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t \\
 =& \frac{\beta_1}{1 - \beta_1} \left[ I - (\eta_{t} \hat{v}_{t}^{-1/2}) (\eta_{t-1} \hat{v}_{t-1}^{-1/2})^{-1} \right] (\tilde{w}_{t-1} - w_t)\\
 & - \eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t \eqsp,
 \end{split}
 \eeq
...
$$
 ...
 \beq \notag
 \begin{split}
&  \left( \nabla f(\overline{w}_t) -  \nabla f(w_t)\right)^\top (\overline{w}_{t+1} - \overline{w}_t)\\
  \leq & L \frac{\beta_1}{1 - \beta_1} \|\eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t\|  \|w_t - \tilde{w}_{t-1}\|\\
...
\beq\label{eq:term3} 
\begin{split}
&  \frac{L}{2} \|\overline{w}_{t+1} - \overline{w}_t\|^2 \\
& \leq  \frac{L}{2} \left[ \frac{\beta_1}{1 - \beta_1} \|\tilde{w}_{t-1} - w_t\| + \|\eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t\|\right]\\
 &  \leq L \|\eta_{t} \hat{v}_{t}^{-1/2} \tilde{g}_t\|^2 + 2L  \left(\frac{\beta_1}{1 - \beta_1}\right)^2 \|\tilde{w}_{t-1} - w_t\|^2  \eqsp.
\end{split}
\eeq
...
& \frac{1}{\major} \sum_{t=1}^{T_{\sf M}} \left( (1 - a\beta_1)   \eta_{t-1} + (\beta_1 + a)   \eta_{t} \right) \|\nabla f(w_t)\|^2 \\
 \leq & \EE\left[  f(\overline{w}_{1}) + \frac{1}{1 - \beta_1}\tilde{\major}^2 \|\eta_{0} \hat{v}_{0}^{-1/2}\|   \right]\\
 &- \EE\left[  \left(f(\overline{w}_{T_{\sf M}+1})  +   \frac{1}{1 - \beta_1}\tilde{\major}^2  \|\eta_{T_{\sf M}} \hat{v}_{T_{\sf M}}^{-1/2} \| \right)      \right]\\
...
& + D_1 \frac{\eta}{T_{\sf M}} \sum_{t=1}^{T_{\sf M}} \EE [ \| \hat{v}_{t-1}^{-1/2}m_{t}\|] + D_2 \frac{\eta}{T_{\sf M}} \sum_{t=1}^{T_{\sf M}} \EE [ \| \hat{v}_{t-1}^{-1/2} \tilde{g}_{t}\|]  \eqsp,
\end{split}
\eeq
where
\beq\notag
\begin{split}
C_1 =& \frac{\major}{(1 - a\beta_1) + (\beta_1 + a)}  \Delta f \\
& + \frac{ 4L \left(\frac{\beta_1}{1 - \beta_1}\right)^2 \major}{(1 - a\beta_1) + (\beta_1 + a)} \frac{(1 + \beta_1^2) (1- \beta_1)}{(1 - \beta_2)(1-\gamma)} \eqsp ,\\
...
\beq \notag
\begin{split}
&\tilde{C}_1 = C_1 +  \frac{\major \left[ \frac{a(1 - \beta_1)^2}{1-\beta_2} + 2L \frac{1}{1-\beta_2}  \right] }{(1 - a\beta_1) + (\beta_1 + a)}\eqsp, \\
&\tilde{C}_2 = C_2 =\frac{\major\tilde{\major}^2   \EE[ \|\hat{v}_{0}^{-1/2} \|]}{(1 - \beta_1) \left((1 - a\beta_1) + (\beta_1 + a)\right)} \eqsp.
\end{split}
\eeq
\end{proof}
...
\beq \notag
\begin{split}
\|w_t - \tilde{w}_{t-1}\|  & = \|-\eta_t \hat{v}_t^{-1/2} (\theta_t + h_{t+1})\|  \\
& = \|\eta_t \hat{v}_t^{-1/2} (g_t + m_{t+1})\|  \\
...
{\scriptsize \beq\label{eq:gradientatell}
\begin{split}
&\nabla_{w^{(\ell)}}  \mathcal{L}(\textsf{MLN}( w, \xi), y) \\
&  =  \mathcal{L}'(\textsf{MLN}( w, \xi), y) \left(\prod_{j=1}^{\ell+1} \sigma'\left(w^{(j)} h^{(j-1)}(w,\xi) \right) \right) h^{(\ell-1)}(w,\xi)  \eqsp.
\end{split}
\eeq
}

...
\aistatstitle{\huge\texttt{FedSKETCH}: Communication-Efficient and Private Federated Learning via Sketching}
...
In this work, we introduce \texttt{FedSKETCH} and \texttt{FedSKETCHGATE} algorithms to address jointly both challenges.
These algorithms are intended to be used for homogeneous and heterogeneous data distribution settings respectively. 
...
Furthermore, due to the lower dimension of sketching used, our method exhibits communication-efficiency property as well. 
We provide, for the aforementioned schemes, sharp convergence guarantees and back up our theory with various set of experiments.
\end{abstract}


\section{Introduction}
Federated Learning is a recently emerging setting for distributed large scale machine learning problems. 
In Federated Learning, data is distributed across devices (which could be any smartphone or IOT edge device)~\cite{mcmahan2016communication,konevcny2016federated} and due to privacy concerns, users are only allowed to communicate with the parameter server. 
The latter orchestrates optimization among devices by aggregating gradient-related information of devices and broadcasts the average of received vectors. Additionally, moving data across the devices for the purpose of learning a global model can be impractical and could violate the privacy of users/devices~\cite{carlini2019secret,mcmahan2017learning}. 
There are several challenges to be addressed in Federated Learning in order to efficiently learn a global model that performs well in average for all devices. 
...
Since the data in in each device is generated locally in the setting of Federated Learning, it may be distributed according to various probability distributions. 
Thus it is supposed that data distribution is non-iid. It is known that non-iid data distribution can lead to poor convergence error in practice~\cite{li2019federated,liang2019variance}. 
...
Another promising recent approach with a potential to tackle all major issues in Federated Learning setting is based on sketching algorithms~\cite{DBLP:journals/tcs/CharikarCF04,cormode2005improved,kleinberg2003bursty,Proc:Li_Church_Hastie_NIPS08}. 
...
FOcussing on privacy,  in~\cite{li2019privacy} derives a single framework in order to tackle these issues jointly and introduces \texttt{DiffSketch} algorithm, based on the Count Sketch operator. 
...
\end{itemize}
The rest of the paper is organized ass follows.
...
Section~\ref{sec:algos} introduces the main algorithms we propose and their respective analysis in the strongly-convex or nonconvex cases.
Section~\ref{sec:experimnt} presents empirical results for our proposed algorithms.
...
We focus on solving the optimization problem in \eqref{eq:main} for the homogeneous data distribution.
In the heterogeneous setting we consider the special case of $q_1=\ldots=q_p=\frac{1}{p}$. 


\subsection{Related Work}
We now provide a summary of the prior related research efforts.

\noindent\textbf{Local SGD with Periodic Averaging:}
Compared to baseline SGD where model averaging happens in every iteration, the main idea behind \emph{Local SGD with periodic averaging} comes from the intuition of variance reduction by periodic model averaging~\cite{zhang2016parallel} with purpose of saving communication rounds. 
...
We denote by $R$ and $B$ the number of communication rounds and number of bits per round per device respectively. 
...
A common sketching method to tackle \eqref{eq:main}, namely \texttt{Count Sketch} ~(see~\cite{DBLP:journals/tcs/CharikarCF04} for more details), is described Algorithm~\ref{alg:csketch}.
\begin{algorithm}[b]
...
\end{align}
then, the induced compressor $C$ satisfies $C\in\mathbb{U}(\Delta)$ with $\Delta=\Delta_2+\frac{1-\Delta_2}{\Delta_1}$.
\end{lemma}
\begin{remark}
...
We now present two new algorithms for the homogeneous setting.
...
In \texttt{FedSKETCH}, the number of local updates, between two consecutive communication rounds, at device $j$ is denoted by $\tau$.
 Unlike~\cite{haddadpour2020federated}, server node does not store any global model, instead device $j$ has two models, $\boldsymbol{x}^{(r)}$ and $\boldsymbol{x}^{(\ell,r)}_j$ which are local and global models respectively. 
...
We also note that these results can be extended to the scenario where devices are sampled, but for simplicity we do provide its analysis. 
...
In the \textbf{Nonconvex} case, we have:  
     \begin{itemize}
         \item \texttt{FedSKETCH-PRIVIX:} Set $\eta=\frac{1}{L\gamma}\sqrt{\frac{k}{R\tau\left(\frac{\mu^2d}{k}+1\right)}}$ and $\gamma\geq k$,  then $\{ {\boldsymbol{w}}^{(r)}\}_{r=>0}$ satisfies  $\frac{1}{R}\sum_{r=0}^{R-1}\left\|\nabla f({\boldsymbol{w}}^{(r)})\right\|_2^2\leq {\epsilon}$ if we set
     $R=O\left(\frac{1}{\epsilon}\right)$ and $ \tau=O\left(\frac{\mu^2d+1}{{k}\epsilon}\right)$.
         \item \texttt{FedSKETCH-HEAPRIX:} Set $\eta=\frac{1}{L\gamma}\sqrt{\frac{k}{R\tau\left(\frac{\mu^2d-1}{k}+1\right)}}$ and $\gamma\geq k$, then $\{ {\boldsymbol{w}}^{(r)}\}_{r=>0}$ satisfies  $\frac{1}{R}\sum_{r=0}^{R-1}\left\|\nabla f({\boldsymbol{w}}^{(r)})\right\|_2^2\leq {\epsilon}$ if we set
     $R=O\left(\frac{1}{\epsilon}\right)$ and $ \tau=O\left(\frac{\mu^2d}{{k}\epsilon}\right)$. 
     \end{itemize}
 
In the \textbf{PL or Strongly convex} case, we have:  
      \begin{itemize}
          \item \texttt{FedSKETCH-PRIVIX} Set  $\eta=\frac{1}{2L\left(\frac{\mu^2d}{k}+1\right)\tau\gamma}$ and $\gamma\geq k$, then $\{ {\boldsymbol{w}}^{(r)}\}_{r=>0}$ satisfies $\mathbb{E}\Big[f({\boldsymbol{w}}^{(R)})-f({\boldsymbol{w}}^{(*)})\Big]\leq \epsilon$ if  we set
     $R=O\left(\left(\frac{\mu^2d}{k}+1\right)\kappa\log\left(\frac{1}{\epsilon}\right)\right)$ and $ \tau=O\left(\frac{\mu^2d+1}{k\left(\frac{\mu^2d}{k}+1\right)\epsilon}\right)$.
         ...
         \item \texttt{FedSKETCH-HEAPRIX:} Set   $\eta=\frac{1}{2L\left(\frac{\mu^2d-1}{k}+1\right)\tau\gamma}$ and $\gamma\geq k$, then $\{ {\boldsymbol{w}}^{(r)}\}_{r=>0}$ satisfies $\mathbb{E}\Big[f({\boldsymbol{w}}^{(R)})-f({\boldsymbol{w}}^{(*)})\Big]\leq \epsilon$ if  we set
...
\begin{figure*}[t]
	\begin{center}
		\mbox{			   
		\includegraphics[width=1.8in]{MNIST_figures/local1_sketch20_iid0_train_loss.eps} \hspace{-0.2in}
		\includegraphics[width=1.8in]{MNIST_figures/local2_sketch20_iid0_train_loss.eps} \hspace{-0.2in}
%		}
%		\mbox{
		\includegraphics[width=1.8in]{MNIST_figures/local5_sketch20_iid0_train_loss.eps} \hspace{-0.2in}
				\includegraphics[width=1.8in]{MNIST_figures/local1_sketch20_iid0_test_acc.eps} \hspace{-0.2in}
		}
		\mbox{
		\includegraphics[width=1.8in]{MNIST_figures/local2_sketch20_iid0_test_acc.eps} \hspace{-0.2in}
		\includegraphics[width=1.8in]{MNIST_figures/local5_sketch20_iid0_test_acc.eps}\hspace{-0.2in}
%		}
%		\mbox{			  
		 \includegraphics[width=1.8in]{MNIST_figures/local1_sketch50_iid0_train_loss.eps} \hspace{-0.2in}
		\includegraphics[width=1.8in]{MNIST_figures/local2_sketch50_iid0_train_loss.eps} \hspace{-0.2in}
		}
		\mbox{
		\includegraphics[width=1.8in]{MNIST_figures/local5_sketch50_iid0_train_loss.eps}\hspace{-0.2in}
				\includegraphics[width=1.8in]{MNIST_figures/local1_sketch50_iid0_test_acc.eps} \hspace{-0.2in}
%		}
%		\mbox{
		\includegraphics[width=1.8in]{MNIST_figures/local2_sketch50_iid0_test_acc.eps} \hspace{-0.2in}
		\includegraphics[width=1.8in]{MNIST_figures/local5_sketch50_iid0_test_acc.eps}
		}
	\end{center}

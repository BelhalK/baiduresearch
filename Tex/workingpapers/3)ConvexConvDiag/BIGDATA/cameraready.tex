...
\begin{algorithm}[ht]
\SetKwInOut{Input}{input}
\Input{Algorithm~\ref{alg:diagnostic} {\tt SGDM}$(\theta, \gamma)$,  initial and minimum stepsize $\gamma_0$, $\gamma_{min}$, reduction $\rho \in (0,1)$ 
}
$\gamma \gets \gamma_0$ \\
...
\newblock {\em Computational Optimization and Applications}, pages 1--58, 2020.
...
\newblock {\em arXiv preprint arXiv:1802.04876}, 2018.
...
\newblock {\em arXiv preprint arXiv:1903.01435}, 2020.
...
\noindent\textbf{Assumption 3.} \ Theorem~\ref{thm:mom_convg_analysis}~\cite{yang2016unified} holds s.t. $\Ex{ f(\theta_n) - f(\theta_\star) } \leq \gamma M$ for some $M > 0$ and large enough $n$. 
...
\begin{proof}
From Theorem~\ref{thm:mom_convg_analysis}~\cite{yang2016unified}, we can see that for large enough $n$, this is a valid condition.

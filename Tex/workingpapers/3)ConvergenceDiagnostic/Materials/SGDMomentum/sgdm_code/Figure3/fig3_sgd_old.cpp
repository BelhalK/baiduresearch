...
param_struct init_params(
	...
	params.var.burnin                = NumericVector(10);	
	params.var.convg                 = NumericVector(10);
...
void batch_update(
	...
	else if (j > last_burnin+250 && params->var.burnin_done==true && params->var.convg_done==false) {
	...
	/* repeat LR */
	else if (params->args.autoLR==true && params->var.convg_done==true && params->var.lr > 1e-5) {
			params->var.lr         *= 0.10;
			params->var.burnin_done = false;
			params->var.convg_done  = false;

...
We denote
\beq\label{eq:stats}
\hat{S}^{(k+1)} = \frac{1}{n} \sum_{i=1}^n \hat{S}^{(k+1)}_i = \frac{1}{n} \sum_{i=1}^n\frac{1}{M} \sum_{m=1}^M S(z_{i,m}^{(k)}, y_i)
\eeq
where $z_{i,m}^{(k)} \sim p(z_i|y_i;\theta^{(k)})$.
At iteration $k$, the sufficient statistics $\hat{\bss}^{(k+1)}$ is approximated as follows:

\beq\label{eq:rmstep}
\textsf{SA-step}:~ \hat{\bss}^{(k+1)} =  \hat{\bss}^{(k)}  + \gamma_{k+1}(\hat{S}^{(k+1)} - \hat{\bss}^{(k)} )
\eeq
...
\beq \label{eq:sestep}
\textsf{Inexact-step}:~\hat{S}^{(k+1)} = \hat{S}^{(k)} + \rho_{k+1} \big( \StocEstep^{(k+1)}- \hat{S}^{(k)}  \big),
\eeq
Note $\{ \rho_{k} \}_{k=1}^\infty \in [0,1]$ is a sequence of step sizes, $\StocEstep^{(k)}$ is a proxy for $\hat{S}^{(k)}$,
...
Also if $\rho_{k}=1$, $\gamma_{k}=1$ and $\StocEstep^{(k)} = \hat{S}^{(k)}$, then we recover the Monte Carlo EM algorithm.
...
\begin{align}
&\emph{(\ISAEM\ \citep{kuhn2019properties, karimi2019non})} & \StocEstep^{(k)} &= \StocEstep^{(k)} + {\textstyle \frac{1}{n}}\big( \hat{S}_{i_k}^{(k)}  - \hat{S}_{i_k}^{(\tau_{i_k}^k)} \big) \label{eq:isaem} \\
&\emph{(\SAEMVR\ This paper )} &\StocEstep^{(k+1)} &= \hat{S}^{(\ell(k))} +  \big( \hat{S}_{i_k}^{(k-1)}  -\hat{S}_{i_k}^{(\ell(k))}   \big) \label{eq:vrsaem}\\
...
\eta_{i, \vartheta}^{(r)} \eqdef \hat{S}_{i}^{(r)} -  \overline{\bss}_i(\vartheta)
\eeq
...
\EE\left[\norm{\eta_{i, \vartheta}^{(r)}}^2 \right] \leq \frac{C_{\eta}}{M_r} \quad \textrm{and} \quad \EE\left[\EE[\eta_{i, \vartheta}^{(r)}|{\cal F}_r]\right] \leq \frac{C}{M_r}
\eeq
\end{assumption}
...
\beq
\EE\left[\hat{S}^{(k+1)} - \hat{\bss}^{(k)}|{\cal F}_k \right] = (\overline{\bss}^{(k)} - \hat{\bss}^{(k)}) +  \left(1 - \frac{1}{n} \right)  \left(n^{-1}\sum_{i=1}^n \hat{S}_i^{(\tau_i^k)} -  \overline{\bss}^{(k)} \right) + \frac{C}{M_k}
\eeq
...
Let $\left\{\gamma_{k}, k \in \mathbb{N}\right\}$ be a sequence of positive step sizes and consider the \ISAEM\ sequence $\left\{\widehat{S}^{k}, k \in \mathbb{N}\right\}$ obtained with $\rho_{k+1}=1$ for any $k$.
\end{Theorem} 

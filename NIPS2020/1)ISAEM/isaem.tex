...
\beq
\begin{split}
 \tilde{S}^{(k+1)} - \hs{k} = & \rho (\overline{\bss}^{(k)}-\hs{k}) + \rho \eta_{i_k}^{(k+1)}+ \rho \left[\big(\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(k)}\big) - \EE[\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(k)}] \right] \\
...
\end{split}
\eeq
where we recall that $\eta_{i_k}^{(k+1)}$, defined in \eqref{eq:boundederror}, is the gap between the MC approximation and the expected statistics.
\end{Lemma*}
\begin{proof}
...
&= \rho (\overline{\bss}^{(k)}-\hs{k}) + \rho \eta_{i_k}^{(k+1)}+ \rho \left[\big(\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(k)}\big) - \EE[\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(k)}] \right] \\
 &+ (1-\rho)\left(\tilde{S}^{(k)} - \hs{k}\right)
\end{split}
\eeq
where we observe that $\EE[\overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(k)}] =\overline{\bss}^{(k)} -   \overline{\StocEstep}^{(k)} $ which concludes the proof.
\end{proof}
\begin{Lemma*} 
 Assume H\ref{ass:convset}. The update \eqref{eq:fisaem} is equivalent to the following update:
\beq
...
\end{Lemma*}
\begin{proof}
Denote $H_{k+1} \eqdef   \tilde{S}^{(k+1)} - \hs{k}$ the drift term of the \FISAEM\ update in \eqref{eq:rmstep}.
Using Lemma~\ref{lem:drift_fisaem} we observe that $\EE[H_{k+1}|{\cal F}_k] =  \rho (\overline{\bss}^{(k)}-\hs{k})  + \rho\EE[\eta_{i_k}^{(k+1)}|{\cal F}_k] + (1-\rho) \EE[\tilde{S}^{(k)} - \hs{k}]$  where ${\cal F}_k$ is the filtration up to iteration $k$.
Thus
...
\beq
\begin{split}
\EE\left[\norm{ \tilde{S}^{(k+1)} - \hs{k}}^2 \right] \leq see
\end{split}
\eeq
\end{Lemma}
\begin{proof}
...
\paragraph{ Bounding $\EE[ \|  \hs{k} -  \tilde{S}^{(k+1)}  \|^2]  $} 

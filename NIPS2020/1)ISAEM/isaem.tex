...
&  \leq  4 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{k-\ell}  \EE[  \|  \os^{(\ell)} - \hs{\ell}  \|^2 ] \\
& + 2 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2  +\frac{\gamma_{k+1}}{\beta}  \big)\sum_{ \ell = 0 }^k  (1- \Lambda_{(\ell+1)})^{k-\ell} \EE \left[\norm{ \eta_{i_\ell}^{(\ell)}}^2 \right]\\

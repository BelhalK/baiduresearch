...
\beq
\begin{split}
\EE\big[\norm{ \hs{k} -  \tilde{S}^{(k)} }^2\big]  \leq ifjrie
\end{split}
\eeq
\end{Lemma}
...
& + \frac{\gamma_{k+1}^2 \Lip{V}\Lip{\bss}^2}{n^3} \sum_{i=1}^n \EE[ \| \hs{k} - \hs{t_i^k} \|^2 ]
\end{split}
\eeq
...
& = \EE \Big[ \| \hs{k+1} - \hs{k} \|^2 + \| \hs{k} - \hs{t_i^k} \|^2 - 2 \gamma_{k+1} \pscal{ \hs{k} - \tilde{S}^{(k+1)} }{\hs{k}- \hs{t_i^k}} \Big] \\
& \leq  \EE \Big[ \| \hs{k+1} - \hs{k} \|^2 + \| \hs{k} - \hs{t_i^k} \|^2 +  \frac{\gamma_{k+1}}{\beta} \| \hs{k} - \tilde{S}^{(k+1)}\|^2 + \gamma_{k+1} \beta \| \hs{k}- \hs{t_i^k} \|^2 \Big]
\end{split}
\eeq
...
& \leq \EE[  \| \hs{k+1} - \hs{k} \|^2 ] + \frac{n-1}{n^2} \sum_{i=1}^n \EE \Big[ (1+\gamma_{k+1} \beta) \|  \hs{k} - \hs{t_i^k} \|^2 + \frac{\gamma_{k+1}}{\beta} \|  \hs{k} - \tilde{S}^{(k+1)} \|^2 \Big]
\end{split}
\eeq
...
\begin{split}
& \frac{1}{n} \sum_{i=1}^n \EE[ \| \hs{k+1} - \hs{t_i^{k+1}} \|^2 ] \\
& \leq \big(\gamma_{k+1}^2 +\frac{n-1}{n}\frac{\gamma_{k+1}}{\beta}  \big)\EE \Big[  \|   \tilde{S}^{(k+1)} -  \hs{k} \|^2  \Big] + \sum_{i=1}^n \EE \Big[  \frac{1 - \frac{1}{n} + \gamma_{k+1} \beta}{n} \|  \hs{k} - \hs{t_i^k} \|^2  \Big] \\
...
\beq
\begin{split}
\Delta^{(k+1)} \leq & 4 \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k (1- \Lambda_{(\ell+1)})^{k-\ell}  \EE[  \|  \os^{(\ell)} - \hs{\ell}  \|^2 ] + 2\big(\gamma_{k+1}^2  +\frac{\gamma_{k+1}}{\beta}  \big)\sum_{ \ell = 0 }^k (1- \Lambda_{(\ell+1)})^{k-\ell} \EE \left[\norm{ \eta_{i_\ell}^{(\ell)}}^2 \right]\\
& +  4 \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{k-\ell}  \EE\left[\norm{ \frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^\ell)}-  \overline{\bss}^{(\ell)}}^2\right]
\end{split}
\eeq
...
&  \leq  4 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{k-\ell}  \EE[  \|  \os^{(\ell)} - \hs{\ell}  \|^2 ] + 2 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2  +\frac{\gamma_{k+1}}{\beta}  \big)\sum_{ \ell = 0 }^k  (1- \Lambda_{(\ell+1)})^{k-\ell} \EE \left[\norm{ \eta_{i_\ell}^{(\ell)}}^2 \right]\\
& +  \sum_{k=0}^{K_{\max}-1} 4 \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{k-\ell}  \EE\left[\norm{ \frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^\ell)}-  \overline{\bss}^{(\ell)}}^2\right]\\
&=  4 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{\ell}  \EE[  \|  \os^{(k)} - \hs{k}  \|^2 ] + 2 \sum_{k=0}^{K_{\max}-1} \big(\gamma_{k+1}^2  +\frac{\gamma_{k+1}}{\beta}  \big)\sum_{ \ell = 0 }^k  (1- \Lambda_{(\ell+1)})^{\ell} \EE \left[\norm{ \eta_{i_\ell}^{(k)}}^2 \right]\\
& +  \sum_{k=0}^{K_{\max}-1} 4 \big(\gamma_{k+1}^2 +\frac{\gamma_{k+1}}{\beta}  \big) \sum_{ \ell = 0 }^k   (1- \Lambda_{(\ell+1)})^{\ell}  \EE\left[\norm{ \frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^k)}-  \overline{\bss}^{(k)}}^2\right]\\
...
& +\sum_{k=0}^{K_{\max}-1} \frac{\gamma_{k+1}^2 \Lip{V}\Lip{\bss}^2}{n^2} \Delta^{(k+1)}
\end{split}
\eeq
and we plug \eqref{eq:Delta} which result in:

\beq
\begin{split}
\sum_{k=0}^{K_{\max}-1}  \tilde{\alpha}_k \EE \left[\norm{  \overline{\bss}^{(k)}  - \hs{k}}^2  \right] + \sum_{k=0}^{K_{\max}-1}  \tilde{\beta}_k \EE\left[\norm{ \frac{1}{n} \sum_{i=1}^n \tilde{S}_i^{(\tau_i^k)}-  \overline{\bss}^{(k)}}^2\right] \leq   \EE \left[ V( \hs{0} ) - V( \hs{K} ) \right]+ \sum_{k=0}^{K_{\max}-1} \tilde{\Gamma}_k         \EE \left[\norm{ \eta_{i_k}^{(k)}}^2 \right] 
\end{split}
\eeq
where:
\begin{align*}
...
When, for any $k >0$, we have by Lemma~\ref{lem:growth} that:
\beq
...
\beq
\begin{split}
\EE\big[\norm{ \tilde{S}^{(k)} - \hs{k}  }^2\big]  \leq ifjrie
\end{split}
\eeq
\end{Lemma*}
\begin{proof}
...
\beq
\begin{split}
 \EE\left[\norm{\tilde{S}^{(k+1)} - \hs{k}}^2 \right]  + \rho^2 \EE\left[\| \frac{1}{n} \sum_{i=1}^n \tilde{S}_{i}^{(t_i^k)}  -  \overline{\bss}^{(k)} \| \right] \leq & \rho^2 \left(2\EE\left[\norm{\overline{\bss}^{(k)}-\hs{k}}^2 \right]+\EE\left[\| \overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)} \|\right] \right) \\
...
\begin{split}
 ...
 &+ \rho^2 \EE[\| \overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)} \|] -\rho^2 \EE[\|\overline{\StocEstep}^{(k)}  -  \overline{\bss}^{(k)} \|]
\end{split}
\eeq
...
\beq
\begin{split}
\EE\left[\norm{\Hdrift_{k+1}}^2 \right]  + \rho^2 \EE[\|\frac{1}{n} \sum_{i=1}^n \tilde{S}_{i}^{(t_i^k)} -  \overline{\bss}^{(k)} \|] \leq & \rho^2 \left(2\EE\left[\norm{\hmean_{k}}^2 \right] +\EE[\| \overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)} \|]\right) \\
& + 2(1-\rho)^2\EE\left[ \norm{\tilde{S}^{(k)} - \hs{k}}^2\right] + \epsilon^{(k+1)}
\end{split}
\eeq
...
& \overset{(b)}{\leq}  -(\upsilon_{\min} - \upsilon_{\max}^2\beta) \rho \EE\left[\norm{\hmean_{k}}^2 \right] +\frac{\rho^2}{2\beta} \xi^{(k+1)} + \frac{(1-\rho)^2}{2\beta} \EE[\norm{\tilde{S}^{(k)} - \hs{k}}^2]
\end{split}
\eeq
...
\beq
\begin{split}
\gamma_{k+1}(\upsilon_{\min} - \upsilon_{\max}^2\beta) \rho \EE\left[\norm{\hmean_{k}}^2 \right]  \leq& \EE\left[V( \hs{k} ) - V( \hs{k+1} ) \right]+\frac{\gamma_{k+1}\rho^2}{2\beta} \xi^{(k+1)} \\
& + \frac{(1-\rho)^2\gamma_{k+1}}{2\beta} \EE[\norm{\tilde{S}^{(k)} - \hs{k}}^2] + \frac{\gamma_{k+1}^2 \Lip{V}}{2} \EE\left[\|  \Hdrift_{k+1}  \|^2\right]
\end{split}
\eeq
...
\beq
\begin{split}
&\gamma_{k+1}\left(\upsilon_{\min} - \upsilon_{\max}^2\beta  - \Lip{V} \rho^2\right) \rho \EE\left[\norm{\hmean_{k}}^2 \right]  + \frac{\rho^2\gamma_{k+1}^2 \Lip{V}}{2}\EE\left[\| \frac{1}{n} \sum_{i=1}^n \tilde{S}_{i}^{(t_i^k)}  -  \overline{\bss}^{(k)} \| \right] \\
&\leq \EE\left[V( \hs{k} ) - V( \hs{k+1} ) \right] + \tilde{\epsilon}^{(k+1)}\\
& + \left( (1-\rho)^2\gamma_{k+1}^2 \Lip{V} + \frac{(1-\rho)^2\gamma_{k+1}}{2\beta} \right) \EE\left[\norm{\tilde{S}^{(k)} - \hs{k}}^2 \right] +\frac{\rho^2\gamma_{k+1}^2 \Lip{V}}{2} \EE\left[\| \overline{\bss}_{i_k}^{(k)} - \tilde{S}_{i_k}^{(t_{i_k}^k)} \|\right]
\end{split}
\eeq

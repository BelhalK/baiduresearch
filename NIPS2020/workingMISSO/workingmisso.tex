...
\begin{assumption}\label{controlapprox}
For all $i \in \inter$, $(\theta,\op) \in \Param^2$, $z_i \in \Zset$ we assume the existence of a function $C_{\sf r}: \Zset \to \rset$ such that:
\beq
\begin{split}
 \sup \limits_{M >0} \frac{1}{\sqrt{M}}  \sum_{m=1}^{M}{ \left\{ r_i (\param ; \op, z_{i,m})  - \sur{i}{\param}{\op} \right\} } & < C_{\sf r}(z_i)\\
 \mathbb{E}_{\op} [C_{\sf r}(z_i) |{\cal F}] & < \infty
\end{split}
\eeq
where ${\cal F}$ is the filtration of the total randomness. Besides, 
\beq
\begin{split}
 \sup \limits_{M >0} \frac{1}{\sqrt{M}}  \sum_{m=1}^{M}{ \left\{           \frac{
 \widehat{\cal L}_i'( \param , \param - \op; \op ) - r_i' (\param, \param - \op ; \op,  z_{i,m} ) }{\| \op - \param\|}            \right\} } & < C_{\sf gr}(z_i)\\
 \mathbb{E}_{\op} [C_{\sf gr}(z_i) |{\cal F}] & < \infty
\end{split}
\eeq
\end{assumption}
...
Have Logistic Regression with missing values and MNIST Bayesian LeNet5 from last year paper.

Working on CIFAR-10 using Bayesian ResNet.

...
\begin{assumption}\label{controlapprox}
For all $i \in \inter$, $(\theta,\op) \in \Param^2$, $z_i \in \Zset$ we assume the existence of an majorizing function $m_{\sf r}: \Zset \to \rset$ and a constant $C_{\sf r} < \infty$ such that:
\beq
\begin{split}
 \sup \limits_{M >0} \frac{1}{\sqrt{M}}  \sum_{m=1}^{M}{ \left\{ r_i (\param ; \op, z_{i,m})  - \sur{i}{\param}{\op} \right\} } < m_{\sf r}(z_i)
\quad \textrm{and} \quad  \mathbb{E}_{\op} \big[m_{\sf r}(z_i) |{\cal F}\big] < C_{\sf r} 
\end{split}
\eeq
where ${\cal F}$ is the filtration of the total randomness and we denoted by $\mathbb{E}_{\op} [\cdot]$ the expectation \wrt a Markov chain $\{z_{i,m}\}_{m=1}^{M}$ with  initial distribution $\xi_{i} (\cdot; \op)$, transition kernel $P_{i,\op}$, and stationary distribution $p_{i}(\cdot; \op)$. Besides,
\beq
\begin{split}
 \sup \limits_{M >0} \frac{1}{\sqrt{M}}  \sum_{m=1}^{M}{ \left\{  \frac{
 \widehat{\cal L}_i'( \param , \param - \op; \op ) - r_i' (\param, \param - \op ; \op,  z_{i,m} ) }{\| \op - \param\|}            \right\} } < m_{\sf gr}(z_i)
 \quad \textrm{and} \quad  \mathbb{E}_{\op} \big[m_{\sf gr}(z_i) |{\cal F}\big] < C_{\sf gr} 
\end{split}
\eeq
\end{assumption}
...
$N_{\mathrm{l} 1}\left(\varepsilon\|m\|_{P, r}, \mathcal{F}, L_{r}(P)\right) \leq K\left(\frac{\operatorname{diam} \Theta}{\varepsilon}\right)^{d}, \quad$ every $0<\varepsilon<\operatorname{diam} \Theta$

\beq
\underset{f \in \mathcal{F}}{\mathbb{E} \sup _{f}\left|X_{f}\right|}=\underset{f \in \mathcal{F}}{\operatorname{Esup}}\left|X_{f}-X_{0}\right| \leq \frac{1}{\sqrt{M}} \int_{0}^{1} \sqrt{\log \mathcal{N}\left(\mathcal{F},\|\cdot\|_{\infty}, \varepsilon\right)} d \varepsilon
\eeq
...
& {\bm \Omega}^{(k)} = \arg \min \limits_{\Omega \in \Theta} \frac{1}{n} \sum_{i=1}^{n}  \tilde{\mathcal{L}}^{(2)}_i(\beta^{(k)}, \Omega,\theta^{(\tau_i^k)},\{z_{i,m}\}_{m=1}^{M_{(\tau_i^k)}})  =   \frac{1}{n}\sum_{i=1}^{n} \frac{1}{M_{(\tau_i^k)}}\sum_{m=1}^{M_{(\tau_i^k)}}z_{i,m}^{(k)}(z_{i,m}^{(k)})^\top  -  {\bm \beta}^{(k)} ({\bm \beta}^{(k)})^\top \\
...
See Appendix \ref{app:update_logistic} for more explanation.
...
\paragraph{Bayesian LeNet-5 on MNIST:}
...
\paragraph{Bayesian ResNet-18 on CIFAR-10:}

Put here the architectureof ResNet-18 introduced in \citep{he2016deep}.
